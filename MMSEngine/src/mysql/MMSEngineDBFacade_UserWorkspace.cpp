...
shared_ptr<Workspace> MMSEngineDBFacade::getWorkspace(int64_t workspaceKey)
{
	...
	try
	{
		...
		lastSQLCommand = "select workspaceKey, name, directoryName, maxStorageInMB, maxEncodingPriority "
		...
		else
		{
			...
			string errorMessage =
				__FILEREF__ + "select failed" + ", workspaceKey: " + to_string(workspaceKey) + ", lastSQLCommand: " + lastSQLCommand;
...
shared_ptr<Workspace> MMSEngineDBFacade::getWorkspace(string workspaceName)
{
	...
	try
	{
		...
		shared_ptr<sql::ResultSet> resultSet(preparedStatement->executeQuery());
		_logger->info(
			__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceName: " + workspaceName +
			", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
			to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
		);
		...
		else
		{
			...
			string errorMessage = __FILEREF__ + "select failed" + ", workspaceName: " + workspaceName + ", lastSQLCommand: " + lastSQLCommand;
			_logger->error(errorMessage);
...
)
{
	...
	try
	{
...

		{
			...
			lastSQLCommand = "insert into MMS_User (userKey, name, eMailAddress, password, country, "
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", trimUserName: " + trimUserName +
				", userEmailAddress: " + userEmailAddress + ", userPassword: " + userPassword + ", userCountry: " + userCountry +
				", strExpirationDate: " + strExpirationDate + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
		}
...

		{
			...
			pair<int64_t, string> workspaceKeyAndConfirmationCode = addWorkspace(
				conn, userKey, admin, createRemoveWorkspace, ingestWorkflow, createProfiles, deliveryAuthorization, shareWorkspace, editMedia,
				editConfiguration, killEncoding, cancelIngestionJob, editEncodersPool, applicationRecorder, trimWorkspaceName, workspaceType,
				deliveryURL, maxEncodingPriority, encodingPeriod, maxIngestionsNumber, maxStorageInMB, languageCode, userExpirationDate
			);
...
)
{
	...
	try
	{
...

		{
			...
			{
				lastSQLCommand = "select workspaceKey from MMS_Code "
				...
				shared_ptr<sql::ResultSet> resultSet(preparedStatement->executeQuery());
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", code: " + shareWorkspaceCode +
					", userEmail: " + userEmailAddress + ", type: " + toString(CodeType::ShareWorkspace) +
					", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (resultSet->next())
			...
			{
				lastSQLCommand = "update MMS_Code set userKey = ?, type = ? "
								 "where code = ?";
				shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
				int queryParameterIndex = 1;
				...
				_logger->info(
					...
					", type: " + toString(CodeType::UserRegistrationComingFromShareWorkspace) + ", code: " + shareWorkspaceCode +
				...
				if (rowsUpdated != 1)
				{
					string errorMessage = __FILEREF__ + "Code update failed" + ", rowsUpdated: " + to_string(rowsUpdated) +
										  ", lastSQLCommand: " + lastSQLCommand + ", shareWorkspaceCode: " + shareWorkspaceCode;
					_logger->error(errorMessage);
...
)
{
	...
	try
	{
		...
		code = createCode(
			conn, workspaceKey, userKey, userEmail, codeType, admin, createRemoveWorkspace, ingestWorkflow, createProfiles, deliveryAuthorization,
	...
	catch (sql::SQLException &se)
	{
		...
		_logger->error(
			__FILEREF__ + "SQL exception" + ", exceptionMessage: " + exceptionMessage +
			", conn: " + (conn != nullptr ? to_string(conn->getConnectionId()) : "-1")
		);
	...
	{
		_logger->error(
			__FILEREF__ + "SQL exception" + ", e.what(): " + e.what() + ", conn: " + (conn != nullptr ? to_string(conn->getConnectionId()) : "-1")
		);
	...
	{
		_logger->error(__FILEREF__ + "SQL exception" + ", conn: " + (conn != nullptr ? to_string(conn->getConnectionId()) : "-1"));
...
)
{
	...
	try
	{
...

		{
			...
			try
			{
				lastSQLCommand = "insert into MMS_Code (code, workspaceKey, userKey, userEmail, "
				...
				preparedStatement->executeUpdate();
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", code: " + code +
					", workspaceKey: " + to_string(workspaceKey) + ", userKey: " + to_string(userKey) + ", userEmail: " + userEmail +
					", type: " + toString(codeType) + ", permissions: " + permissions + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
			}
			catch (sql::SQLException &se)
	...
	try
	{
...

		{
			...
			string strExpirationDate;
			{
				...
				/*
				sprintf(
					strExpirationDate, "%04d-%02d-%02d %02d:%02d:%02d", tmDateTime.tm_year + 1900, tmDateTime.tm_mon + 1, tmDateTime.tm_mday,
					tmDateTime.tm_hour, tmDateTime.tm_min, tmDateTime.tm_sec
				);
				*/
				strExpirationDate = fmt::format(
			...
			preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", userName: " + userName + ", userEmailAddress: " +
				userEmailAddress + ", userPassword: " + userPassword + ", userCountry: " + userCountry + ", strExpirationDate: " + strExpirationDate +
				", elapsed (secs): @" + to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			...
			while (getline(ssDefaultWorkspaceKeys, defaultWorkspaceKey, separator))
			{
				if (!defaultWorkspaceKey.empty())
				{
					...
					string localApiKey = createAPIKeyForActiveDirectoryUser(
						conn, userKey, userEmailAddress, createRemoveWorkspace, ingestWorkflow, createProfiles, deliveryAuthorization, shareWorkspace,
						editMedia, editConfiguration, killEncoding, cancelIngestionJob, editEncodersPool, applicationRecorder, llDefaultWorkspaceKey,
...
)
{
	...
	try
	{
		...
		apiKey = createAPIKeyForActiveDirectoryUser(
			conn, userKey, userEmailAddress, createRemoveWorkspace, ingestWorkflow, createProfiles, deliveryAuthorization, shareWorkspace, editMedia,
...
string MMSEngineDBFacade::createAPIKeyForActiveDirectoryUser(
	shared_ptr<MySQLConnection> conn, int64_t userKey, string userEmailAddress, bool createRemoveWorkspace, bool ingestWorkflow, bool createProfiles,
	bool deliveryAuthorization, bool shareWorkspace, bool editMedia, bool editConfiguration, bool killEncoding, bool cancelIngestionJob,
...
)
{
	...
	try
	{
		// create the API of the user for each existing Workspace
		{
			...
			lastSQLCommand = "insert into MMS_APIKey (apiKey, userKey, workspaceKey, isOwner, isDefault, "
							 "permissions, creationDate, expirationDate) values ("
							 "?, ?, ?, ?, ?, ?, NOW(), STR_TO_DATE(?, '%Y-%m-%d %H:%i:%S'))";
			shared_ptr<sql::PreparedStatement> preparedStatementAPIKey(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", apiKey: " + apiKey +
				", userKey: " + to_string(userKey) + ", workspaceKey: " + to_string(workspaceKey) + ", isOwner: " + to_string(isOwner) +
				", isDefault: " + to_string(isDefault) + ", permissions: " + permissions + ", strExpirationDate: " + strExpirationDate +
				", elapsed (secs): @" + to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);

			addWorkspaceForAdminUsers(conn, workspaceKey, expirationInDaysWorkspaceDefaultValue);
		}
	}
	catch (sql::SQLException &se)
	{
	...
	{
		{
			...
			lastSQLCommand = "insert into MMS_Workspace ("
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceName: " + workspaceName +
				", workspaceDirectoryName: " + workspaceDirectoryName + ", workspaceType: " + to_string(static_cast<int>(workspaceType)) +
				", deliveryURL: " + deliveryURL + ", enabled: " + to_string(enabled) +
				", maxEncodingPriority: " + MMSEngineDBFacade::toString(maxEncodingPriority) + ", encodingPeriod: " + toString(encodingPeriod) +
				", maxIngestionsNumber: " + to_string(maxIngestionsNumber) + ", maxStorageInMB: " + to_string(maxStorageInMB) +
				", languageCode: " + languageCode + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
		}
		...
		{
			lastSQLCommand = "update MMS_Workspace set directoryName = ? where workspaceKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceKey: " + to_string(workspaceKey) +
				", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (rowsUpdated != 1)
			{
		...
		confirmationCode = MMSEngineDBFacade::createCode(
			conn, workspaceKey, userKey, "", // userEmail,
		...
		{
			lastSQLCommand = "insert into MMS_WorkspaceMoreInfo (workspaceKey, currentDirLevel1, "
		...
		{
			lastSQLCommand = "insert into MMS_EncodersPool(workspaceKey, label, lastEncoderIndexUsed) "
							 "values (?, NULL, 0)";
		...
		{
			lastSQLCommand = "insert into MMS_ContentProvider (contentProviderKey, workspaceKey, name) "
							 "values (NULL, ?, ?)";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, workspaceKey);
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceKey: " + to_string(workspaceKey) +
				", _defaultContentProviderName: " + _defaultContentProviderName + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
		}
...
tuple<string, string, string> MMSEngineDBFacade::confirmRegistration(string confirmationCode, int expirationInDaysWorkspaceDefaultValue)
{
	...
	try
	{
		...
		CodeType codeType;
		{
			lastSQLCommand = "select userKey, permissions, workspaceKey, type "
							 "from MMS_Code "
		...
		{
			lastSQLCommand = "select apiKey from MMS_APIKey where userKey = ? and workspaceKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, userKey);
			preparedStatement->setInt64(queryParameterIndex++, workspaceKey);
		...
		if (!apiKeyAlreadyPresent)
		{
			...
			if (codeType == CodeType::UserRegistration)
			{
				...
				lastSQLCommand = "update MMS_Workspace set isEnabled = ? where workspaceKey = ?";
				shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
				int queryParameterIndex = 1;
				preparedStatement->setInt(queryParameterIndex++, enabled);
				preparedStatement->setInt64(queryParameterIndex++, workspaceKey);
				...
				int rowsUpdated = preparedStatement->executeUpdate();
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", enabled: " + to_string(enabled) +
					", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (rowsUpdated != 1)
		...
		string name;
		{
			lastSQLCommand = "select name, eMailAddress from MMS_User where userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, userKey);
			...
			{
				string errorMessage =
					__FILEREF__ + "User are not present" + ", userKey: " + to_string(userKey) + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
...
void MMSEngineDBFacade::addWorkspaceForAdminUsers(shared_ptr<MySQLConnection> conn, int64_t workspaceKey, int expirationInDaysWorkspaceDefaultValue)
{
	string apiKey;
	string lastSQLCommand;
	...
	try
	{
		...
		for (string adminEmailAddress : _adminEmailAddresses)
		{
			int64_t userKey;
			{
				lastSQLCommand = "select userKey from MMS_User "
								 "where eMailAddress = ?";
				...
				shared_ptr<sql::ResultSet> resultSet(preparedStatement->executeQuery());
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", adminEmailAddress: " + adminEmailAddress +
					", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (resultSet->next())
			...
			{
				lastSQLCommand = "select count(*) from MMS_APIKey "
				...
				{
					string errorMessage = __FILEREF__ + "count(*) has to return a row" + ", userKey: " + to_string(userKey) +
										  ", workspaceKey: " + to_string(workspaceKey) + ", lastSQLCommand: " + lastSQLCommand;
					_logger->error(errorMessage);
...
vector<tuple<int64_t, string, string>> MMSEngineDBFacade::deleteWorkspace(int64_t userKey, int64_t workspaceKey)
{
	...
	try
	{
		...
		bool isOwner = false;
		{
			lastSQLCommand = "select isOwner, permissions "
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceKey: " + to_string(workspaceKey) +
				", userKey: " + to_string(userKey) + ", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (resultSet->next())
			{
		...
		{
			lastSQLCommand = "select u.userKey, u.name, u.eMailAddress from MMS_APIKey ak, MMS_User u "
			...
			while (resultSet->next())
			{
				...
				lastSQLCommand = "select count(*) from MMS_APIKey where userKey = ?";
				shared_ptr<sql::PreparedStatement> countPreparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
				int queryParameterIndex = 1;
				...
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", userKey: " + to_string(userKey) +
					", countResultSet->rowsCount: " + to_string(countResultSet->rowsCount()) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
...

		{
			...
			lastSQLCommand = "delete from MMS_Workspace where workspaceKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, workspaceKey);
		...
		if (usersToBeRemoved.size() > 0)
		{
			...
			// So all should be removed automatically from DB
			lastSQLCommand = "delete from MMS_User where userKey in (" + sUsers + ")";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			int rowsUpdated = preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", rowsUpdated: " + to_string(rowsUpdated) +
				", elapsed (secs): @" + to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
		}
...
MMSEngineDBFacade::checkAPIKey(string apiKey, bool fromMaster)
{
	...
	try
	{
		...
		{
			lastSQLCommand = "select userKey, workspaceKey, permissions from MMS_APIKey "
			...
			{
				string errorMessage =
					__FILEREF__ + "apiKey is not present or it is expired" + ", apiKey: " + apiKey + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
...
Json::Value MMSEngineDBFacade::login(string eMailAddress, string password)
{
	...
	int64_t userKey = -1;
	{
		...
		try
		{
			...
			{
				lastSQLCommand = "select userKey, name, country, insolvent, "
				...
				shared_ptr<sql::ResultSet> resultSet(preparedStatement->executeQuery());
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", eMailAddress: " + eMailAddress +
					", password: " + password + ", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (resultSet->next())
				{
...

	{
		...
		try
		{
			...
			{
				lastSQLCommand = "update MMS_User set lastSuccessfulLogin = NOW() "
								 "where userKey = ?";
...
Json::Value MMSEngineDBFacade::getWorkspaceList(int64_t userKey, bool admin, bool costDetails)
{
	...
	try
	{
		...
		{
			if (admin)
				lastSQLCommand = "select count(*) from MMS_Workspace w, MMS_APIKey a "
		...
		{
			if (admin)
				lastSQLCommand = "select w.workspaceKey, w.isEnabled, w.name, w.maxEncodingPriority, "
			...
			while (resultSet->next())
			{
				Json::Value workspaceDetailRoot = getWorkspaceDetailsRoot(conn, resultSet, userAPIKeyInfo); //, encoders);
...
Json::Value MMSEngineDBFacade::getLoginWorkspace(int64_t userKey, bool fromMaster)
{
	...
	try
	{
...

		{
			...
			if (resultSet->next())
			{
				loginWorkspaceRoot = getWorkspaceDetailsRoot(conn, resultSet, userAPIKeyInfo); //, encoders);
			}
			else
			{
				lastSQLCommand = "select w.workspaceKey, w.isEnabled, w.name, w.maxEncodingPriority, "
...
Json::Value MMSEngineDBFacade::getWorkspaceDetailsRoot(
	shared_ptr<MySQLConnection> conn, shared_ptr<sql::ResultSet> resultSet, bool userAPIKeyInfo
	// bool encoders
)
{
	...
	try
	{
		...
		if (userAPIKeyInfo)
		{
			...
			field = "default";
			userAPIKeyRoot[field] = resultSet->getInt("isDefault") == 1 ? "true" : "false";
			...
			{
				{
					string lastSQLCommand = "select u.userKey as workspaceOwnerUserKey, u.name as workspaceOwnerUserName "
											"from MMS_APIKey ak, MMS_User u "
											"where ak.userKey = u.userKey "
											"and ak.workspaceKey = ? and ak.isOwner = b'1'";
					shared_ptr<sql::PreparedStatement> preparedStatementOwnerWorkspace(conn->_sqlConnection->prepareStatement(lastSQLCommand));
					int queryParameterIndex = 1;
					preparedStatementOwnerWorkspace->setInt64(queryParameterIndex++, workspaceKey);
					chrono::system_clock::time_point startSql = chrono::system_clock::now();
					shared_ptr<sql::ResultSet> resultSetOwnerWorkspace(preparedStatementOwnerWorkspace->executeQuery());
					_logger->info(
						__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", workspaceKey: " + to_string(workspaceKey) +
						", resultSetOwnerWorkspace->rowsCount: " + to_string(resultSetOwnerWorkspace->rowsCount()) + ", elapsed (secs): @" +
						to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
					);
					if (resultSetOwnerWorkspace->next())
					{
						field = "workspaceOwnerUserKey";
						workspaceDetailRoot[field] = resultSetOwnerWorkspace->getInt64("workspaceOwnerUserKey");

						field = "workspaceOwnerUserName";
						workspaceDetailRoot[field] = static_cast<string>(resultSetOwnerWorkspace->getString("workspaceOwnerUserName"));
					}
				}
			}
		}
...
Json::Value MMSEngineDBFacade::updateWorkspaceDetails(
	...
	int64_t newMaxIngestionsNumber, bool maxStorageInMBChanged, int64_t newMaxStorageInMB, bool languageCodeChanged, string newLanguageCode,
	bool expirationDateChanged, string newExpirationDate, bool newCreateRemoveWorkspace, bool newIngestWorkflow, bool newCreateProfiles,
	bool newDeliveryAuthorization, bool newShareWorkspace, bool newEditMedia, bool newEditConfiguration, bool newKillEncoding,
...
)
{
	...
	try
	{
		...
		bool isOwner = false;
		{
			lastSQLCommand = "select isOwner, permissions from MMS_APIKey "
			...
			else
			{
				string errorMessage = __FILEREF__ + "user/workspace are not found" + ", workspaceKey: " + to_string(workspaceKey) +
									  ", userKey: " + to_string(userKey) + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
		...
		if (admin)
		{
			...
			if (oneParameterPresent)
			{
				lastSQLCommand = string("update MMS_Workspace ") + setSQL +
								 " "
								 "where workspaceKey = ?";
				shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
				int queryParameterIndex = 1;
				...
				int rowsUpdated = preparedStatement->executeUpdate();
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", newEnabled: " + to_string(newEnabled) +
					", newMaxEncodingPriority: " + newMaxEncodingPriority + ", newEncodingPeriod: " + newEncodingPeriod +
					", newMaxIngestionsNumber: " + to_string(newMaxIngestionsNumber) + ", newMaxStorageInMB: " + to_string(newMaxStorageInMB) +
					", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (rowsUpdated != 1)
				{
					string errorMessage =
						__FILEREF__ + "no update was done" + ", workspaceKey: " + to_string(workspaceKey) + ", newEnabled: " + to_string(newEnabled) +
						", newMaxEncodingPriority: " + newMaxEncodingPriority + ", newEncodingPeriod: " + newEncodingPeriod +
						", newMaxIngestionsNumber: " + to_string(newMaxIngestionsNumber) + ", newMaxStorageInMB: " + to_string(newMaxStorageInMB) +
						", rowsUpdated: " + to_string(rowsUpdated) + ", lastSQLCommand: " + lastSQLCommand;
					_logger->warn(errorMessage);
			...
			if (expirationDateChanged)
			{
				...
				lastSQLCommand = "update MMS_APIKey "
				...
				int rowsUpdated = preparedStatement->executeUpdate();
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", newExpirationDate: " + newExpirationDate +
					", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) + ", elapsed (secs): @" +
					to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (rowsUpdated == 0)
				{
					string errorMessage = __FILEREF__ + "no update was done" + ", newExpirationDate: " + newExpirationDate +
										  ", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) +
										  ", lastSQLCommand: " + lastSQLCommand;
					_logger->warn(errorMessage);
...

		{
			...
			if (oneParameterPresent)
			{
				...
				int rowsUpdated = preparedStatement->executeUpdate();
				_logger->info(
					__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", newName: " + newName + ", newLanguageCode: " +
					newLanguageCode + ", workspaceKey: " + to_string(workspaceKey) + ", rowsUpdated: " + to_string(rowsUpdated) +
					", elapsed (secs): @" + to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
				);
				if (rowsUpdated != 1)
				{
...

		{
			...
			lastSQLCommand = "update MMS_APIKey set permissions = ? "
							 "where workspaceKey = ? and userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setString(queryParameterIndex++, permissions);
			...
			int rowsUpdated = preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", permissions: " + permissions +
				", workspaceKey: " + to_string(workspaceKey) + ", userKey: " + to_string(userKey) + ", rowsUpdated: " + to_string(rowsUpdated) +
				", elapsed (secs): @" + to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (rowsUpdated != 1)
			{
...
Json::Value MMSEngineDBFacade::setWorkspaceAsDefault(int64_t userKey, int64_t workspaceKey, int64_t workspaceKeyToBeSetAsDefault)
{
	Json::Value workspaceDetailRoot;
	string lastSQLCommand;
	...
	try
	{
		...
		string apiKey;
		{
			lastSQLCommand = "select apiKey from MMS_APIKey where workspaceKey = ? and userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			_logger->info(
				...
				", workspaceKeyToBeSetAsDefault: " + to_string(workspaceKeyToBeSetAsDefault) + ", userKey: " + to_string(userKey) +
				", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (resultSet->next())
		...
		{
			lastSQLCommand = "update MMS_APIKey set isDefault = 0 "
							 "where userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, userKey);
		...
		{
			lastSQLCommand = "update MMS_APIKey set isDefault = 1 "
							 "where apiKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setString(queryParameterIndex++, apiKey);
...
pair<int64_t, int64_t> MMSEngineDBFacade::getWorkspaceUsage(shared_ptr<MySQLConnection> conn, int64_t workspaceKey)
{
	...
	{
		{
			lastSQLCommand = "select SUM(pp.sizeInBytes) as totalSizeInBytes from MMS_MediaItem mi, MMS_PhysicalPath pp "
		...
		{
			lastSQLCommand = "select maxStorageInMB from MMS_Workspace where workspaceKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setInt64(queryParameterIndex++, workspaceKey);
...
pair<int64_t, string> MMSEngineDBFacade::getUserDetailsByEmail(string email)
{
	...
	try
	{
		...
		{
			lastSQLCommand = "select userKey, name from MMS_User where eMailAddress = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			shared_ptr<sql::ResultSet> resultSet(preparedStatement->executeQuery());
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", emailserKey: " + email +
				", resultSet->rowsCount: " + to_string(resultSet->rowsCount()) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (resultSet->next())
			...
			{
				string errorMessage = __FILEREF__ + "User is not present" + ", email: " + email + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
	...
	try
	{
...

		{
			...
			if (passwordChanged)
			{
				...
				{
					lastSQLCommand = "select password from MMS_User where userKey = ?";
					shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
					int queryParameterIndex = 1;
					preparedStatement->setInt64(queryParameterIndex++, userKey);
					...
					{
						string errorMessage =
							__FILEREF__ + "User is not present" + ", userKey: " + to_string(userKey) + ", lastSQLCommand: " + lastSQLCommand;
						_logger->error(errorMessage);
			...
			lastSQLCommand = string("update MMS_User ") + setSQL +
							 " "
							 "where userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", newPassword: " + newPassword + ", name: " + name +
				", email: " + email + ", country: " + country + ", expirationDate: " + expirationDate + ", userKey: " + to_string(userKey) +
				", rowsUpdated: " + to_string(rowsUpdated) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			if (rowsUpdated != 1)
		...
		{
			lastSQLCommand = "select DATE_FORMAT(convert_tz(creationDate, @@session.time_zone, '+00:00'), '%Y-%m-%dT%H:%i:%sZ') as creationDate, "
							 "DATE_FORMAT(convert_tz(expirationDate, @@session.time_zone, '+00:00'), '%Y-%m-%dT%H:%i:%sZ') as expirationDate, "
			...
			if (resultSet->next())
			{
				...
				field = "email";
				loginDetailsRoot[field] = static_cast<string>(resultSet->getString("eMailAddress"));
			...
			{
				string errorMessage = __FILEREF__ + "userKey is wrong" + ", userKey: " + to_string(userKey) + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
...
string MMSEngineDBFacade::createResetPasswordToken(int64_t userKey)
{
	...
	try
	{
		...
		{
			lastSQLCommand = "insert into MMS_ResetPasswordToken (token, userKey, creationDate) "
							 "values (?, ?, NOW())";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			...
			preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", resetPasswordToken: " + resetPasswordToken +
				", userKey: " + to_string(userKey) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
		}
...
pair<string, string> MMSEngineDBFacade::resetPassword(string resetPasswordToken, string newPassword)
{
	...
	try
	{
		...
		int userKey;
		{
			lastSQLCommand = "select u.name, u.eMailAddress, u.userKey "
			...
			else
			{
				string errorMessage = __FILEREF__ + "reset password token is not present or is expired" +
									  ", resetPasswordToken: " + resetPasswordToken + ", lastSQLCommand: " + lastSQLCommand;
				_logger->error(errorMessage);
		...
		{
			lastSQLCommand = "update MMS_User set password = ? where userKey = ?";
			shared_ptr<sql::PreparedStatement> preparedStatement(conn->_sqlConnection->prepareStatement(lastSQLCommand));
			int queryParameterIndex = 1;
			preparedStatement->setString(queryParameterIndex++, newPassword);
			preparedStatement->setInt64(queryParameterIndex++, userKey);
			...
			int rowsUpdated = preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", newPassword: " + newPassword +
				", userKey: " + to_string(userKey) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
			/*
...
int64_t MMSEngineDBFacade::saveLoginStatistics(
	int userKey, string ip, string continent, string continentCode, string country, string countryCode, string region, string city, string org,
	string isp, int timezoneGMTOffset
)
{
	...
	try
	{
		...
		{
			lastSQLCommand = "insert into MMS_LoginStatistic (userKey, ip, continent, continentCode, "
			...
			preparedStatement->executeUpdate();
			_logger->info(
				__FILEREF__ + "@SQL statistics@" + ", lastSQLCommand: " + lastSQLCommand + ", userKey: " + to_string(userKey) + ", ip: " + ip +
				", continent: " + continent + ", continentCode: " + continentCode + ", country: " + country + ", countryCode: " + countryCode +
				", region: " + region + ", city: " + city + ", org: " + org + ", isp: " + isp +
				", timezoneGMTOffset: " + to_string(timezoneGMTOffset) + ", elapsed (secs): @" +
				to_string(chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - startSql).count()) + "@"
			);
