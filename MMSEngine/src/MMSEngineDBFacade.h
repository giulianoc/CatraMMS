...
class MMSEngineDBFacade
{
  ...
  public:
...
#ifdef __POSTGRES__
	json getWorkspaceCost(PostgresConnTrans &trans, int64_t workspaceKey);
#else
#endif
	...
	int64_t addEncodingProfile(
		PostgresConnTrans &trans, int64_t workspaceKey, string label, MMSEngineDBFacade::ContentType contentType,
...
#ifdef __POSTGRES__
	void updateIngestionJobMetadataContent(PostgresConnTrans &trans, int64_t ingestionJobKey, string metadataContent);
#else
	...
	*/
...
#ifdef __POSTGRES__
	void endWorkflow(PostgresConnTrans &trans, bool commit, int64_t ingestionRootKey, string processedMetadataContent);
#else
	...
	string ingestionRoot_columnAsString(int64_t workspaceKey, string columnName, int64_t ingestionRootKey, bool fromMaster);
	...
	shared_ptr<PostgresHelper::SqlResultSet> workflowQuery(
		vector<string> &requestedColumns, int64_t workspaceKey, int64_t ingestionRootKey, bool fromMaster, int startIndex = -1, int rows = -1,
	...
	string ingestionJob_columnAsString(int64_t workspaceKey, string columnName, int64_t ingestionJobKey, bool fromMaster);
	...
	json ingestionJob_columnAsJson(int64_t workspaceKey, string columnName, int64_t ingestionJobKey, bool fromMaster);
	...
	string externalUniqueName_columnAsString(
	...
	shared_ptr<PostgresHelper::SqlResultSet> externalUniqueNameQuery(
		vector<string> &requestedColumns, int64_t workspaceKey, string uniqueName, int64_t mediaItemKey, bool fromMaster, int startIndex = -1,
		int rows = -1, string orderBy = "", bool notFoundAsException = true
	);
	...
	void recoverEncodingsNotCompleted(string processorMMS, vector<shared_ptr<MMSEngineDBFacade::EncodingItem>> &encodingItems);
	...
	void updateOutputURL(int64_t ingestionJobKey, int64_t encodingJobKey, int outputIndex, bool srtFeed, string outputURL);
	...
	json encodingJob_columnAsJson(string columnName, int64_t encodingJobKey, bool fromMaster);
	...
	shared_ptr<PostgresHelper::SqlResultSet> encodingJobQuery(
		vector<string> &requestedColumns, int64_t encodingJobKey, int64_t ingestionJobKey, bool fromMaster, int startIndex = -1, int rows = -1,
		string orderBy = "", bool notFoundAsException = true
	);
	...
	*/
	...
	string stream_columnAsString(int64_t workspaceKey, string columnName, int64_t confKey = -1, string label = "");
	int64_t stream_columnAsInt64(int64_t workspaceKey, string columnName, int64_t confKey = -1, string label = "");
	json stream_columnAsJson(int64_t workspaceKey, string columnName, int64_t confKey = -1, string label = "");
	...
	shared_ptr<PostgresHelper::SqlResultSet> streamQuery(
		vector<string> &requestedColumns, int64_t workspaceKey, int64_t confKey = -1, string label = "", bool fromMaster = false, int startIndex = -1,
		int rows = -1, string orderBy = "", bool notFoundAsException = true
	);
	...
	shared_ptr<DBConnectionPool<PostgresConnection>> masterPostgresConnectionPool() { return _masterPostgresConnectionPool; }
  ...
  private:
	...
	void updateIngestionJob(
		PostgresConnTrans &trans, int64_t ingestionJobKey, IngestionStatus newIngestionStatus, string errorMessage,
...
#ifdef __POSTGRES__
	void
	manageExternalUniqueName(PostgresConnTrans &trans, int64_t workspaceKey, int64_t mediaItemKey, bool allowUniqueNameOverride, string uniqueName);
#else
...
#ifdef __POSTGRES__
	void
	manageCrossReferences(PostgresConnTrans &trans, int64_t ingestionJobKey, int64_t workspaceKey, int64_t mediaItemKey, json crossReferencesRoot);
#endif
...
#ifdef __POSTGRES__
	pair<int64_t, int64_t> getWorkspaceUsage(const PostgresConnTrans &trans, int64_t workspaceKey);
#else
	pair<int64_t, int64_t> getWorkspaceUsage(shared_ptr<MySQLConnection> conn, int64_t workspaceKey);
#endif

#ifdef __POSTGRES__
	json getWorkspaceDetailsRoot(PostgresConnTrans &trans, row &row, bool userAPIKeyInfo, bool costDetails);
#else
