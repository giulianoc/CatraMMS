...
#else
#define __FILEREF__                                                            \
	string("[") + basename((char *)__FILE__) + ":" + to_string(__LINE__) + "] "
#endif
#endif
...
struct AlreadyLocked : public exception
{
	...
	{
		_errorMessage = string("Already locked") + ", label: " + label +
						", owner: " + owner + ", currentLockDuration (secs): " +
						to_string(currentLockDuration);
	}
...
class MMSEngineDBFacade
{
  ...
  public:
	...
	static CodeType toCodeType(const string &codeType)
	{
		...
		transform(
			codeType.begin(), codeType.end(), lowerCase.begin(),
		...
		if (lowerCase == "userregistration")
			...
			throw runtime_error(
				string("Wrong CodeType") + ", current codeType: " + codeType
			);
	}
	...
	static LockType toLockType(const string &lockType)
	{
		...
		transform(
			lockType.begin(), lockType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "ingestion")
			...
			throw runtime_error(
				string("Wrong LockType") + ", current lockType: " + lockType
			);
	}
	...
	static OncePerDayType toOncePerDayType(const string &oncePerDayType)
	{
		...
		transform(
			oncePerDayType.begin(), oncePerDayType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static ContentType toContentType(const string &contentType)
	{
		...
		transform(
			contentType.begin(), contentType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "video")
			...
			throw runtime_error(
				string("Wrong ContentType (correct value: video, audio or image"
				) +
	...
	static EncodingPriority toEncodingPriority(const string &priority)
	{
		...
		transform(
			priority.begin(), priority.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "low")
			...
			throw runtime_error(
				string("Wrong EncodingPriority") + ", priority: " + priority
			);
	}
	...
	static const char *toString(const EncodingStatus &encodingStatus)
	{
		...
		default:
			throw runtime_error(
				string("Wrong EncodingStatus: ") +
				to_string(static_cast<int>(encodingStatus))
			);
		}
	}
	static EncodingStatus toEncodingStatus(const string &encodingStatus)
	{
		...
		transform(
			encodingStatus.begin(), encodingStatus.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static EncodingType toEncodingType(const string &encodingType)
	{
		...
		transform(
			encodingType.begin(), encodingType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "encodeimage")
			...
			throw runtime_error(
				string("Wrong EncodingType") + ", encodingType: " + encodingType
			);
	}
	...
	static EncodingError toEncodingError(const string &encodingError)
	{
		...
		transform(
			encodingError.begin(), encodingError.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static DeliveryTechnology
	toDeliveryTechnology(const string &deliveryTechnology)
	{
		string lowerCase;
		lowerCase.resize(deliveryTechnology.size());
		transform(
			deliveryTechnology.begin(), deliveryTechnology.end(),
		...
		if (lowerCase == "download")
			...
			throw runtime_error(
				string("Wrong DeliveryTechnology") +
	...
	static EncodingPeriod toEncodingPeriod(const string &encodingPeriod)
	{
		...
		transform(
			encodingPeriod.begin(), encodingPeriod.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static VideoSpeedType toVideoSpeedType(const string &videoSpeedType)
	{
		...
		transform(
			videoSpeedType.begin(), videoSpeedType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static IngestionType toIngestionType(const string &ingestionType)
	{
		...
		transform(
			ingestionType.begin(), ingestionType.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static const char *toString(const IngestionStatus &ingestionStatus)
	{
		...
		default:
			throw runtime_error(
				string("Wrong IngestionStatus: ") +
				to_string(static_cast<int>(ingestionStatus))
			);
		}
	}
	static IngestionStatus toIngestionStatus(const string &ingestionStatus)
	{
		...
		transform(
			ingestionStatus.begin(), ingestionStatus.end(), lowerCase.begin(),
			[](unsigned char c) { return tolower(c); }
		);
	...
	static bool
	isIngestionStatusFinalState(const IngestionStatus &ingestionStatus)
	{
		string prefix = "End";
		string sIngestionStatus = MMSEngineDBFacade::toString(ingestionStatus);

		return (
			sIngestionStatus.size() >= prefix.size() &&
			0 == sIngestionStatus.compare(0, prefix.size(), prefix)
		);
	}
	static bool isIngestionStatusSuccess(const IngestionStatus &ingestionStatus)
	{
		return (
			...
			ingestionStatus ==
				IngestionStatus::End_NotToBeExecuted_ChunkNotSelected
		);
	}
	static bool isIngestionStatusFailed(const IngestionStatus &ingestionStatus)
	{
		return (
			isIngestionStatusFinalState(ingestionStatus) &&
			!isIngestionStatusSuccess(ingestionStatus)
		);
	}
	...
	static IngestionRootStatus
	toIngestionRootStatus(const string &ingestionRootStatus)
	{
		string lowerCase;
		lowerCase.resize(ingestionRootStatus.size());
		transform(
			ingestionRootStatus.begin(), ingestionRootStatus.end(),
			lowerCase.begin(), [](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "notcompleted")
			...
			throw runtime_error(
				string("Wrong IngestionRootStatus") +
	...
	static CrossReferenceType
	toCrossReferenceType(const string &crossReferenceType)
	{
		string lowerCase;
		lowerCase.resize(crossReferenceType.size());
		transform(
			crossReferenceType.begin(), crossReferenceType.end(),
			lowerCase.begin(), [](unsigned char c) { return tolower(c); }
		);
		...
		if (lowerCase == "videoofimage")
			...
			throw runtime_error(
				string("Wrong CrossReferenceType") +
  ...
  public:
	MMSEngineDBFacade(
		json configuration, size_t masterDbPoolSize, size_t slaveDbPoolSize,
	...
	int64_t addInvoice(
		int64_t userKey, string description, int amount, string expirationDate
	);

	json getInvoicesList(int64_t userKey, bool admin, int start, int rows);

	string createCode(
		int64_t workspaceKey, int64_t userKey, string userEmail,
		CodeType codeType, bool admin, bool createRemoveWorkspace,
		bool ingestWorkflow, bool createProfiles, bool deliveryAuthorization,
		bool shareWorkspace, bool editMedia, bool editConfiguration,
		bool killEncoding, bool cancelIngestionJob, bool editEncodersPool,
		bool applicationRecorder
	);

#ifdef __POSTGRES__
	tuple<int64_t, int64_t, string> registerUserAndAddWorkspace(
		string userName, string userEmailAddress, string userPassword,
		string userCountry, string userTimezone, string workspaceName,
		WorkspaceType workspaceType, string deliveryURL,
		EncodingPriority maxEncodingPriority, EncodingPeriod encodingPeriod,
		long maxIngestionsNumber, long maxStorageInMB, string languageCode,
		chrono::system_clock::time_point userExpirationDate
	);
#else
	tuple<int64_t, int64_t, string> registerUserAndAddWorkspace(
		string userName, string userEmailAddress, string userPassword,
		string userCountry, string workspaceName, WorkspaceType workspaceType,
		string deliveryURL, EncodingPriority maxEncodingPriority,
		EncodingPeriod encodingPeriod, long maxIngestionsNumber,
		long maxStorageInMB, string languageCode,
		chrono::system_clock::time_point userExpirationDate
	);
#endif
	...
	tuple<int64_t, int64_t, string> registerUserAndShareWorkspace(
		string userName, string userEmailAddress, string userPassword,
		string userCountry, string userTimezone, string shareWorkspaceCode,
		chrono::system_clock::time_point userExpirationDate
	);
#else
	tuple<int64_t, int64_t, string> registerUserAndShareWorkspace(
		string userName, string userEmailAddress, string userPassword,
		string userCountry, string shareWorkspaceCode,
		chrono::system_clock::time_point userExpirationDate
	);
#endif

	pair<int64_t, string> createWorkspace(
		int64_t userKey, string workspaceName, WorkspaceType workspaceType,
		string deliveryURL, EncodingPriority maxEncodingPriority,
		EncodingPeriod encodingPeriod, long maxIngestionsNumber,
		long maxStorageInMB, string languageCode, bool admin,
		chrono::system_clock::time_point userExpirationDate
	);

	vector<tuple<int64_t, string, string>>
	deleteWorkspace(int64_t userKey, int64_t workspaceKey);
#ifdef __POSTGRES__
	tuple<bool, string, string>
	unshareWorkspace(int64_t userKey, int64_t workspaceKey);
#endif
	...
	pair<int64_t, string> registerActiveDirectoryUser(
		string userName, string userEmailAddress, string userCountry,
		string userTimezone, bool createRemoveWorkspace, bool ingestWorkflow,
		bool createProfiles, bool deliveryAuthorization, bool shareWorkspace,
		bool editMedia, bool editConfiguration, bool killEncoding,
		bool cancelIngestionJob, bool editEncodersPool,
		bool applicationRecorder, string defaultWorkspaceKeys,
	...
	pair<int64_t, string> registerActiveDirectoryUser(
		string userName, string userEmailAddress, string userCountry,
		bool createRemoveWorkspace, bool ingestWorkflow, bool createProfiles,
		bool deliveryAuthorization, bool shareWorkspace, bool editMedia,
		bool editConfiguration, bool killEncoding, bool cancelIngestionJob,
		bool editEncodersPool, bool applicationRecorder,
	...
	string createAPIKeyForActiveDirectoryUser(
		int64_t userKey, string userEmailAddress, bool createRemoveWorkspace,
		bool ingestWorkflow, bool createProfiles, bool deliveryAuthorization,
		bool shareWorkspace, bool editMedia, bool editConfiguration,
		bool killEncoding, bool cancelIngestionJob, bool editEncodersPool,
		bool applicationRecorder, int64_t workspaceKey,
		int expirationInDaysWorkspaceDefaultValue
	);
...
#ifdef __POSTGRES__
	pair<int64_t, string>
	getUserDetailsByEmail(string email, bool warningIfError);
#else
...
#ifdef __POSTGRES__
	void saveGEOInfo(
		string ipAddress, transaction_base *trans,
	...
	json updateWorkspaceDetails(
		int64_t userKey, int64_t workspaceKey, bool enabledChanged,
		bool newEnabled, bool nameChanged, string newName,
		...
		bool currentCostForDedicatedEncoder_power_1Changed,
		int64_t currentCostForDedicatedEncoder_power_1,
		bool dedicatedEncoder_power_2Changed, int64_t dedicatedEncoder_power_2,
		bool currentCostForDedicatedEncoder_power_2Changed,
		int64_t currentCostForDedicatedEncoder_power_2,
		bool dedicatedEncoder_power_3Changed, int64_t dedicatedEncoder_power_3,
		bool currentCostForDedicatedEncoder_power_3Changed,
		int64_t currentCostForDedicatedEncoder_power_3, bool CDN_type_1Changed,
		int64_t CDN_type_1, bool currentCostForCDN_type_1Changed,
		int64_t currentCostForCDN_type_1, bool support_type_1Changed,
		bool support_type_1, bool currentCostForSupport_type_1Changed,
		int64_t currentCostForSupport_type_1,

		bool newCreateRemoveWorkspace, bool newIngestWorkflow,
		bool newCreateProfiles, bool newDeliveryAuthorization,
		bool newShareWorkspace, bool newEditMedia, bool newEditConfiguration,
		bool newKillEncoding, bool newCancelIngestionJob,
...
#ifdef __POSTGRES__
	json updateUser(
		bool admin, bool ldapEnabled, int64_t userKey, bool nameChanged,
		string name, bool emailChanged, string email, bool countryChanged,
		string country, bool timezoneChanged, string timezone,
		bool insolventChanged, bool insolvent, bool expirationDateChanged,
		string expirationDate, bool passwordChanged, string newPassword,
		string oldPassword
	);
#else
	json updateUser(
		bool admin, bool ldapEnabled, int64_t userKey, bool nameChanged,
		string name, bool emailChanged, string email, bool countryChanged,
		string country, bool insolventChanged, bool insolvent,
		bool expirationDateChanged, string expirationDate, bool passwordChanged,
	...
	pair<string, string>
	resetPassword(string resetPassworkToken, string newPassword);

	void addIngestionJobOutput(
		int64_t ingestionJobKey, int64_t mediaItemKey, int64_t physicalPathKey,
	...
	int64_t addEncodingProfilesSetIfNotAlreadyPresent(
		...
		int64_t workspaceKey, MMSEngineDBFacade::ContentType contentType,
		string label, bool removeEncodingProfilesIfPresent
	);
#else
	int64_t addEncodingProfilesSetIfNotAlreadyPresent(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
		MMSEngineDBFacade::ContentType contentType, string label,
...
#else
	int64_t addEncodingProfile(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey, string label,
		MMSEngineDBFacade::ContentType contentType,
		DeliveryTechnology deliveryTechnology, string jsonProfile,
	...
	int64_t addEncodingProfile(
		int64_t workspaceKey, string label,
		MMSEngineDBFacade::ContentType contentType,
		DeliveryTechnology deliveryTechnology, string jsonEncodingProfile
	);

	void
	removeEncodingProfile(int64_t workspaceKey, int64_t encodingProfileKey);
	...
	void getIngestionsToBeManaged(
		vector<tuple<
			...
			IngestionType, IngestionStatus>> &ingestionsToBeManaged,
		string processorMMS, int maxIngestionJobs,
	...
	int64_t addIngestionRoot(
		...
		int64_t userKey, string rootType, string rootLabel,
		string metaDataContent
	);
#else
	int64_t addIngestionRoot(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey, int64_t userKey,
		string rootType, string rootLabel, string metaDataContent
	);
#endif
	...
	void addIngestionJobDependency(
		...
		int64_t ingestionJobKey, int dependOnSuccess,
		int64_t dependOnIngestionJobKey, int orderNumber,
		bool referenceOutputDependency
	);
#else
	void addIngestionJobDependency(
		shared_ptr<MySQLConnection> conn, int64_t ingestionJobKey,
		int dependOnSuccess, int64_t dependOnIngestionJobKey, int orderNumber,
		bool referenceOutputDependency
	);
#endif
	...
	int64_t addIngestionJob(
		...
		string processingStartingFrom, vector<int64_t> dependOnIngestionJobKeys,
		int dependOnSuccess, vector<int64_t> waitForGlobalIngestionJobKeys
	);
#else
	int64_t addIngestionJob(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
	...
	void getIngestionJobsKeyByGlobalLabel(
		int64_t workspaceKey, string globalIngestionLabel, bool fromMaster,
		vector<int64_t> &ingestionJobsKey
	);

	void updateIngestionJobMetadataContent(
		int64_t ingestionJobKey, string metadataContent
	);
	...
	void updateIngestionJobParentGroupOfTasks(
		shared_ptr<PostgresConnection> conn, work &trans,
		int64_t ingestionJobKey, int64_t parentGroupOfTasksIngestionJobKey
	);
#else
	void updateIngestionJobParentGroupOfTasks(
		shared_ptr<MySQLConnection> conn, int64_t ingestionJobKey,
	...
	*/
	...
	shared_ptr<MySQLConnection> endIngestionJobs(
		shared_ptr<MySQLConnection> conn, bool commit, int64_t ingestionRootKey,
		string processedMetadataContent
	);
#endif

	void updateIngestionJob(
		int64_t ingestionJobKey, IngestionStatus newIngestionStatus,
		string errorMessage, string processorMMS = "noToBeUpdated"
	);

	void appendIngestionJobErrorMessage(
		int64_t ingestionJobKey, string errorMessage
	);

	bool updateIngestionJobSourceDownloadingInProgress(
		int64_t ingestionJobKey, double downloadingPercentage
	);

	bool updateIngestionJobSourceUploadingInProgress(
		int64_t ingestionJobKey, double uploadingPercentage
	);

	void updateIngestionJobSourceBinaryTransferred(
		int64_t ingestionJobKey, bool sourceBinaryTransferred
	);
	...
	json getIngestionRootsStatus(
		shared_ptr<Workspace> workspace, int64_t ingestionRootKey,
		int64_t mediaItemKey, int start, int rows,
		// bool startAndEndIngestionDatePresent,
		string startIngestionDate, string endIngestionDate, string label,
		string status, bool asc, bool dependencyInfo, bool ingestionJobOutputs,
		bool fromMaster
	);

	json getIngestionJobsStatus(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey, int start,
		int rows, string label, bool labelLike,
		/* bool startAndEndIngestionDatePresent, */ string startIngestionDate,
		string endIngestionDate, string startScheduleDate, string ingestionType,
		string configurationLabel, string outputChannelLabel,
		int64_t recordingCode, bool broadcastIngestionJobKeyNotNull,
		string jsonParametersCondition, bool asc, string status,
		bool dependencyInfo, bool ingestionJobOutputs, bool fromMaster
	);

	json getEncodingJobsStatus(
		shared_ptr<Workspace> workspace, int64_t encodingJobKey, int start,
		int rows,
		// bool startAndEndIngestionDatePresent,
		...
		bool alsoEncodingJobsFromOtherWorkspaces, bool asc, string status,
		string types, bool fromMaster
	);

#ifdef __POSTGRES__
	json updateMediaItem(
		int64_t workspaceKey, int64_t mediaItemKey, bool titleModified,
		string newTitle, bool userDataModified, string newUserData,
		bool retentionInMinutesModified, int64_t newRetentionInMinutes,
		bool tagsModified, json tagsRoot, bool uniqueNameModified,
	...
	json updatePhysicalPath(
		int64_t workspaceKey, int64_t mediaItemKey, int64_t physicalPathKey,
		int64_t newRetentionInMinutes, bool admin
	);

	json getMediaItemsList(
		int64_t workspaceKey, int64_t mediaItemKey, string uniqueName,
		int64_t physicalPathKey, vector<int64_t> &otherMediaItemsKey, int start,
		int rows, bool contentTypePresent, ContentType contentType,
		// bool startAndEndIngestionDatePresent,
		string startIngestionDate, string endIngestionDate, string title,
		int liveRecordingChunk, int64_t recordingCode,
		int64_t utcCutPeriodStartTimeInMilliSeconds,
		int64_t utcCutPeriodEndTimeInMilliSecondsPlusOneSecond,
		string jsonCondition, vector<string> &tagsIn, vector<string> &tagsNotIn,
		string orderBy, string jsonOrderBy, json responseFields, bool admin,
		bool fromMaster
	);

	json getTagsList(
		int64_t workspaceKey, int start, int rows, int liveRecordingChunk,
		bool contentTypePresent, ContentType contentType, string tagNameFilter,
		bool fromMaster
	);
	...
	int64_t addUpdateWorkflowAsLibrary(
		int64_t userKey, int64_t workspaceKey, string label,
	...
	json getEncodingProfilesSetList(
		int64_t workspaceKey, int64_t encodingProfilesSetKey,
		bool contentTypePresent, ContentType contentType
	);

	json getEncodingProfileList(
		int64_t workspaceKey, int64_t encodingProfileKey,
		bool contentTypePresent, ContentType contentType, string label
	);
	...
	int64_t getPhysicalPathDetails(
		int64_t workspaceKey, int64_t mediaItemKey, ContentType contentType,
		string encodingProfileLabel, bool warningIfMissing, bool fromMaster
	);

	string getPhysicalPathDetails(
		int64_t physicalPathKey, bool warningIfMissing, bool fromMaster
	);

	tuple<int64_t, int, string, string, uint64_t, bool, int64_t>
	getSourcePhysicalPath(
	...
	getMediaItemKeyDetailsByUniqueName(
	...
	void getVideoDetails(
		...
		vector<tuple<
			...
			&videoTracks,
		vector<tuple<int64_t, int, int64_t, long, string, long, int, string>>
			&audioTracks
	);
	...
	vector<int64_t>
	getEncodingProfileKeysBySetLabel(int64_t workspaceKey, string label);

	tuple<
		string, MMSEngineDBFacade::ContentType,
		MMSEngineDBFacade::DeliveryTechnology, string>
	getEncodingProfileDetailsByKey(
		int64_t workspaceKey, int64_t encodingProfileKey
	);

	tuple<
		int64_t, MMSEngineDBFacade::DeliveryTechnology, int,
		shared_ptr<Workspace>, string, string, string, string, uint64_t, bool>
	...
	tuple<
		...
		shared_ptr<Workspace>, string, string, string, string, uint64_t, bool>
	getStorageDetails(
		int64_t mediaItemKey, int64_t encodingProfileKey, bool warningIfMissing,
		bool fromMaster
	);
	...
	void getAllStorageDetails(
		...
		vector<tuple<
			MMSEngineDBFacade::DeliveryTechnology, int, string, string, string,
			int64_t, bool>> &allStorageDetails
	);

	int64_t createDeliveryAuthorization(
		int64_t userKey, string clientIPAddress, int64_t physicalPathKey,
		int64_t liveDeliveryKey, string deliveryURI, int ttlInSeconds,
		int maxRetries
	);

	bool checkDeliveryAuthorization(
		int64_t deliveryAuthorizationKey, string contentURI
	);
	...
	void getEncodingJobs(
		...
		int timeBeforeToPrepareResourcesInMinutes, int maxEncodingsNumber
	);

	int64_t getEncodingProfileKeyByLabel(
		int64_t workspaceKey, MMSEngineDBFacade::ContentType contentType,
		string encodingProfileLabel, bool contentTypeToBeUsed = true
	);

	void addEncodingJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		MMSEngineDBFacade::ContentType contentType,
		EncodingPriority encodingPriority, int64_t encodingProfileKey,
	...
	void addEncoding_OverlayImageOnVideoJob(
		...
		int64_t sourceVideoMediaItemKey, int64_t sourceVideoPhysicalPathKey,
		int64_t videoDurationInMilliSeconds, string mmsSourceVideoAssetPathName,
		string sourceVideoPhysicalDeliveryURL, string sourceVideoFileExtension,
		...
		EncodingPriority encodingPriority, string mmsWorkflowIngestionURL,
	...
	void addEncoding_OverlayTextOnVideoJob(
		...
		string sourceAssetPathName, int64_t sourceDurationInMilliSeconds,
		string sourcePhysicalDeliveryURL, string sourceFileExtension,

		string sourceTranscoderStagingAssetPathName,
		string encodedTranscoderStagingAssetPathName,
		string encodedNFSStagingAssetPathName, string mmsWorkflowIngestionURL,
		string mmsBinaryIngestionURL, string mmsIngestionURL
	);

	void addEncoding_GenerateFramesJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		EncodingPriority encodingPriority, string nfsImagesDirectory,
		string transcoderStagingImagesDirectory,
		string sourcePhysicalDeliveryURL,
		string sourceTranscoderStagingAssetPathName, string sourceAssetPathName,
		int64_t sourceVideoPhysicalPathKey, string sourceFileExtension,
		string sourceFileName, int64_t videoDurationInMilliSeconds,
		double startTimeInSeconds, int maxFramesNumber, string videoFilter,
		int periodInSeconds, bool mjpeg, int imageWidth, int imageHeight,
		string mmsWorkflowIngestionURL, string mmsBinaryIngestionURL,
		string mmsIngestionURL
	);
	...
	void addEncoding_SlideShowJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		int64_t encodingProfileKey, json encodingProfileDetailsRoot,
		string targetFileFormat, json imagesRoot, json audiosRoot,
		float shortestAudioDurationInSeconds,
		string encodedTranscoderStagingAssetPathName,
		string encodedNFSStagingAssetPathName, string mmsWorkflowIngestionURL,
		string mmsBinaryIngestionURL, string mmsIngestionURL,
		EncodingPriority encodingPriority
	...
	void addEncoding_FaceRecognitionJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		int64_t sourceMediaItemKey, int64_t sourceVideoPhysicalPathKey,
		string sourcePhysicalPath, string faceRecognitionCascadeName,
		string faceRecognitionOutput, EncodingPriority encodingPriority,
		long initialFramesNumberToBeSkipped, bool oneFramePerSecond
	);

	void addEncoding_FaceIdentificationJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		string sourcePhysicalPath, string faceIdentificationCascadeName,
	...
	void addEncoding_LiveRecorderJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		string ingestionJobLabel, string streamSourceType,
		// bool highAvailability,
		string configurationLabel, int64_t confKey, string url,
		string encodersPoolLabel, EncodingPriority encodingPriority,

		int pushListenTimeout, int64_t pushEncoderKey, string pushServerName,
		json captureRoot, json tvRoot,

		bool monitorHLS, bool liveRecorderVirtualVOD,
		...
		string recordedFileNamePrefix, string virtualVODStagingContentsPath,
	...
	void addEncoding_LiveProxyJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		json inputsRoot, string streamSourceType,
		int64_t utcProxyPeriodStart, // int64_t utcProxyPeriodEnd,
		// long maxAttemptsNumberInCaseOfErrors,
		long waitingSecondsBetweenAttemptsInCaseOfErrors, json outputsRoot,
		string mmsWorkflowIngestionURL
	);

	void addEncoding_VODProxyJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		json inputsRoot, int64_t utcProxyPeriodStart, json outputsRoot,
		long maxAttemptsNumberInCaseOfErrors,
	...
	void addEncoding_LiveGridJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		json inputChannelsRoot, json outputsRoot
	);
	...
	void addEncoding_VideoSpeed(
		...
		string sourceAssetPathName, int64_t sourceDurationInMilliSeconds,
		string sourceFileExtension, string sourcePhysicalDeliveryURL,
		string sourceTranscoderStagingAssetPathName, int64_t encodingProfileKey,
		json encodingProfileDetailsRoot,
		string encodedTranscoderStagingAssetPathName,
		string encodedNFSStagingAssetPathName, string mmsWorkflowIngestionURL,
		string mmsBinaryIngestionURL, string mmsIngestionURL,
	...
	void addEncoding_AddSilentAudio(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		json sourcesRoot, int64_t encodingProfileKey,
		json encodingProfileDetailsRoot, string mmsWorkflowIngestionURL,
		string mmsBinaryIngestionURL, string mmsIngestionURL,
		EncodingPriority encodingPriority
	);
	...
	void addEncoding_PictureInPictureJob(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		int64_t mainSourceMediaItemKey, int64_t mainSourcePhysicalPathKey,
		string mainSourceAssetPathName,
		int64_t mainSourceDurationInMilliSeconds,
		string mainSourceFileExtension, string mainSourcePhysicalDeliveryURL,
		string mainSourceTranscoderStagingAssetPathName,
		int64_t overlaySourceMediaItemKey, int64_t overlaySourcePhysicalPathKey,
		...
		string overlaySourceTranscoderStagingAssetPathName, bool soundOfMain,
		int64_t encodingProfileKey, json encodingProfileDetailsRoot,
		string encodedTranscoderStagingAssetPathName,
		string encodedNFSStagingAssetPathName, string mmsWorkflowIngestionURL,
		string mmsBinaryIngestionURL, string mmsIngestionURL,
	...
	void addEncoding_CutFrameAccurate(
		...
		string sourceAssetPathName, int64_t sourceDurationInMilliSeconds,
		string sourceFileExtension, string sourcePhysicalDeliveryURL,
		string sourceTranscoderStagingAssetPathName, string endTime,
		...
		EncodingPriority encodingPriority, int64_t newUtcStartTimeInMilliSecs,
		int64_t newUtcEndTimeInMilliSecs
	);

	void updateIngestionAndEncodingLiveRecordingPeriod(
		int64_t ingestionJobKey, int64_t encodingJobKey,
		time_t utcRecordingPeriodStart, time_t utcRecordingPeriodEnd
	);

	int updateEncodingJob(
		int64_t encodingJobKey, EncodingError encodingError,
		bool isIngestionJobFinished, int64_t ingestionJobKey,
		string ingestionErrorMessage = "", bool forceEncodingToBeFailed = false
	);
	...
	void updateOutputRtmpAndPlaURL(
		int64_t ingestionJobKey, int64_t encodingJobKey, int outputIndex,
		string rtmpURL, string playURL
	);

	void updateOutputHLSDetails(
		int64_t ingestionJobKey, int64_t encodingJobKey, int outputIndex,
		int64_t deliveryCode, int segmentDurationInSeconds,
		int playlistEntriesNumber, string manifestDirectoryPath,
		string manifestFileName, string otherOutputOptions
	);
	...
	pair<int64_t, int64_t> saveSourceContentMetadata(
		...
		MMSEngineDBFacade::ContentType contentType, int64_t encodingProfileKey,
		json parametersRoot, bool externalReadOnlyStorage, string relativePath,
		string mediaSourceFileName, int mmsPartitionIndexUsed,
	...
	int64_t saveVariantContentMetadata(
		int64_t workspaceKey, int64_t ingestionJobKey,
		int64_t liveRecordingIngestionJobKey, int64_t mediaItemKey,
		bool externalReadOnlyStorage, string externalDeliveryTechnology,
		string externalDeliveryURL, string encodedFileName, string relativePath,
		int mmsPartitionIndexUsed, unsigned long long sizeInBytes,
	...
	*/
	...
	void addCrossReference(
		int64_t ingestionJobKey, int64_t sourceMediaItemKey,
		CrossReferenceType crossReferenceType, int64_t targetMediaItemKey,
	...
	json addYouTubeConf(
		int64_t workspaceKey, string label, string tokenType,
		string refreshToken, string accessToken
	);

	json modifyYouTubeConf(
		int64_t confKey, int64_t workspaceKey, string label, bool labelModified,
		string tokenType, bool tokenTypeModified, string refreshToken,
	...
	int64_t
	addFacebookConf(int64_t workspaceKey, string label, string userAccessToken);

	void modifyFacebookConf(
		int64_t confKey, int64_t workspaceKey, string label,
	...
	json
	getFacebookConfList(int64_t workspaceKey, int64_t confKey, string label);
	...
	int64_t
	addTwitchConf(int64_t workspaceKey, string label, string refreshToken);

	void modifyTwitchConf(
		int64_t confKey, int64_t workspaceKey, string label, string refreshToken
	);
	...
	void modifyTiktokConf(
		int64_t confKey, int64_t workspaceKey, string label, string token
	);
	...
	json addStream(
		int64_t workspaceKey, string label, string sourceType,
		int64_t encodersPoolKey, string url, string pushProtocol,
		int64_t pushEncoderKey, string pushServerName, int pushServerPort,
		string pushUri, int pushListenTimeout, int captureVideoDeviceNumber,
		string captureVideoInputFormat, int captureFrameRate, int captureWidth,
		int captureHeight, int captureAudioDeviceNumber,
		int captureChannelsNumber, int64_t tvSourceTVConfKey, string type,
		string description, string name, string region, string country,
		int64_t imageMediaItemKey, string imageUniqueName, int position,
		json userData
	);

	json modifyStream(
		int64_t confKey, string labelKey, int64_t workspaceKey,
		bool labelToBeModified, string label, bool sourceTypeToBeModified,
		string sourceType, bool encodersPoolKeyToBeModified,
		int64_t encodersPoolKey, bool urlToBeModified, string url,
		...
		string captureVideoInputFormat, bool captureFrameRateToBeModified,
		int captureFrameRate, bool captureWidthToBeModified, int captureWidth,
		...
		bool typeToBeModified, string type, bool descriptionToBeModified,
		string description, bool nameToBeModified, string name,
		bool regionToBeModified, string region, bool countryToBeModified,
		string country, bool imageToBeModified, int64_t imageMediaItemKey,
		string imageUniqueName, bool positionToBeModified, int position,
...
#ifdef __POSTGRES__
	json getStreamList(
		int64_t workspaceKey, int64_t liveURLKey, int start, int rows,
	...
	tuple<
		int64_t, string, string, string, string, int64_t, string, int, string,
		int, int, string, int, int, int, int, int, int64_t>
	getStreamDetails(int64_t workspaceKey, string label, bool warningIfMissing);

	tuple<string, string, string>
	getStreamDetails(int64_t workspaceKey, int64_t confKey);

	json addSourceTVStream(
		string type, int64_t serviceId, int64_t networkId,
		int64_t transportStreamId, string name, string satellite,
		int64_t frequency, string lnb, int videoPid, string audioPids,
		int audioItalianPid, int audioEnglishPid, int teletextPid,
		string modulation, string polarization, int64_t symbolRate,
		int64_t bandwidthInHz, string country, string deliverySystem
	);

	json modifySourceTVStream(
		int64_t confKey,

		bool typeToBeModified, string type, bool serviceIdToBeModified,
		int64_t serviceId, bool networkIdToBeModified, int64_t networkId,
		bool transportStreamIdToBeModified, int64_t transportStreamId,
		bool nameToBeModified, string name, bool satelliteToBeModified,
		string satellite, bool frequencyToBeModified, int64_t frequency,
		bool lnbToBeModified, string lnb, bool videoPidToBeModified,
		int videoPid, bool audioPidsToBeModified, string audioPids,
	...
	json getSourceTVStreamList(
		int64_t confKey, int start, int rows, string type, int64_t serviceId,
		string name, int64_t frequency, string lnb, int videoPid,
	...
	int64_t addAWSChannelConf(
		int64_t workspaceKey, string label, string channelId, string rtmpURL,
		string playURL, string type
	);

	void modifyAWSChannelConf(
		int64_t confKey, int64_t workspaceKey, string label, string channelId,
		string rtmpURL, string playURL, string type
	);

	void removeAWSChannelConf(int64_t workspaceKey, int64_t confKey);

	json getAWSChannelConfList(
		int64_t workspaceKey, int64_t confKey, string label,
		int type
	); // 0: all, 1: SHARED, 2: DEDICATED

	tuple<string, string, string, bool> reserveAWSChannel(
		int64_t workspaceKey, string label, int outputIndex,
		int64_t ingestionJobKey
	);

	string releaseAWSChannel(
		int64_t workspaceKey, int outputIndex, int64_t ingestionJobKey
	);

	int64_t addCDN77ChannelConf(
		int64_t workspaceKey, string label, string rtmpURL, string resourceURL,
		string filePath, string secureToken, string type
	);

	void modifyCDN77ChannelConf(
		int64_t confKey, int64_t workspaceKey, string label, string rtmpURL,
		string resourceURL, string filePath, string secureToken, string type
	);

	void removeCDN77ChannelConf(int64_t workspaceKey, int64_t confKey);

	json getCDN77ChannelConfList(
		int64_t workspaceKey, int64_t confKey, string label,
		int type
	); // 0: all, 1: SHARED, 2: DEDICATED

	tuple<string, string, string>
	getCDN77ChannelDetails(int64_t workspaceKey, string label);
	...
	void releaseCDN77Channel(
		int64_t workspaceKey, int outputIndex, int64_t ingestionJobKey
	);

	int64_t addRTMPChannelConf(
		int64_t workspaceKey, string label, string rtmpURL, string streamName,
		string userName, string password, string playURL, string type
	);

	void modifyRTMPChannelConf(
		int64_t confKey, int64_t workspaceKey, string label, string rtmpURL,
		string streamName, string userName, string password, string playURL,
		string type
	);

	void removeRTMPChannelConf(int64_t workspaceKey, int64_t confKey);

	json getRTMPChannelConfList(
		int64_t workspaceKey, int64_t confKey, string label,
		int type
	); // 0: all, 1: SHARED, 2: DEDICATED
	...
	void releaseRTMPChannel(
		int64_t workspaceKey, int outputIndex, int64_t ingestionJobKey
	);

	int64_t addHLSChannelConf(
		int64_t workspaceKey, string label, int64_t deliveryCode,
		int segmentDuration, int playlistEntriesNumber, string type
	);

	void modifyHLSChannelConf(
		int64_t confKey, int64_t workspaceKey, string label,
		int64_t deliveryCode, int segmentDuration, int playlistEntriesNumber,
		string type
	);

	void removeHLSChannelConf(int64_t workspaceKey, int64_t confKey);

	json getHLSChannelConfList(
		int64_t workspaceKey, int64_t confKey, string label,
		int type
	); // 0: all, 1: SHARED, 2: DEDICATED
	...
	int64_t addFTPConf(
		int64_t workspaceKey, string label, string server, int port,
		string userName, string password, string remoteDirectory
	);

	void modifyFTPConf(
		int64_t confKey, int64_t workspaceKey, string label, string server,
		int port, string userName, string password, string remoteDirectory
	);
	...
	int64_t addEMailConf(
		int64_t workspaceKey, string label, string addresses, string subject,
		string message
	);

	void modifyEMailConf(
		int64_t confKey, int64_t workspaceKey, string label, string addresses,
	...
	json addRequestStatistic(
		int64_t workspaceKey, string ipAddress, string userId,
		int64_t physicalPathKey, int64_t confStreamKey, string title
	);

	json getRequestStatisticList(
		int64_t workspaceKey, string userId, string title, string startDate,
		string endDate, int start, int rows
	);

	json getRequestStatisticPerContentList(
		int64_t workspaceKey, string title, string userId, string startDate,
		string endDate, int64_t minimalNextRequestDistanceInSeconds,
	...
	json getRequestStatisticPerMonthList(
		int64_t workspaceKey, string title, string userId,
	...
	json getLoginStatisticList(
		string startDate, string endDate, int start, int rows
	);

	void setLock(
		LockType lockType, int waitingTimeoutInSecondsIfLocked, string owner,
		string label, int milliSecondsToSleepWaitingLock = 500,
		string data = "no data"
	);
	...
	int64_t addEncoder(
		string label, bool external, bool enabled, string protocol,
		string publicServerName, string internalServerName, int port
	);

	void modifyEncoder(
		int64_t encoderKey, bool labelToBeModified, string label,
		bool externalToBeModified, bool external, bool enabledToBeModified,
		bool enabled, bool protocolToBeModified, string protocol,
	...
	bool isEncoderRunning(
		bool external, string protocol, string publicServerName,
		string internalServerName, int port
	);

	pair<bool, int> getEncoderInfo(
		bool external, string protocol, string publicServerName,
		string internalServerName, int port
	);

	void
	addAssociationWorkspaceEncoder(int64_t workspaceKey, int64_t encoderKey);

	void addAssociationWorkspaceEncoder(
		int64_t workspaceKey, string sharedEncodersPoolLabel,
		json sharedEncodersLabel
	);

	void
	removeAssociationWorkspaceEncoder(int64_t workspaceKey, int64_t encoderKey);

	json getEncoderWorkspacesAssociation(int64_t encoderKey);

	json getEncoderList(
		bool admin, int start, int rows, bool allEncoders, int64_t workspaceKey,
		bool runningInfo, int64_t encoderKey, string label, string serverName,
		int port,
	...
	json getEncodersPoolList(
		int start, int rows, int64_t workspaceKey, int64_t encodersPoolKey,
		string label,
		string labelOrder // "" or "asc" or "desc"
	);
	...
	pair<string, bool>
	getEncoderURL(int64_t encoderKey, string serverName = "");

	int64_t addEncodersPool(
		int64_t workspaceKey, string label, vector<int64_t> &encoderKeys
	);

	int64_t modifyEncodersPool(
		int64_t encodersPoolKey, int64_t workspaceKey, string newLabel,
	...
	void addUpdatePartitionInfo(
		int partitionKey, string partitionName, uint64_t currentFreeSizeInBytes,
...
#else
	pair<int, uint64_t>
	getPartitionToBeUsedAndUpdateFreeSpace(uint64_t ullFSEntrySizeInBytes);
#endif

	fs::path getPartitionPathName(int partitionKey);

	uint64_t updatePartitionBecauseOfDeletion(
		int partitionKey, uint64_t ullFSEntrySizeInBytes
	);
	...
	json getStreamInputRoot(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		string configurationLabel, string useVideoTrackFromPhysicalPathName,
		string useVideoTrackFromPhysicalDeliveryURL, int maxWidth,
		string userAgent, string otherInputOptions,
	...
	json getCountdownInputRoot(
		...
		string mmsSourceVideoAssetDeliveryURL, int64_t physicalPathKey,
		int64_t videoDurationInMilliSeconds, json drawTextDetailsRoot
	);

	json getDirectURLInputRoot(string url, json drawTextDetailsRoot);

	string getStreamingYouTubeLiveURL(
		shared_ptr<Workspace> workspace, int64_t ingestionJobKey,
		int64_t confKey, string liveURL
	);

	bool oncePerDayExecution(OncePerDayType oncePerDayType);

	static MMSEngineDBFacade::DeliveryTechnology
	fileFormatToDeliveryTechnology(string fileFormat);
  ...
  private:
	...
	shared_ptr<DBConnectionPool<PostgresConnection>>
		_masterPostgresConnectionPool;
	shared_ptr<PostgresConnectionFactory> _postgresSlaveConnectionFactory;
	shared_ptr<DBConnectionPool<PostgresConnection>>
		_slavePostgresConnectionPool;
...
#ifdef __POSTGRES__
	string getPostgresArray(
		vector<string> &arrayElements, bool emptyElementToBeRemoved,
	...
	json getStreamList(
		nontransaction &trans, shared_ptr<PostgresConnection> conn,
		int64_t workspaceKey, int64_t liveURLKey, int start, int rows,
		string label, bool labelLike, string url, string sourceType,
	...
	json getStreamList(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
		int64_t liveURLKey, int start, int rows, string label, bool labelLike,
		string url, string sourceType, string type, string name, string region,
		string country, string labelOrder
	);
#endif
...
#else
	string createAPIKeyForActiveDirectoryUser(
		shared_ptr<MySQLConnection> conn, int64_t userKey,
		string userEmailAddress, bool createRemoveWorkspace,
		bool ingestWorkflow, bool createProfiles, bool deliveryAuthorization,
		bool shareWorkspace, bool editMedia, bool editConfiguration,
		bool killEncoding, bool cancelIngestionJob, bool editEncodersPool,
		bool applicationRecorder, int64_t workspaceKey,
	...
	string createCode(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey, int64_t userKey,
		string userEmail, CodeType codeType, bool admin,
		bool createRemoveWorkspace, bool ingestWorkflow, bool createProfiles,
		bool deliveryAuthorization, bool shareWorkspace, bool editMedia,
		bool editConfiguration, bool killEncoding, bool cancelIngestionJob,
		bool editEncodersPool, bool applicationRecorder
	...
	isIngestionJobToBeManaged(
		int64_t ingestionJobKey, int64_t workspaceKey,
		IngestionStatus ingestionStatus, IngestionType ingestionType,
		shared_ptr<PostgresConnection> conn, transaction_base *trans
	);
#else
	tuple<bool, int64_t, int, MMSEngineDBFacade::IngestionStatus>
...
#ifdef __POSTGRES__
#else
	pair<shared_ptr<sql::ResultSet>, int64_t>
	getMediaItemsList_withoutTagsCheck(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
		int64_t mediaItemKey, vector<int64_t> &otherMediaItemsKey, int start,
		int rows, bool contentTypePresent, ContentType contentType,
		// bool startAndEndIngestionDatePresent,
		string startIngestionDate, string endIngestionDate, string title,
		int liveRecordingChunk, int64_t recordingCode,
		int64_t utcCutPeriodStartTimeInMilliSeconds,
		int64_t utcCutPeriodEndTimeInMilliSecondsPlusOneSecond,
		string jsonCondition, string orderBy, string jsonOrderBy, bool admin
	);

	pair<shared_ptr<sql::ResultSet>, int64_t> getMediaItemsList_withTagsCheck(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
		string temporaryTableName, int64_t mediaItemKey,
		vector<int64_t> &otherMediaItemsKey, int start, int rows,
		bool contentTypePresent, ContentType contentType,
		// bool startAndEndIngestionDatePresent,
		string startIngestionDate, string endIngestionDate, string title,
		int liveRecordingChunk, int64_t recordingCode,
	...
	void updateIngestionJob(
		shared_ptr<MySQLConnection> conn, int64_t ingestionJobKey,
		IngestionStatus newIngestionStatus, string errorMessage,
	...
	pair<int64_t, string> addWorkspace(
		...
		bool admin, bool createRemoveWorkspace, bool ingestWorkflow,
		bool createProfiles, bool deliveryAuthorization, bool shareWorkspace,
		bool editMedia, bool editConfiguration, bool killEncoding,
		bool cancelIngestionJob, bool editEncodersPool,
		bool applicationRecorder, string workspaceName,
		WorkspaceType workspaceType, string deliveryURL,
		EncodingPriority maxEncodingPriority, EncodingPeriod encodingPeriod,
		long maxIngestionsNumber, long maxStorageInMB, string languageCode,
		chrono::system_clock::time_point userExpirationDate
...
#else
	pair<int64_t, string> addWorkspace(
		shared_ptr<MySQLConnection> conn, int64_t userKey, bool admin,
		bool createRemoveWorkspace, bool ingestWorkflow, bool createProfiles,
		bool deliveryAuthorization, bool shareWorkspace, bool editMedia,
		bool editConfiguration, bool killEncoding, bool cancelIngestionJob,
		bool editEncodersPool, bool applicationRecorder, string workspaceName,
		WorkspaceType workspaceType, string deliveryURL,
		EncodingPriority maxEncodingPriority, EncodingPeriod encodingPeriod,
		long maxIngestionsNumber, long maxStorageInMB, string languageCode,
		chrono::system_clock::time_point userExpirationDate
	...
	void manageExternalUniqueName(
		shared_ptr<MySQLConnection> conn, int64_t workspaceKey,
		int64_t mediaItemKey, bool allowUniqueNameOverride, string uniqueName
	);
#endif
...
#else
	int64_t addUpdateWorkflowAsLibrary(
		shared_ptr<MySQLConnection> conn, int64_t userKey, int64_t workspaceKey,
		string label, int64_t thumbnailMediaItemKey, string jsonWorkflow,
		bool admin
	);
#endif
	...
	void manageCrossReferences(
		shared_ptr<PostgresConnection> conn, transaction_base *trans,
		int64_t ingestionJobKey, int64_t workspaceKey, int64_t mediaItemKey,
	...
	void manageIngestionJobStatusUpdate(
		int64_t ingestionJobKey, IngestionStatus newIngestionStatus,
		bool updateIngestionRootStatus, shared_ptr<PostgresConnection> conn,
		transaction_base *trans
	);
#else
	void manageIngestionJobStatusUpdate(
		int64_t ingestionJobKey, IngestionStatus newIngestionStatus,
		bool updateIngestionRootStatus, shared_ptr<MySQLConnection> conn
	);
#endif
	...
	getWorkspaceUsage(shared_ptr<MySQLConnection> conn, int64_t workspaceKey);
#endif

#ifdef __POSTGRES__
	json getWorkspaceDetailsRoot(
		shared_ptr<PostgresConnection> conn, nontransaction &trans, row &row,
		bool userAPIKeyInfo, bool costDetails
	);
#else
	json getWorkspaceDetailsRoot(
		shared_ptr<MySQLConnection> conn, shared_ptr<sql::ResultSet> resultSet,
		bool userAPIKeyInfo
	);
#endif
...
#else
	json getEncoderRoot(
		bool admin, bool runningInfo, shared_ptr<sql::ResultSet> resultSet
	);
#endif
...
#else
	void addTags(
		shared_ptr<MySQLConnection> conn, int64_t mediaItemKey, json tagsRoot
	);
#endif
