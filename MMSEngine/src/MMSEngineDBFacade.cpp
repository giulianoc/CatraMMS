        ...
        vector<tuple<string,string,bool,bool,bool,bool,bool,bool>>& vWorkspaceNameAPIKeyIfOwnerAndFlags)
{
    ...
    try
    {
        ...
        {
            lastSQLCommand = 
                "select w.name, a.apiKey, a.isOwner, a.flags from MMS_APIKey a, MMS_Workspace w where a.workspaceKey = w.workspaceKey and userKey = ?";
            shared_ptr<sql::PreparedStatement> preparedStatement (conn->_sqlConnection->prepareStatement(lastSQLCommand));
            int queryParameterIndex = 1;
            preparedStatement->setInt64(queryParameterIndex++, userKey);
            ...
            while (resultSet->next())
            {
                ...
                tuple<string,string,bool,bool,bool,bool,bool,bool> workspaceNameAPIKeyIfOwnerAndFlags =
    ...
    int64_t physicalPathKey, bool warningIfMissing)
{
    ...
    try
    {
        ...
        return make_pair(mediaItemKey, contentType);
    }
    catch(sql::SQLException se)
    {
        string exceptionMessage(se.what());
        ...
        
        {
            ...
            while (encodingResultSet->next())
            {
                ...
                else if (encodingItem->_encodingType == EncodingType::OverlayImageOnVideo)
                {
                    ...
                    {
                        lastSQLCommand = 
                            "select m.workspaceKey, p.partitionNumber, p.mediaItemKey, p.fileName, p.relativePath "
                            "from MMS_MediaItem m, MMS_PhysicalPath p where m.mediaItemKey = p.mediaItemKey and p.physicalPathKey = ?";
                        shared_ptr<sql::PreparedStatement> preparedStatementPhysicalPath (conn->_sqlConnection->prepareStatement(lastSQLCommand));
                        int queryParameterIndex = 1;
                    ...
                    {
                        lastSQLCommand = 
                            "select p.partitionNumber, p.fileName, p.relativePath "
                            "from MMS_MediaItem m, MMS_PhysicalPath p where m.mediaItemKey = p.mediaItemKey and p.physicalPathKey = ?";
                        shared_ptr<sql::PreparedStatement> preparedStatementPhysicalPath (conn->_sqlConnection->prepareStatement(lastSQLCommand));
                        int queryParameterIndex = 1;
...
int MMSEngineDBFacade::addOverlayTextOnVideoJob (
    int64_t ingestionJobKey,
    EncodingPriority encodingPriority,
        ...
        try
        {
            lastSQLCommand = 
                "create table if not exists MMS_APIKey ("
                    ...
                    "flags              SET('ADMIN', 'INGEST_WORKFLOW', 'CREATE_PROFILES', 'DELIVERY_AUTHORIZATION', 'SHARE_WORKSPACE') NOT NULL,"
                    "creationDate		TIMESTAMP DEFAULT CURRENT_TIMESTAMP,"
                    "expirationDate		DATETIME NOT NULL,"
