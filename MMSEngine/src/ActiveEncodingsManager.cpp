...
)
{
  ...
  _encodersLoadBalancer = make_shared<EncodersLoadBalancer> (
      _mmsEngineDBFacade, _configuration, _logger
  );
...

  {
	...
	int maxFaceRecognitionNumber = JSONUtils::asInt (
	    _configuration["mms"], "maxFaceRecognitionNumber", 0
	);
	_logger->info (
	    __FILEREF__ + "Configuration item"
	    + ", mms->maxFaceRecognitionNumber: "
	    + to_string (maxFaceRecognitionNumber)
	);
	maxFaceRecognitionNumber
	    += (MAXHIGHENCODINGSTOBEMANAGED + MAXMEDIUMENCODINGSTOBEMANAGED
	        + MAXLOWENCODINGSTOBEMANAGED);
	...
	for (EncodingJob &encodingJob : _lowPriorityEncodingJobs)
	{
	  encodingJob._encoderVideoAudioProxy.init (
	      lastProxyIdentifier++, &_mtEncodingJobs, _configuration,
	      multiEventsSet, _mmsEngineDBFacade, _mmsStorage,
	      _encodersLoadBalancer, faceRecognitionNumber,
	      maxFaceRecognitionNumber,
#ifdef __LOCALENCODER__
...
bool
ActiveEncodingsManager::isProcessorShutdown ()
{
...
void
ActiveEncodingsManager::operator() ()
{
  bool shutdown = false;
  ...
  while (!shutdown)
  {
	try
	{
	       ...
	       sortedEncodingPriorities)
	  {
		...
		for (int encodingJobIndex = 0;
		     encodingJobIndex < maxEncodingsToBeManaged; encodingJobIndex++)
		{
		  EncodingJob *encodingJob = &(encodingJobs[encodingJobIndex]);

		  if (encodingJob->_status
		      == EncoderVideoAudioProxy::EncodingJobStatus::Free)
		  {
			...
			continue;
		               ...
		               == EncoderVideoAudioProxy::EncodingJobStatus::Running
		           || encodingJob->_status
		                  == EncoderVideoAudioProxy::EncodingJobStatus::
		                      GoingToRun)
		  {
			if (encodingJob->_status
			    == EncoderVideoAudioProxy::EncodingJobStatus::Running)
			...
			*/
			...
			if (encodingJob->_encodingItem->_encodingType
			    ...
			    && chrono::duration_cast<chrono::hours> (
			           chrono::system_clock::now ()
			           - encodingJob->_encodingJobStart
			       ) > chrono::hours (24))
			{
			  _logger->error (
			      __FILEREF__ + "EncodingJob is not finishing"
			      ...
			      + to_string (chrono::duration_cast<chrono::hours> (
			                       ...
			                       .count ())
			      + "@" + ", workspace: "
			      + encodingJob->_encodingItem->_workspace->_name
			      + ", _ingestionJobKey: "
			      + to_string (encodingJob->_encodingItem->_ingestionJobKey)
			      + ", _encodingJobKey: "
			      + to_string (encodingJob->_encodingItem->_encodingJobKey)
			      + ", _encodingPriority: "
			      + to_string (static_cast<int> (
			          encodingJob->_encodingItem->_encodingPriority
			      ))
			      + ", _encodingType: "
			      ...
			      + ", encodingJob->_status: "
			      + EncoderVideoAudioProxy::toString (encodingJob->_status)
			  );
			}
			else
			{
			  _logger->info (
			      __FILEREF__ + "EncodingJob still running"
			      + ", elapsed (minutes): "
			      + to_string (chrono::duration_cast<chrono::minutes> (
			                       chrono::system_clock::now ()
			                       - encodingJob->_encodingJobStart
			      )
			                       .count ())
		       ...
		       // EncoderVideoAudioProxy::EncodingJobStatus::ToBeRun)
		  {
			...
			_logger->info (
			    __FILEREF__ + "processEncodingJob begin" + ", workspace: "
			    + encodingJob->_encodingItem->_workspace->_name
			    + ", _ingestionJobKey: "
			    + to_string (encodingJob->_encodingItem->_ingestionJobKey)
			    + ", _encodingJobKey: "
			...
			try
			{
			  ...
			  _logger->info (
			      __FILEREF__ + "processEncodingJob done"
			...
			{
			  _logger->error (
			      __FILEREF__ + "processEncodingJob failed"
			      + ", @MMS statistics@ - elapsed (seconds): @"
			      + to_string (chrono::duration_cast<chrono::seconds> (
			                       chrono::system_clock::now ()
			                       - processingItemStart
		...
		_logger->info (
		    __FILEREF__ + "Processing encoding jobs statistics"
		    + ", encodingPriority: "
		    + MMSEngineDBFacade::toString (encodingPriority)
		    + ", maxEncodingsToBeManaged: "
		    + to_string (maxEncodingsToBeManaged)
		    + ", freeEncodingJobsNumber: " + to_string (freeEncodingJobsNumber)
		    + ", runningEncodingJobsNumber: "
		    + to_string (runningEncodingJobsNumber)
		    + ", goingToRunEncodingJobsNumber: "
		    + to_string (goingToRunEncodingJobsNumber)
		    + ", toBeRunEncodingJobsNumber: "
		    + to_string (toBeRunEncodingJobsNumber)
		);

		if (freeEncodingJobsNumber == 0)
		  _logger->warn (
		      __FILEREF__ + "maxEncodingsToBeManaged should to be increased"
		      + ", encodingPriority: "
		      + MMSEngineDBFacade::toString (encodingPriority)
		      + ", maxEncodingsToBeManaged: "
		      + to_string (maxEncodingsToBeManaged)
		      + ", freeEncodingJobsNumber: "
		      + to_string (freeEncodingJobsNumber)
		      + ", runningEncodingJobsNumber: "
		      + to_string (runningEncodingJobsNumber)
		      + ", goingToRunEncodingJobsNumber: "
		      + to_string (goingToRunEncodingJobsNumber)
	  ...
	  long elapsedInSeconds
	      = chrono::duration_cast<chrono::seconds> (endEvent - startEvent)
	            .count ();
	  _logger->info (
	      __FILEREF__ + "End checking encodingJobs"
...
void
ActiveEncodingsManager::processEncodingJob (EncodingJob *encodingJob)
{
	...
	case MMSEngineDBFacade::EncodingType::AddSilentAudio:
	  {
		encodingJob->_encoderVideoAudioProxy.setEncodingData (
		    &(encodingJob->_status), encodingJob->_encodingItem
		);
		...
		thread encoderVideoAudioProxyThread (
		    ref (encodingJob->_encoderVideoAudioProxy)
		);
		...
		encodingJob->_status
		    = EncoderVideoAudioProxy::EncodingJobStatus::GoingToRun;
...
void
ActiveEncodingsManager::addEncodingItem (
...
)
{
  ...
  if (encodingItem->_encodingPriority
      == MMSEngineDBFacade::EncodingPriority::High)
  {
	encodingJobs = _highPriorityEncodingJobs;
	maxEncodingsToBeManaged = MAXHIGHENCODINGSTOBEMANAGED;
  ...
  for (encodingJobIndex = 0; encodingJobIndex < maxEncodingsToBeManaged;
       encodingJobIndex++)
  {
	if ((encodingJobs[encodingJobIndex])._status
	    == EncoderVideoAudioProxy::EncodingJobStatus::Free)
	{
	  (encodingJobs[encodingJobIndex])._status
	      = EncoderVideoAudioProxy::EncodingJobStatus::ToBeRun;
	  (encodingJobs[encodingJobIndex])._encodingJobStart
	      = chrono::system_clock::now ();
  ...
  {
	_logger->warn (
	    __FILEREF__ + "Max Encodings Manager capacity reached"
	    ...
	    + ", encodingPriority: "
	    + MMSEngineDBFacade::toString (encodingItem->_encodingPriority)
  ...
  _logger->info (
      __FILEREF__ + "Encoding Job Key added"
...
unsigned long
ActiveEncodingsManager::addEncodingItems (
...
)
{
  ...
  for (shared_ptr<MMSEngineDBFacade::EncodingItem> encodingItem :
       vEncodingItems)
  {
	_logger->info (
	    __FILEREF__ + "Adding Encoding Item " + to_string (encodingItemIndex)
	    + "/" + to_string (encodingItemsNumber)
	    ...
	    + to_string (static_cast<int> (encodingItem->_encodingPriority))
	...
	{
	  _logger->error (
	      __FILEREF__ + "addEncodingItem failed "
	      + to_string (encodingItemIndex) + "/"
	      + to_string (encodingItemsNumber)
	      + ", encodingItem->_workspace->_name: "
	      + encodingItem->_workspace->_name
