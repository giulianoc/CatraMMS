...
CheckRefreshPartitionFreeSizeTimes::CheckRefreshPartitionFreeSizeTimes(
	...
	: Times2(refreshPartitionFreeSizeTimesSchedule, MMSENGINE_CHECKREFRESHPARTITIONFREESIZETIMES_CLASSNAME)
...
void CheckRefreshPartitionFreeSizeTimes::handleTimeOut(void)
{
	...
	shared_ptr<Event2> event =
		_multiEventsSet->getEventsFactory()->getFreeEvent<Event2>(MMSENGINE_EVENTTYPEIDENTIFIER_CHECKREFRESHPARTITIONFREESIZEEVENT);
	...
	_logger->debug(
		__FILEREF__ + "addEvent: EVENT_TYPE" + ", MMSENGINE_EVENTTYPEIDENTIFIER_CHECKREFRESHPARTITIONFREESIZE" +
