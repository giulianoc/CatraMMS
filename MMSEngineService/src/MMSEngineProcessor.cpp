...
MMSEngineProcessor::MMSEngineProcessor(
	int processorIdentifier, shared_ptr<spdlog::logger> logger,
	shared_ptr<MultiEventsSet> multiEventsSet,
	shared_ptr<MMSEngineDBFacade> mmsEngineDBFacade,
	shared_ptr<MMSStorage> mmsStorage, shared_ptr<long> processorsThreadsNumber,
...
)
{
	...
	_mmsWorkflowIngestionURL = mmsAPIProtocol + "://" + mmsAPIHostname + ":" +
							   to_string(mmsAPIPort) + "/catramms/" +
							   mmsAPIVersion + mmsAPIWorkflowURI;

	_mmsIngestionURL = mmsAPIProtocol + "://" + mmsAPIHostname + ":" +
					   to_string(mmsAPIPort) + "/catramms/" + mmsAPIVersion +
					   mmsAPIIngestionURI;

	_mmsBinaryIngestionURL = mmsBinaryProtocol + "://" + mmsBinaryHostname +
							 ":" + to_string(mmsBinaryPort) + "/catramms/" +
							 mmsBinaryVersion + mmsBinaryIngestionURI;
	...
	while (!processorShutdown)
	{
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_LOCALASSETINGESTIONEVENT: // 2
		{
			...
			_multiEventsSet->getEventsFactory()
				->releaseEvent<LocalAssetIngestionEvent>(
					localAssetIngestionEvent
				);
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_CONTENTRETENTIONEVENT: // 4
		{
			...
			try
			{
				...
				{
					thread contentRetention(
						&MMSEngineProcessor::handleContentRetentionEventThread,
						this, _processorsThreadsNumber
					);
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_MULTILOCALASSETINGESTIONEVENT: // 5
		{
			...
			_multiEventsSet->getEventsFactory()
				->releaseEvent<MultiLocalAssetIngestionEvent>(
					multiLocalAssetIngestionEvent
				);
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_DBDATARETENTIONEVENT: // 7
		{
			...
			try
			{
				...
				*/
				{
					thread dbDataRetention(
						&MMSEngineProcessor::handleDBDataRetentionEventThread,
						this
					);
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_CHECKREFRESHPARTITIONFREESIZEEVENT: // 8
		{
			...
			try
			{
				...
				*/
				{
					thread checkRefreshPartitionFreeSize(
						&MMSEngineProcessor::
		...
		case MMSENGINE_EVENTTYPEIDENTIFIER_GEOINFOEVENT: // 10
		{
			...
			try
			{
				...
				*/
				{
					thread geoInfo(
						&MMSEngineProcessor::handleGEOInfoEventThread, this
					);
					geoInfo.detach();
				}
			}
		...
		default:
			throw runtime_error(
				string("Event type identifier not managed") +
				to_string(event->getEventKey().first)
			);
		}
		...
		long elapsedInSeconds =
			chrono::duration_cast<chrono::seconds>(endEvent - startEvent)
				.count();
...
int MMSEngineProcessor::getMaxAdditionalProcessorThreads()
{
	...
	SPDLOG_INFO(
		...
		", maxAdditionalProcessorThreads: " +
		to_string(maxAdditionalProcessorThreads)
	);
...
void MMSEngineProcessor::handleCheckIngestionEvent()
{
	...
	try
	{
		...
		vector<tuple<
			int64_t, string, shared_ptr<Workspace>, string, string,
		...
		try
		{
			...
			_mmsEngineDBFacade->getIngestionsToBeManaged(
				ingestionsToBeManaged, _processorMMS, _maxIngestionJobsPerEvent,
				 ...
				 ingestionToBeManaged : ingestionsToBeManaged)
		{
			...
			try
			{
				...
				tie(ingestionJobKey, ingestionJobLabel, workspace,
				...
				SPDLOG_INFO(
					...
					to_string(workspace->_workspaceKey) +
					", ingestionDate: " + ingestionDate + ", ingestionType: " +
				...
				{
					if (ingestionType !=
						MMSEngineDBFacade::IngestionType::RemoveContent)
					{
						_mmsEngineDBFacade
							->checkWorkspaceStorageAndMaxIngestionNumber(
								workspace->_workspaceKey
							);
					}
				}
				...
				if (ingestionStatus == MMSEngineDBFacade::IngestionStatus::
										   SourceDownloadingInProgress ||
					ingestionStatus == MMSEngineDBFacade::IngestionStatus::
										   ...
										   SourceUploadingInProgress)
				{
					...
					string sourceFileName =
						to_string(ingestionJobKey) + "_source";

					{
						...
						localAssetIngestionEvent->setSource(
							MMSENGINEPROCESSORNAME
						);
						localAssetIngestionEvent->setDestination(
							MMSENGINEPROCESSORNAME
						);
						localAssetIngestionEvent->setExpirationTimePoint(
							chrono::system_clock::now()
						);

						localAssetIngestionEvent->setExternalReadOnlyStorage(
							false
						);
						localAssetIngestionEvent->setIngestionJobKey(
							ingestionJobKey
						);
						localAssetIngestionEvent->setIngestionSourceFileName(
							sourceFileName
						);
						...
						localAssetIngestionEvent
							->setIngestionRowToBeUpdatedAsSuccess(true);

						localAssetIngestionEvent->setMetadataContent(
							metaDataContent
						);
				...
				else // Start_TaskQueued
				{
					...
					{
						string errorMessage =
							string("metadata json is not well format") +
							", _processorIdentifier: " +
							to_string(_processorIdentifier) +
							", ingestionJobKey: " + to_string(ingestionJobKey) +
							", metaDataContent: " + metaDataContent;
					...
					try
					{
						...
						if (ingestionType ==
							...
							dependencies = validator.validateSingleTaskMetadata(
								workspace->_workspaceKey, ingestionType,
								parametersRoot
							);
					}
					catch (runtime_error &e)
...

					{
							...
							MMSEngineDBFacade::IngestionType::GroupOfTasks)
						{
							...
							{
								manageGroupOfTasks(
									ingestionJobKey, workspace, parametersRoot
								);
							}
							catch (runtime_error &e)
							{
								 ...
								 MMSEngineDBFacade::IngestionType::AddContent)
						{
							...
							{
								tuple<
									MMSEngineDBFacade::IngestionStatus, string,
								...
								tie(nextIngestionStatus, mediaSourceURL,
									mediaFileFormat, md5FileCheckSum,
									fileSizeInBytes, externalReadOnlyStorage) =
									mediaSourceDetails;
							}
							catch (runtime_error &e)
							{
							...
							try
							{
								if (externalReadOnlyStorage)
								{
									...
									localAssetIngestionEvent
										->setExternalReadOnlyStorage(true);
									localAssetIngestionEvent
										->setExternalStorageMediaSourceURL(
											mediaSourceURL
										);
									localAssetIngestionEvent
										->setIngestionJobKey(ingestionJobKey);
									// localAssetIngestionEvent->setIngestionSourceFileName(sourceFileName);
									// localAssetIngestionEvent->setMMSSourceFileName("");
									localAssetIngestionEvent->setWorkspace(
										workspace
									);
									...
									localAssetIngestionEvent
										->setIngestionRowToBeUpdatedAsSuccess(
											true
										);
								...
								else
								{
										 ...
										 // MMSEngineDBFacade::IngestionStatus::SourceUploadingInProgress)
									{
										...
										_mmsEngineDBFacade->updateIngestionJob(
											ingestionJobKey,
											nextIngestionStatus, errorMessage,
											processorMMS
										);
									}
								}
							...
							{
								string errorMessage =
									string("Downloading media source or update "
									 ...
									 RemoveContent)
						{
							...
							try
							{
								...
								*/
										...
										maxAdditionalProcessorThreads)
								{
									...
									_mmsEngineDBFacade->updateIngestionJob(
										ingestionJobKey, ingestionStatus,
										errorMessage, processorMMS
									);
								}
								else
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::FTPDelivery)
						{
							// mediaItemKeysDependency is present because
							// checked by
							// _mmsEngineDBFacade->getIngestionsToBeManaged
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::LocalCopy)
						{
							// mediaItemKeysDependency is present because
							// checked by
							// _mmsEngineDBFacade->getIngestionsToBeManaged
							try
							{
								...
								{
									string errorMessage =
										string("Local-Copy Task is not enabled "
											   "in this MMS deploy") +
										", _processorIdentifier: " +
										to_string(_processorIdentifier) +
										", ingestionJobKey: " +
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::HTTPCallback)
						{
							// mediaItemKeysDependency is present because
							// checked by
							// _mmsEngineDBFacade->getIngestionsToBeManaged
								...
								*/
								...
								else
								{
									thread httpCallbackThread(
										&MMSEngineProcessor::httpCallbackThread,
										this, _processorsThreadsNumber,
										ingestionJobKey, workspace,
										parametersRoot,
										dependencies // it cannot be passed as
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::Encode)
						{
							try
							{
								manageEncodeTask(
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::VideoSpeed)
						{
							try
							{
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 ingestionType ==
									 MMSEngineDBFacade::IngestionType::
										 PeriodicalFrames ||
								 ingestionType == MMSEngineDBFacade::
													  IngestionType::IFrames ||
								 ingestionType ==
									 MMSEngineDBFacade::IngestionType::
										 ...
										 MotionJPEGByIFrames)
						{
								...
								else // Frame
								{
											...
											maxAdditionalProcessorThreads)
									{
										_logger->warn(
											...
											"_processorsThreadsNumber.use_"
											"count(): " +
											to_string(_processorsThreadsNumber
														  .use_count()) +
											", _processorThreads + "
											"maxAdditionalProcessorThreads: " +
											to_string(
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::Slideshow)
						{
							// mediaItemKeysDependency is present because
							// checked by
							// _mmsEngineDBFacade->getIngestionsToBeManaged
									 ...
									 ConcatDemuxer)
						{
							...
							try
							{
								...
								else
								{
									thread manageConcatThread(
										&MMSEngineProcessor::manageConcatThread,
										this, _processorsThreadsNumber,
										ingestionJobKey, workspace,
										parametersRoot,
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::Cut)
						{
							// mediaItemKeysDependency is present because
							// checked by
							// _mmsEngineDBFacade->getIngestionsToBeManaged
									 ...
									 FaceRecognition)
						{
							...
							try
							{
								manageFaceRecognitionMediaTask(
									ingestionJobKey, ingestionStatus, workspace,
									parametersRoot, dependencies
								);
							}
							catch (runtime_error &e)
							{
								 ...
								 MMSEngineDBFacade::IngestionType::LiveRecorder)
						{
							try
							{
								manageLiveRecorder(
									...
									ingestionStatus, workspace, parametersRoot
								);
							}
							catch (runtime_error &e)
							{
								 ...
								 MMSEngineDBFacade::IngestionType::LiveProxy)
						{
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::VODProxy)
						{
							try
							{
								manageVODProxy(
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::Countdown)
						{
							try
							{
								manageCountdown(
									ingestionJobKey, ingestionStatus,
									ingestionDate, workspace, parametersRoot,
									dependencies
								);
							}
							catch (runtime_error &e)
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::LiveGrid)
						{
							try
							{
								manageLiveGrid(
							...
							catch (exception &e)
							{
								...
								throw runtime_error(errorMessage);
								 ...
								 MMSEngineDBFacade::IngestionType::LiveCut)
						{
							try
							{
								/* 2021-02-19: check on threads is already done
								...
								else
								{
									...
									if (segmenterType == "hlsSegmenter")
									{
										thread manageLiveCutThread(
											&MMSEngineProcessor::
												manageLiveCutThread_hlsSegmenter,
											this, _processorsThreadsNumber,
											ingestionJobKey, ingestionJobLabel,
									...
									else
									{
										thread manageLiveCutThread(
											&MMSEngineProcessor::
												manageLiveCutThread_streamSegmenter,
											this, _processorsThreadsNumber,
											ingestionJobKey, workspace,
											parametersRoot
										);
		...
		{
			shared_ptr<Event2> event =
				_multiEventsSet->getEventsFactory()->getFreeEvent<Event2>(
...
)
{
	...
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "handleLocalAssetIngestionEventThread",
...
)
{
	...
	try
	{
		if (!localAssetIngestionEvent.getExternalReadOnlyStorage())
		{
			...
			workspaceIngestionBinaryPathName.append("/").append(
				localAssetIngestionEvent.getIngestionSourceFileName()
			);
			...
			else if (fileFormat == "m3u8-tar.gz")
			{
				...
				string localWorkspaceIngestionBinaryPathName =
					workspaceIngestionBinaryPathName + ".tar.gz";
					...
					fs::is_regular_file(localWorkspaceIngestionBinaryPathName))
				{
					...
					{
						string errorMessage =
							string("manageTarFileInCaseOfIngestionOfSegments "
								   "failed") +
							", ingestionJobKey: " +
							to_string(
								localAssetIngestionEvent.getIngestionJobKey()
		...
		else
		{
			...
			if (!(mediaSourceURL.size() >= externalStoragePrefix.size() &&
				  0 ==
					  mediaSourceURL.compare(
						  0, externalStoragePrefix.size(), externalStoragePrefix
					  )))
			{
				string errorMessage =
					string("mediaSourceURL is not an externalStorage reference"
					) +
					", _processorIdentifier: " +
					to_string(_processorIdentifier) + ", ingestionJobKey: " +
					to_string(localAssetIngestionEvent.getIngestionJobKey()) +
			...
			externalStorageRelativePathName =
				mediaSourceURL.substr(externalStoragePrefix.length());
			binaryPathName =
				_mmsStorage->getMMSRootRepository() /
				("ExternalStorage_" +
	...
	try
	{
		...
		tuple<
			MMSEngineDBFacade::IngestionStatus, string, string, string, int,
			bool>
			mediaSourceDetails = getMediaSourceDetails(
				localAssetIngestionEvent.getIngestionJobKey(),
				localAssetIngestionEvent.getWorkspace(),
		...
		// Here the mediaFileFormat is retrieved
		{
			...
			if ((mediaSourceURL.size() >= youTubePrefix1.size() &&
				 0 == mediaSourceURL.compare(
						  0, youTubePrefix1.size(), youTubePrefix1
					  )) ||
	...
	try
	{
		validateMediaSourceFile(
			localAssetIngestionEvent.getIngestionJobKey(),
			binaryPathName.string(), mediaFileFormat, md5FileCheckSum,
			fileSizeInBytes
		);
	}
	catch (runtime_error &e)
	...
	try
	{
		if (!localAssetIngestionEvent.getExternalReadOnlyStorage())
		{
			...
			mmsAssetPathName = _mmsStorage->moveAssetInMMSRepository(
				localAssetIngestionEvent.getIngestionJobKey(), binaryPathName,
				localAssetIngestionEvent.getWorkspace()->_directoryName,
				mediaSourceFileName, relativePathToBeUsed,
		...
		else
		{
			...
			size_t fileNameIndex =
				externalStorageRelativePathName.find_last_of("/");
			...
			relativePathToBeUsed =
				externalStorageRelativePathName.substr(0, fileNameIndex + 1);
			mediaSourceFileName =
				externalStorageRelativePathName.substr(fileNameIndex + 1);
		}
	}
	catch (runtime_error &e)
	{
	...
	if (mediaFileFormat == "m3u8-tar.gz")
	{
		...
		try
		{
			for (fs::directory_entry const &entry :
				 fs::directory_iterator(mmsAssetPathName))
			{
				try
				{
					...
					if (entry.path().filename().string().size() >=
							m3u8Suffix.size() &&
		...
		try
		{
			...
			if (mediaFileFormat == "m3u8-tar.gz")
				mediaInfoDetails = ffmpeg.getMediaInfo(
					localAssetIngestionEvent.getIngestionJobKey(),
					isMMSAssetPathName, timeoutInSeconds,
					mmsAssetPathName + "/" + m3u8FileName, videoTracks,
					audioTracks
				);
			else
				mediaInfoDetails = ffmpeg.getMediaInfo(
					localAssetIngestionEvent.getIngestionJobKey(),
					isMMSAssetPathName, timeoutInSeconds, mmsAssetPathName,
	...
	{
		string errorMessage =
			string("Unknown mediaFileFormat") +
			", _processorIdentifier: " + to_string(_processorIdentifier) +
			", ingestionJobKey: " +
			to_string(localAssetIngestionEvent.getIngestionJobKey()) +
			", mmsAssetPathName: " + mmsAssetPathName;
	...
	try
	{
		...
		if (mediaFileFormat == "m3u8-tar.gz")
		{
			...
			// links
			for (fs::directory_entry const &entry :
				 fs::recursive_directory_iterator(mmsAssetPathName))
			{
				if (entry.is_regular_file())
		...
		int64_t variantOfMediaItemKey = -1;
		{
			...
			if (JSONUtils::isMetadataPresent(
					parametersRoot, variantOfMediaItemKeyField
				))
			{
				variantOfMediaItemKey = JSONUtils::asInt64(
					parametersRoot, variantOfMediaItemKeyField, -1
				);
			}
			else if (JSONUtils::isMetadataPresent(
						 parametersRoot, variantOfUniqueNameField
					 ))
			{
				bool warningIfMissing = false;

				string variantOfUniqueName = JSONUtils::asString(
					parametersRoot, variantOfUniqueNameField, ""
				);
				...
				tie(variantOfMediaItemKey, ignore) = mediaItemKeyDetails;
						 ...
						 parametersRoot, variantOfIngestionJobKeyField
					 ))
			{
				int64_t variantOfIngestionJobKey = JSONUtils::asInt64(
					parametersRoot, variantOfIngestionJobKeyField, -1
				);
				vector<tuple<int64_t, int64_t, MMSEngineDBFacade::ContentType>>
					mediaItemsDetails;
				bool warningIfMissing = false;

				_mmsEngineDBFacade->getMediaItemDetailsByIngestionJobKey(
					localAssetIngestionEvent.getWorkspace()->_workspaceKey,
					variantOfIngestionJobKey, -1, mediaItemsDetails,
					warningIfMissing,
				...
				{
					string errorMessage =
						string("IngestionJob does not refer the correct media "
							   "Items number") +
						", _processorIdentifier: " +
						to_string(_processorIdentifier) +
						", variantOfIngestionJobKey: " +
						to_string(variantOfIngestionJobKey) +
						", workspaceKey: " +
						to_string(localAssetIngestionEvent.getWorkspace()
									  ->_workspaceKey) +
				...
				tuple<int64_t, int64_t, MMSEngineDBFacade::ContentType>
					mediaItemsDetailsReturn = mediaItemsDetails[0];
				tie(variantOfMediaItemKey, ignore, ignore) =
					mediaItemsDetailsReturn;
			}
		}
		...
		if (variantOfMediaItemKey == -1)
		{
			SPDLOG_INFO(
				...
				to_string(localAssetIngestionEvent.getExternalReadOnlyStorage()
				) +
				...
				+ ", imageWidth: " + to_string(imageWidth) + ", imageHeight: " +
				to_string(imageHeight) + ", imageFormat: " + imageFormat +
			...
			pair<int64_t, int64_t> mediaItemKeyAndPhysicalPathKey =
				_mmsEngineDBFacade->saveSourceContentMetadata(
					...
					localAssetIngestionEvent.getIngestionJobKey(),
					localAssetIngestionEvent
						.getIngestionRowToBeUpdatedAsSuccess(),
					contentType, encodingProfileKey, parametersRoot,
					localAssetIngestionEvent.getExternalReadOnlyStorage(),
					relativePathToBeUsed, mediaSourceFileName, mmsPartitionUsed,
					sizeInBytes,
			...
			SPDLOG_INFO(
				...
				to_string(localAssetIngestionEvent.getIngestionJobKey()) +
				", mediaItemKey: " +
				to_string(mediaItemKeyAndPhysicalPathKey.first) +
				", physicalPathKey: " +
				to_string(mediaItemKeyAndPhysicalPathKey.second)
			);
		}
		else
		{
			...
			int64_t physicalItemRetentionInMinutes = -1;
			{
				...
				{
					string retention =
						JSONUtils::asString(parametersRoot, field, "1d");
					...
					if (JSONUtils::isMetadataPresent(userDataRoot, field))
					{
							...
							))
						{
							...
							sourceIngestionJobKey =
								JSONUtils::asInt64(mmsDataRoot, field, -1);
						}
					}
				}
			}
			...
			SPDLOG_INFO(
				...
				", encodingProfileKey: " + to_string(encodingProfileKey) +
				", physicalItemRetentionInMinutes: " +
				to_string(physicalItemRetentionInMinutes)
			...
			int64_t physicalPathKey =
				_mmsEngineDBFacade->saveVariantContentMetadata(
					localAssetIngestionEvent.getWorkspace()->_workspaceKey,
					localAssetIngestionEvent.getIngestionJobKey(),
					sourceIngestionJobKey, variantOfMediaItemKey,
					localAssetIngestionEvent.getExternalReadOnlyStorage(),
					externalDeliveryTechnology, externalDeliveryURL,

					mediaSourceFileName, relativePathToBeUsed, mmsPartitionUsed,
			...
			SPDLOG_INFO(
				...
				to_string(variantOfMediaItemKey) +
...
)
{
	try
	{
		...
		validator.fillReferencesOutput(
			workspace->_workspaceKey, parametersRoot, referencesOutput
		);
		...
		for (pair<int64_t, int64_t> referenceOutput : referencesOutput)
		{
			...
			try
			{
				...
				_mmsEngineDBFacade->addIngestionJobOutput(
					ingestionJobKey, referenceOutput.first,
					referenceOutput.second, liveRecordingIngestionJobKey
				);
			}
			catch (runtime_error &e)
			{
		...
		*/
		...
		string errorMessage = "";
		if (groupOfTasksIngestionStatus !=
			MMSEngineDBFacade::IngestionStatus::End_TaskSuccess)
			errorMessage =
		...
		_mmsEngineDBFacade->updateIngestionJob(
			ingestionJobKey, groupOfTasksIngestionStatus, errorMessage
		);
	}
	catch (runtime_error &e)
	{
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "removeContentThread", _processorIdentifier,
	...
	try
	{
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				{
					if (dependencyType ==
						Validator::DependencyType::MediaItemKey)
					{
						bool warningIfMissing = false;
						tuple<
							MMSEngineDBFacade::ContentType, string, string,
						...
						tie(localContentType, localTitle, localUserData,
							localIngestionDate, ignore,
							ingestionJobKeyOfItemToBeRemoved) =
							mediaItemKeyDetails;

						int ingestionDependenciesNumber =
							_mmsEngineDBFacade
								->getNotFinishedIngestionDependenciesNumberByIngestionJobKey(
									ingestionJobKeyOfItemToBeRemoved,
						...
						if (ingestionDependenciesNumber > 0)
						{
							string errorMessage =
								...
								", ingestionJobKey: " +
								to_string(ingestionJobKey) +
								", ingestionDependenciesNumber not finished: " +
					...
					else
					{
						bool warningIfMissing = false;
						tuple<
							int64_t, MMSEngineDBFacade::ContentType, string,
						...
						string localIngestionDate;
						int64_t ingestionJobKeyOfItemToBeRemoved;
						tie(localMediaItemKey, localContentType, localTitle,
							localUserData, localIngestionDate,
							ingestionJobKeyOfItemToBeRemoved, ignore, ignore,
							ignore) = mediaItemDetails;
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "ftpDeliveryContentThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		tie(ftpServer, ftpPort, ftpUserName, ftpPassword, ftpRemoteDirectory) =
			ftp;
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				if (dependencyType == Validator::DependencyType::MediaItemKey)
				{
					...
					tie(physicalPathKey, mmsAssetPathName, ignore, ignore,
				...
				else
				{
...

					{
						...
						tie(mediaItemKey, ignore, ignore, ignore, ignore,
					...
					tie(mmsAssetPathName, ignore, ignore, fileName, sizeInBytes,
						deliveryFileName) =
						physicalPathFileNameSizeInBytesAndDeliveryFileName;
				}

				ftpUploadMediaSource(
					mmsAssetPathName, fileName, sizeInBytes, ingestionJobKey,
					workspace, mediaItemKey, physicalPathKey, ftpServer,
					ftpPort, ftpUserName, ftpPassword, ftpRemoteDirectory,
					deliveryFileName
				);
			}
			catch (runtime_error &e)
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "localCopyContentThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "extractTracksContentThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		ingestionJobKey
	);
		...
		{
			{
					 ...
					 trackIndex++)
				{
					...
					string trackType =
						JSONUtils::asString(trackRoot, field, "");

					int trackNumber = 0;
					field = "TrackNumber";
					trackNumber = JSONUtils::asInt(trackRoot, field, 0);

					tracksToBeExtracted.push_back(
						make_pair(trackType, trackNumber)
					);
				}
			}
			 ...
			 dependencies)
		{
			...
			try
			{
...

				{
					...
					{
						localSourceFileName = to_string(ingestionJobKey) + "_" +
											  to_string(key) + "_extractTrack" +
											  "." + outputFileFormat;
					...
					string mediaMetaDataContent = generateMediaMetadataToIngest(
						ingestionJobKey, outputFileFormat, title,
						imageOfVideoMediaItemKey, cutOfVideoMediaItemKey,
						cutOfAudioMediaItemKey, startTimeInSeconds,
						endTimeInSeconds, parametersRoot
					);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "httpCallbackThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		json httpHeadersRoot = json::array();
		{
			...
			callbackTimeoutInSeconds =
				JSONUtils::asInt(parametersRoot, field, 120);
						 ...
						 &keyAndDependencyType : dependencies)
				{
					...
					try
					{
							...
							Validator::DependencyType::MediaItemKey)
						{
							...
							bool warningIfMissing = false;
							tuple<
								int64_t, string, int, string, string, int64_t,
								string>
								physicalPathDetails =
									_mmsStorage->getPhysicalPathDetails(
							...
							tie(physicalPathKey, ignore, ignore, ignore, ignore,
								ignore, ignore) = physicalPathDetails;
						}
						else
						{
...

							{
								...
								tuple<
									int64_t, MMSEngineDBFacade::ContentType,
									string, string, string, int64_t, string,
									string, int64_t>
									mediaItemDetails =
										_mmsEngineDBFacade
											->getMediaItemKeyDetailsByPhysicalPathKey(
								...
								tie(mediaItemKey, ignore, ignore, ignore,
									ignore, ignore, ignore, ignore, ignore) =
									mediaItemDetails;
							}
						}

						httpBody = regex_replace(
							httpBody, regex("\\$\\{mediaItemKey\\}"),
							to_string(mediaItemKey)
						);
						httpBody = regex_replace(
							httpBody, regex("\\$\\{physicalPathKey\\}"),
						...
						userHttpCallback(
							...
							httpPort, httpURI, httpURLParameters, formData,
							httpMethod, callbackTimeoutInSeconds,
							httpHeadersRoot, httpBody, userName, password,
							maxRetries
						);
					}
					catch (runtime_error &e)
						 ...
						 &keyAndDependencyType : dependencies)
				{
					...
					try
					{
						...
						json callbackMedatada;
						{
								...
								Validator::DependencyType::MediaItemKey)
							{
...

								{
									bool warningIfMissing = false;
									tuple<
										MMSEngineDBFacade::ContentType, string,
										string, string, int64_t, int64_t>
										contentTypeTitleUserDataIngestionDateRemovedInAndIngestionJobKey =
											_mmsEngineDBFacade
												->getMediaItemKeyDetails(
													workspace->_workspaceKey,
													mediaItemKey,
													warningIfMissing,
													// 2022-12-18: MIK potrebbe
									...
									string localTitle;
									string userData;
									tie(contentType, localTitle, userData,
...

								{
									...
									bool warningIfMissing = false;
									tuple<
										int64_t, string, int, string, string,
										int64_t, string>
										physicalPathDetails =
											_mmsStorage->getPhysicalPathDetails(
												key, encodingProfileKey,
									...
									tie(physicalPathKey, physicalPath, ignore,
										ignore, fileName, ignore, ignore) =
										physicalPathDetails;

									callbackMedatada["physicalPathKey"] =
										physicalPathKey;
							...
							else
							{
...

								{
									...
									string localTitle;
									string userData;
									tie(mediaItemKey, contentType, localTitle,
										userData, ignore, ignore, ignore,
										ignore, ignore) =
										mediaItemKeyContentTypeTitleUserDataIngestionDateIngestionJobKeyAndFileName;

									callbackMedatada["mediaItemKey"] =
										mediaItemKey;
									callbackMedatada["title"] = localTitle;
...

								{
									int64_t encodingProfileKey = -1;
									tuple<
										string, int, string, string, int64_t,
										string>
										physicalPathDetails =
											_mmsStorage->getPhysicalPathDetails(
												physicalPathKey,
									...
									tie(physicalPath, ignore, ignore, fileName,
										ignore, ignore) = physicalPathDetails;
									...
									MMSEngineDBFacade::ContentType::Audio)
							{
								try
								{
									...
									callbackMedatada["durationInSeconds"] =
										durationInSeconds;
								}
								catch (runtime_error &e)
								{
						...
						userHttpCallback(
							...
							httpPort, httpURI, httpURLParameters, formData,
							httpMethod, callbackTimeoutInSeconds,
							httpHeadersRoot, data, userName, password,
							maxRetries
						);
					}
					catch (runtime_error &e)
		...
		else
		{
			...
			{
				userHttpCallback(
					ingestionJobKey, httpProtocol, httpHostName, httpPort,
					httpURI, httpURLParameters, formData, httpMethod,
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "postOnFacebookThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				if (dependencyType == Validator::DependencyType::MediaItemKey)
				{
					...
					tie(ignore, mmsAssetPathName, ignore, ignore, ignore,
						sizeInBytes, ignore) = physicalPathDetails;
				...
				else
				{
					...
					tie(mmsAssetPathName, ignore, ignore, ignore, sizeInBytes,
						ignore) = physicalPathDetails;
				...
				{
					postVideoOnFacebook(
						mmsAssetPathName, sizeInBytes, ingestionJobKey,
						workspace, facebookConfigurationLabel, facebookNodeType,
						facebookNodeId
					);
				}
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "postOnYouTubeThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		bool youTubeMadeForKids;
		{
			...
			youTubePrivacyStatus =
				JSONUtils::asString(parametersRoot, field, "private");

			field = "madeForKids";
			youTubeMadeForKids =
				JSONUtils::asBool(parametersRoot, field, false);
		}
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				postVideoOnYouTube(
					mmsAssetPathName, sizeInBytes, ingestionJobKey, workspace,
					youTubeConfigurationLabel, youTubeTitle, youTubeDescription,
					youTubeTags, youTubeCategoryId, youTubePrivacyStatus,
					youTubeMadeForKids
				);
			}
			catch (runtime_error &e)
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "changeFileFormatThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				if (dependencyType == Validator::DependencyType::MediaItemKey)
				{
					...
					tie(physicalPathKey, mmsSourceAssetPathName, ignore,
						relativePath, ignore, ignore, ignore) =
						physicalPathDetails;
				}
				else
				{
					...
					tie(mmsSourceAssetPathName, ignore, relativePath, ignore,
						ignore, ignore) = physicalPathDetails;

					bool warningIfMissing = false;
					tuple<
						int64_t, MMSEngineDBFacade::ContentType, string, string,
				...
				vector<tuple<
					int64_t, int, int64_t, int, int, string, string, long,
					string>>
					videoTracks;
				vector<tuple<
					int64_t, int, int64_t, long, string, long, int, string>>
					audioTracks;
				...
				{
					string changeFormatFileName =
						to_string(ingestionJobKey) + "_" +
						to_string(mediaItemKey) + "_changeFileFormat";
					if (outputFileFormat == "m3u8-tar.gz" ||
						outputFileFormat == "m3u8-streaming")
					...
					string stagingChangeFileFormatAssetPathName;
					{
						...
						stagingChangeFileFormatAssetPathName =
							_mmsStorage->getStagingAssetPathName(
					...
					try
					{
						...
						mediaInfoDetails = ffmpeg.getMediaInfo(
							...
							timeoutInSeconds,
							stagingChangeFileFormatAssetPathName, videoTracks,
							audioTracks
						);
					...
					try
					{
						...
						mmsChangeFileFormatAssetPathName =
							_mmsStorage->moveAssetInMMSRepository(
								ingestionJobKey,
								stagingChangeFileFormatAssetPathName,
								workspace->_directoryName, changeFormatFileName,
								relativePath,
					...
					// remove staging directory
					{
						...
						try
						{
							...
							{
								directoryPathName =
									stagingChangeFileFormatAssetPathName.substr(
					...
					try
					{
						int64_t physicalItemRetentionInMinutes = -1;
						{
							string field = "physicalItemRetention";
							if (JSONUtils::isMetadataPresent(
									parametersRoot, field
								))
							{
								string retention = JSONUtils::asString(
									parametersRoot, field, "1d"
								);
								physicalItemRetentionInMinutes =
									MMSEngineDBFacade::parseRetention(retention
									);
						...
						{
							mmsAssetSizeInBytes =
								fs::file_size(mmsChangeFileFormatAssetPathName);
						}
						...
						string externalDeliveryURL;
						int64_t liveRecordingIngestionJobKey = -1;
						int64_t changeFormatPhysicalPathKey =
							_mmsEngineDBFacade->saveVariantContentMetadata(
								workspace->_workspaceKey, ingestionJobKey,
								liveRecordingIngestionJobKey, mediaItemKey,
								externalReadOnlyStorage,
								externalDeliveryTechnology, externalDeliveryURL,
								changeFormatFileName, relativePath,
								mmsPartitionIndexUsed, mmsAssetSizeInBytes,
						...
						SPDLOG_INFO(
							...
							", _ingestionJobKey: " +
							to_string(ingestionJobKey) +
							", changeFormatPhysicalPathKey: " +
							to_string(changeFormatPhysicalPathKey)
						);
					}
					catch (exception &e)
					{
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "generateAndIngestFrameThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		ingestionJobKey
	);
			 ...
			 dependencies)
		{
			...
			try
			{
				...
				if (dependencyType == Validator::DependencyType::MediaItemKey)
				{
					...
					tie(sourcePhysicalPathKey, sourcePhysicalPath, ignore,
				...
				fillGenerateFramesParameters(
					workspace, ingestionJobKey, ingestionType, parametersRoot,
					sourceMediaItemKey, sourcePhysicalPathKey,

					periodInSeconds, startTimeInSeconds, maxFramesNumber,
					videoFilter, mjpeg, imageWidth, imageHeight,
					durationInMilliSeconds
				);

				string fileFormat = "jpg";
				string frameFileName =
					to_string(ingestionJobKey) + "." + fileFormat;
...

				{
					...
					string imageMetaDataContent = generateMediaMetadataToIngest(
						ingestionJobKey, fileFormat, title,
						imageOfVideoMediaItemKey, cutOfVideoMediaItemKey,
						cutOfAudioMediaItemKey, startTimeInSeconds,
						endTimeInSeconds, parametersRoot
					);
	...
	try
	{
		...
		bool oneFramePerSecond;
		{
			...
			if (faceRecognitionOutput == "FrameContainingFace")
			{
				...
				initialFramesNumberToBeSkipped =
					JSONUtils::asInt(parametersRoot, field, 0);

				field = "oneFramePerSecond";
				oneFramePerSecond =
					JSONUtils::asBool(parametersRoot, field, true);
			}
		}

		{
			...
			if (dependencyType == Validator::DependencyType::MediaItemKey)
			{
				...
				tie(sourcePhysicalPathKey, mmsAssetPathName, ignore, ignore,
					ignore, ignore, ignore) =
					physicalPathKeyPhysicalPathFileNameSizeInBytesAndDeliveryFileName;
...

				{
					bool warningIfMissing = false;
					tuple<
						MMSEngineDBFacade::ContentType, string, string, string,
						int64_t, int64_t>
						contentTypeTitleUserDataIngestionDateRemovedInAndIngestionJobKey =
							_mmsEngineDBFacade->getMediaItemKeyDetails(
			...
			else
			{
...

				{
					...
					tie(sourceMediaItemKey, contentType, ignore, ignore, ignore,
						ignore, ignore, ignore, ignore) =
						mediaItemKeyContentTypeTitleUserDataIngestionDateIngestionJobKeyAndFileName;
				}
			}
			...
			_mmsEngineDBFacade->addEncoding_FaceRecognitionJob(
				...
				sourcePhysicalPathKey, mmsAssetPathName,
				faceRecognitionCascadeName, faceRecognitionOutput,
				encodingPriority, initialFramesNumberToBeSkipped,
				oneFramePerSecond
			);
		}
	}
...
)
{
	try
	{
		...
		string deepLearnedModelTagsCommaSeparated;
		{
			...
			deepLearnedModelTagsCommaSeparated =
				JSONUtils::asString(parametersRoot, field, "");
		}

		{
			...
			_mmsEngineDBFacade->addEncoding_FaceIdentificationJob(
				workspace, ingestionJobKey, mmsAssetPathName,
				faceIdentificationCascadeName,
				deepLearnedModelTagsCommaSeparated, encodingPriority
			);
		}
	}
	catch (runtime_error &e)
...
)
{
	try
	{
		...
		{
			{
...

				{
					bool warningIfMissing = false;
					tuple<
						int64_t, string, string, string, string, int64_t,
						string, int, string, int, int, string, int, int, int,
						int, int, int64_t>
						channelConfDetails =
					...
					tie(confKey, streamSourceType, encodersPoolLabel, pullUrl,
						pushProtocol, pushEncoderKey, pushServerName,
						pushServerPort, pushUri, pushListenTimeout,
						captureVideoDeviceNumber, captureVideoInputFormat,
			...
			recordingPeriodStart =
				JSONUtils::asString(recordingPeriodRoot, field, "");
			...
			if (JSONUtils::isMetadataPresent(parametersRoot, field))
			{
				...
				monitorHlsChannelConfigurationLabel =
					JSONUtils::asString(monitorHLSRoot, field, "");
				...
				if (JSONUtils::isMetadataPresent(monitorHLSRoot, keyField))
					monitorEncodingProfileKey =
						JSONUtils::asInt64(monitorHLSRoot, keyField, 0);
				else if (JSONUtils::isMetadataPresent(
							 monitorHLSRoot, labelField
						 ))
				{
					string encodingProfileLabel =
						JSONUtils::asString(monitorHLSRoot, labelField, "");

					MMSEngineDBFacade::ContentType contentType;
					if (JSONUtils::isMetadataPresent(
							monitorHLSRoot, contentTypeField
						))
					{
						...
						monitorEncodingProfileKey =
							_mmsEngineDBFacade->getEncodingProfileKeyByLabel(
			...
			if (JSONUtils::isMetadataPresent(parametersRoot, field))
			{
				...
				field = "hlsChannelConfigurationLabel";
				virtualVODHlsChannelConfigurationLabel =
					JSONUtils::asString(virtualVODRoot, field, "");

				field = "maxDuration";
				liveRecorderVirtualVODMaxDurationInMinutes =
					JSONUtils::asInt(virtualVODRoot, field, 30);
				...
				if (JSONUtils::isMetadataPresent(virtualVODRoot, keyField))
					virtualVODEncodingProfileKey =
						JSONUtils::asInt64(virtualVODRoot, keyField, 0);
				else if (JSONUtils::isMetadataPresent(
							 virtualVODRoot, labelField
						 ))
				{
					string encodingProfileLabel =
						JSONUtils::asString(virtualVODRoot, labelField, "");

					MMSEngineDBFacade::ContentType contentType;
					if (JSONUtils::isMetadataPresent(
							virtualVODRoot, contentTypeField
						))
					{
						contentType = MMSEngineDBFacade::toContentType(
							JSONUtils::asString(
			...
			if (JSONUtils::isMetadataPresent(
					parametersRoot, "framesToBeDetected"
				))
			{
					 ...
					 pictureIndex++)
				{
					...
					if (JSONUtils::isMetadataPresent(
							frameToBeDetectedRoot, "picturePhysicalPathKey"
						))
					{
						int64_t physicalPathKey = JSONUtils::asInt64(
							frameToBeDetectedRoot, "picturePhysicalPathKey", -1
						);
						...
						tie(picturePathName, ignore, ignore, ignore, ignore,
							ignore) = physicalPathDetails;

						bool videoFrameToBeCropped = JSONUtils::asBool(
							frameToBeDetectedRoot, "videoFrameToBeCropped",
							false
						);
						if (videoFrameToBeCropped)
						{
							...
							tuple<int, int, string, int> imageDetails =
								_mmsEngineDBFacade->getImageDetails(
									-1, physicalPathKey,
									// 2022-12-18: MIK potrebbe essere stato
									// appena aggiunto
									true
						...
						frameToBeDetectedRoot["picturePathName"] =
							picturePathName;

						framesToBeDetectedRoot[pictureIndex] =
							frameToBeDetectedRoot;
					}
				}
			}
		}
		...
		time_t utcRecordingPeriodEnd =
			DateTime::sDateSecondsToUtc(recordingPeriodEnd);
		...
		if (streamSourceType == "IP_PULL")
		{
			...
			string youTubePrefix1("https://www.youtube.com/");
			string youTubePrefix2("https://youtu.be/");
			if ((liveURL.size() >= youTubePrefix1.size() &&
			...
			{
				liveURL = _mmsEngineDBFacade->getStreamingYouTubeLiveURL(
					workspace, ingestionJobKey, confKey, liveURL
				);
			}
		}
		...
		else if (streamSourceType == "TV")
		{
			...
			tie(tvType, tvServiceId, tvFrequency, tvSymbolRate, tvBandwidthInHz,
		...
		{
			int encodersNumber =
				_mmsEngineDBFacade->getEncodersNumberByEncodersPool(
		...
		if (monitorHLS || liveRecorderVirtualVOD)
		{
			string monitorVirtualVODHlsChannelConfigurationLabel;
			{
				if (virtualVODHlsChannelConfigurationLabel != "")
					monitorVirtualVODHlsChannelConfigurationLabel =
						virtualVODHlsChannelConfigurationLabel;
				else
					monitorVirtualVODHlsChannelConfigurationLabel =
						monitorHlsChannelConfigurationLabel;
			...
			{
				if (monitorEncodingProfileKey != -1 &&
					virtualVODEncodingProfileKey != -1)
					monitorVirtualVODEncodingProfileKey =
						virtualVODEncodingProfileKey;
				else if (monitorEncodingProfileKey != -1 &&
						 virtualVODEncodingProfileKey == -1)
					monitorVirtualVODEncodingProfileKey =
						monitorEncodingProfileKey;
				else if (monitorEncodingProfileKey == -1 &&
			...
			if (monitorVirtualVODEncodingProfileKey != -1)
			{
				...
				tuple<
					string, MMSEngineDBFacade::ContentType,
		...
		string chunksTranscoderStagingContentsPath;
		{
			...
			string stagingLiveRecordingAssetPathName =
				_mmsStorage->getStagingAssetPathName(
					neededForTranscoder, workspace->_directoryName,
			...
			size_t directoryEndIndex =
				stagingLiveRecordingAssetPathName.find_last_of("/");
			...
			chunksTranscoderStagingContentsPath =
				stagingLiveRecordingAssetPathName.substr(
					0, directoryEndIndex + 1
				);
		}
		...
		string segmentListFileName =
			to_string(ingestionJobKey) + ".liveRecorder.list";

		string recordedFileNamePrefix =
			string("liveRecorder_") + to_string(ingestionJobKey);
		...
		{
			{
				...
				virtualVODStagingContentsPath =
					_mmsStorage->getStagingAssetPathName(
						neededForTranscoder, workspace->_directoryName,
						to_string(ingestionJobKey) +
							"_virtualVOD", // directoryNamePrefix,
						"/",			   // _encodingItem->_relativePath,
		...
		if (liveRecorderVirtualVOD && _liveRecorderVirtualVODImageLabel != "")
		{
			try
			{
				...
				pair<int64_t, MMSEngineDBFacade::ContentType> mediaItemDetails =
					_mmsEngineDBFacade->getMediaItemKeyDetailsByUniqueName(
						workspace->_workspaceKey,
						_liveRecorderVirtualVODImageLabel, warningIfMissing,
						// 2022-12-18: MIK potrebbe essere stato appena aggiunto
						true
					);
		...
		_mmsEngineDBFacade->addEncoding_LiveRecorderJob(
			workspace, ingestionJobKey, ingestionJobLabel, streamSourceType,
			...
			monitorHLS, liveRecorderVirtualVOD,
			monitorVirtualVODOutputRootIndex,

			localOutputsRoot, framesToBeDetectedRoot,

			chunksTranscoderStagingContentsPath, chunksNFSStagingContentsPath,
			segmentListFileName, recordedFileNamePrefix,
			virtualVODStagingContentsPath,
			virtualVODTranscoderStagingContentsPath,
		...
		*/
		...
		{
			{
				...
				useVideoTrackFromMediaItemKey =
					JSONUtils::asInt64(parametersRoot, field, -1);

				{
					...
					tie(ignore, streamSourceType, ignore, ignore, ignore,
			...
			if (JSONUtils::isMetadataPresent(parametersRoot, field))
			{
				...
				if (timePeriod)
				{
					...
					string proxyPeriodStart =
						JSONUtils::asString(proxyPeriodRoot, field, "");
					utcProxyPeriodStart =
						DateTime::sDateSecondsToUtc(proxyPeriodStart);
					...
					string proxyPeriodEnd =
						JSONUtils::asString(proxyPeriodRoot, field, "");
					utcProxyPeriodEnd =
						DateTime::sDateSecondsToUtc(proxyPeriodEnd);
				}
			}
			...
			waitingSecondsBetweenAttemptsInCaseOfErrors =
				JSONUtils::asInt64(parametersRoot, field, 5);
		...
		if (useVideoTrackFromMediaItemKey != -1)
		{
			...
			SPDLOG_INFO(
				...
				", ingestionJobKey: " + to_string(ingestionJobKey) +
				", useVideoTrackFromMediaItemKey: " +
				to_string(useVideoTrackFromMediaItemKey)
			);

			{
				...
				tuple<int64_t, string, int, string, string, int64_t, string>
					physicalPathDetails = _mmsStorage->getPhysicalPathDetails(
						useVideoTrackFromMediaItemKey, encodingProfileKey,
						warningIfMissing,
						// 2022-12-18: MIK potrebbe essere stato appena aggiunto
						true
					);
				tie(sourcePhysicalPathKey, useVideoTrackFromPhysicalPathName,
					ignore, ignore, ignore, ignore, ignore) =
					physicalPathDetails;
			}
			...
			// calculate delivery URL in case of an external encoder
			{
				...
				tie(useVideoTrackFromPhysicalDeliveryURL, ignore) =
		...
		{
			inputsRoot = parametersRoot["internalMMS"]["broadcaster"]
									   ["broadcasterInputsRoot"];
		}
		else
		{
		...
		else
		{
				 ...
				 dependencies)
			{
				...
				if (dependencyType == Validator::DependencyType::MediaItemKey)
				{
					...
					tie(sourcePhysicalPathKey, sourcePhysicalPathName, ignore,
						ignore, ignore, ignore, ignore) = physicalPathDetails;
					...
					tuple<
						...
						mediaItemKeyDetails =
							_mmsEngineDBFacade->getMediaItemKeyDetails(
								workspace->_workspaceKey, sourceMediaItemKey,
								warningIfMissing,
								// 2022-12-18: MIK potrebbe essere stato appena
								// aggiunto
								true
					...
					tie(ignore, mediaItemTitle, ignore, ignore, ignore,
				...
				else
				{
					...
					tie(sourcePhysicalPathName, ignore, ignore, ignore, ignore,
						ignore) = physicalPathDetails;
					...
					tie(sourceMediaItemKey, ignore, mediaItemTitle, ignore,
						ignore, ignore, ignore, ignore, ignore) =
						mediaItemKeyDetails;
				}
				...
				string sourcePhysicalDeliveryURL;
				{
					...
					tie(sourcePhysicalDeliveryURL, ignore) =
						deliveryAuthorizationDetails;
				}

				sources.push_back(make_tuple(
					sourcePhysicalPathKey, mediaItemTitle,
	...
	try
	{
		...
		int64_t sourcePhysicalPathKey;
		{
			...
			if (dependencyType == Validator::DependencyType::MediaItemKey)
			{
				...
				bool warningIfMissing = false;
				tuple<int64_t, string, int, string, string, int64_t, string>
					physicalPath = _mmsStorage->getPhysicalPathDetails(
						key, encodingProfileKey, warningIfMissing,
						// 2022-12-18: MIK potrebbe essere stato appena aggiunto
						true
					);
				tie(sourcePhysicalPathKey, mmsSourceVideoAssetPathName, ignore,
					ignore, ignore, ignore, ignore) = physicalPath;
			...
			else
			{
				...
				tuple<
					...
					mediaItemKeyContentTypeTitleUserDataIngestionDateIngestionJobKeyAndFileName =
						_mmsEngineDBFacade
							->getMediaItemKeyDetailsByPhysicalPathKey(
								workspace->_workspaceKey, sourcePhysicalPathKey,
								warningIfMissing,
								// 2022-12-18: MIK potrebbe essere stato appena
								// aggiunto
								true
				...
				tie(sourceMediaItemKey, localContentType, localTitle, userData,
					ingestionDate, localIngestionJobKey, ignore, ignore, ignore
				) = mediaItemKeyContentTypeTitleUserDataIngestionDateIngestionJobKeyAndFileName;
			}
			...
			// calculate delivery URL in case of an external encoder
			{
				...
				tie(mmsSourceVideoAssetDeliveryURL, ignore) =
					deliveryAuthorizationDetails;
			}
		}
	...
	for (int outputIndex = 0; outputIndex < outputsRoot.size(); outputIndex++)
	{
		...
		if (outputType == "CDN_AWS")
		{
			...
			awsChannelConfigurationLabel =
				JSONUtils::asString(outputRoot, field, "");

			field = "awsSignedURL";
			awsSignedURL = JSONUtils::asBool(outputRoot, field, false);

			field = "awsExpirationInMinutes";
			awsExpirationInMinutes =
				JSONUtils::asInt(outputRoot, field, 1440); // 1 day
		}
		...
		{
			string encodingProfileLabel =
				JSONUtils::asString(outputRoot, labelField, "");
	...
	try
	{
		...
		*/
		...
		json outputsRoot;
		{
			...
			{
				string configurationLabel =
					JSONUtils::asString(inputChannelsRoot[inputChannelIndex]);

				bool warningIfMissing = false;
				tuple<
					int64_t, string, string, string, string, int64_t, string,
					int, string, int, int, string, int, int, int, int, int,
					int64_t>
				...
				string streamSourceType;
				string liveURL;
				tie(confKey, streamSourceType, ignore, liveURL, ignore, ignore,
					ignore, ignore, ignore, ignore, ignore, ignore, ignore,
					ignore, ignore, ignore, ignore, ignore) =
					confKeyAndChannelURL;
	...
	try
	{
		...
		bool errorIfAChunkIsMissing = false;
		{
			...
			*/
			...
			recordingCode =
				JSONUtils::asInt64(liveCutParametersRoot, field, -1);

			field = "maxWaitingForLastChunkInSeconds";
			maxWaitingForLastChunkInSeconds =
				JSONUtils::asInt64(liveCutParametersRoot, field, 90);

			field = "errorIfAChunkIsMissing";
			errorIfAChunkIsMissing =
			...
			cutPeriodStartTimeInMilliSeconds =
				JSONUtils::asString(cutPeriodRoot, field, "");
		...
		int64_t utcCutPeriodStartTimeInMilliSeconds =
			DateTime::sDateMilliSecondsToUtc(cutPeriodStartTimeInMilliSeconds);

		// next code is the same in the Validator class
		int64_t utcCutPeriodEndTimeInMilliSeconds =
			DateTime::sDateMilliSecondsToUtc(cutPeriodEndTimeInMilliSeconds);
		...
		*/
		...
		chrono::system_clock::time_point startLookingForChunks =
			chrono::system_clock::now();
		...
		while (!lastRequestedChunk &&
			   (chrono::duration_cast<chrono::seconds>(
					chrono::system_clock::now() - startLookingForChunks
				)
					.count() < maxWaitingForLastChunkInSeconds))
		{
			...
			int rows =
				60 *
				1; // assuming every MediaItem is one minute, let's take 1 hour
			...
			string jsonOrderBy =
				"JSON_EXTRACT(userData, '$.mmsData.utcChunkStartTime') asc";
			...
			do
			{
					 ...
					 mediaItemIndex < mediaItemsRoot.size(); mediaItemIndex++)
				{
					...
					json userDataRoot;
					{
						field = "userData";
						string userData =
							JSONUtils::asString(mediaItemRoot, field, "");
						if (userData == "")
						{
							string errorMessage =
								string() +
					...
					int64_t currentUtcChunkStartTime =
						JSONUtils::asInt64(mmsDataRoot, field, 0);
					...
					SPDLOG_INFO(
						...
						", ingestionJobKey: " + to_string(ingestionJobKey) +
						", mediaITemKey: " + to_string(mediaItemKey) +
						", currentUtcChunkStartTime: " +
						to_string(currentUtcChunkStartTime) + " (" +
						currentChunkStartTime + ")" +
						", currentUtcChunkEndTime: " +
						to_string(currentUtcChunkEndTime) + " (" +
						currentChunkEndTime + ")"
					);

					// check if it is the next chunk
					if (utcPreviousUtcChunkEndTime != -1 &&
						utcPreviousUtcChunkEndTime != currentUtcChunkStartTime)
					{
						string previousUtcChunkEndTime;
						{
							...
							localtime_r(
								&utcPreviousUtcChunkEndTime, &tmDateTime
							);

							sprintf(
								strDateTime, "%04d-%02d-%02d %02d:%02d:%02d",
								tmDateTime.tm_year + 1900,
								tmDateTime.tm_mon + 1, tmDateTime.tm_mday,
						...
						string errorMessage =
							string("#Chunks check. Next chunk was not found") +
							", _processorIdentifier: " +
							to_string(_processorIdentifier) +
							", ingestionJobKey: " + to_string(ingestionJobKey) +
							", utcPreviousUtcChunkEndTime: " +
							to_string(utcPreviousUtcChunkEndTime) + " (" +
							...
							to_string(utcCutPeriodEndTimeInMilliSeconds) +
							" (" + cutPeriodEndTimeInMilliSeconds + ")";
					...
					if (firstRetrievedChunk)
					{
						...
						if (!(currentUtcChunkStartTime * 1000 <=
								  utcCutPeriodStartTimeInMilliSeconds &&
							  utcCutPeriodStartTimeInMilliSeconds <
								  currentUtcChunkEndTime * 1000))
						{
							firstRequestedChunk = false;
							...
							string errorMessage =
								string(
									"#Chunks check. First chunk was not found"
								) +
								", _processorIdentifier: " +
								to_string(_processorIdentifier) +
								", ingestionJobKey: " +
				...
				SPDLOG_INFO(
					...
					", rows: " + to_string(rows) + ", mediaItemsRoot.size: " +
					to_string(mediaItemsRoot.size()) +
			...
			if (!lastRequestedChunk)
			{
						...
						.count() < maxWaitingForLastChunkInSeconds)
				{
					...
					this_thread::sleep_for(
						chrono::seconds(secondsToWaitLastChunk)
					);
				}
			}
		}
		...
		{
			string errorMessage =
				string("#Chunks check. Chunks not available") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
		...
		string workflowMetadata;
		{
			...
			json concatDemuxerParametersRoot;
			{
				...
				concatDemuxerRoot[field] =
					"Concat from " + to_string(utcFirstChunkStartTime) + " (" +
					firstChunkStartTime + ") to " +
			...
			json cutRoot;
			{
				...
				field = "retention";
				cutParametersRoot[field] =
					JSONUtils::asString(liveCutParametersRoot, field, "");
	...
	try
	{
			...
			*/
			...
			do
			{
					 ...
					 mediaItemIndex < mediaItemsRoot.size(); mediaItemIndex++)
				{
					...
					int64_t currentUtcChunkEndTimeInMilliSecs =
						JSONUtils::asInt64(mmsDataRoot, field, 0);
					...
					string currentChunkEndTime;
					{
						...
						int64_t currentUtcChunkStartTime =
							currentUtcChunkStartTimeInMilliSecs / 1000;
						localtime_r(&currentUtcChunkStartTime, &tmDateTime);
						sprintf(
							strDateTime, "%04d-%02d-%02d %02d:%02d:%02d.%03d",
							tmDateTime.tm_year + 1900, tmDateTime.tm_mon + 1,
							tmDateTime.tm_mday, tmDateTime.tm_hour,
							tmDateTime.tm_min, tmDateTime.tm_sec,
						...
						int64_t currentUtcChunkEndTime =
							currentUtcChunkEndTimeInMilliSecs / 1000;
						localtime_r(&currentUtcChunkEndTime, &tmDateTime);
						sprintf(
							strDateTime, "%04d-%02d-%02d %02d:%02d:%02d.%03d",
							tmDateTime.tm_year + 1900, tmDateTime.tm_mon + 1,
					...
					SPDLOG_INFO(
						...
						to_string(currentUtcChunkStartTimeInMilliSecs) + " (" +
						currentChunkStartTime + ")" +
						", currentUtcChunkEndTimeInMilliSecs: " +
						to_string(currentUtcChunkEndTimeInMilliSecs) + " (" +
						currentChunkEndTime + ")"
					);

					// check if it is the next chunk
					if (utcPreviousUtcChunkEndTimeInMilliSecs != -1 &&
							...
							currentUtcChunkStartTimeInMilliSecs)
					{
						string previousUtcChunkEndTime;
						{
							...
							int64_t utcPreviousUtcChunkEndTime =
								utcPreviousUtcChunkEndTimeInMilliSecs / 1000;
							localtime_r(
								&utcPreviousUtcChunkEndTime, &tmDateTime
							);
							...
							sprintf(
								...
								tmDateTime.tm_hour, tmDateTime.tm_min,
								tmDateTime.tm_sec,
								(int)(utcPreviousUtcChunkEndTimeInMilliSecs %
									  1000)
							);
							previousUtcChunkEndTime = strDateTime;
						}
						...
						// it is not the next chunk
						string errorMessage =
							string("Next chunk was not found") +
							", _processorIdentifier: " +
							to_string(_processorIdentifier) +
							", ingestionJobKey: " + to_string(ingestionJobKey) +
							", utcPreviousUtcChunkEndTimeInMilliSecs: " +
							to_string(utcPreviousUtcChunkEndTimeInMilliSecs) +
							" (" + previousUtcChunkEndTime + ")" +
							", currentUtcChunkStartTimeInMilliSecs: " +
							to_string(currentUtcChunkStartTimeInMilliSecs) +
							" (" + currentChunkStartTime + ")" +
					...
					if (firstRetrievedChunk)
					{
						...
						if (!(currentUtcChunkStartTimeInMilliSecs <=
								  utcCutPeriodStartTimeInMilliSeconds &&
							  utcCutPeriodStartTimeInMilliSeconds <
								  currentUtcChunkEndTimeInMilliSecs))
						{
							...
							// it is not the first chunk
							string errorMessage =
								string("First chunk was not found") +
								", _processorIdentifier: " +
								to_string(_processorIdentifier) +
								", ingestionJobKey: " +
								to_string(ingestionJobKey) +
...

					{
						...
						{
							field = "encodingProfileKey";
							mediaItemKeyReferenceRoot[field] =
								chunkEncodingProfileKey;
						}
...

					{
						...
						{
							lastRequestedChunk = true;
							utcLastChunkEndTimeInMilliSecs =
								currentUtcChunkEndTimeInMilliSecs;
							lastChunkEndTime = currentChunkEndTime;
						}
					}
		...
		string cutLabel;
		{
			...
			json concatDemuxerParametersRoot;
			{
				...
				concatDemuxerRoot[field] =
					"Concat from " +
					to_string(utcFirstChunkStartTimeInMilliSecs) + " (" +
					firstChunkStartTime + ") to " +
					to_string(utcLastChunkEndTimeInMilliSecs) + " (" +
					lastChunkEndTime + ")";
		...
		*/
		{
			...
			{
				if (!JSONUtils::isMetadataPresent(
						workflowResponseRoot, "tasks"
					))
				{
					string errorMessage =
						string("LiveCut workflow ingestion: wrong response, "
							   "tasks not found") +
						", _processorIdentifier: " +
						to_string(_processorIdentifier) +
						", ingestionJobKey: " + to_string(ingestionJobKey) +
					 ...
					 taskIndex++)
				{
					...
					{
						cutIngestionJobKey =
							JSONUtils::asInt64(taskRoot, "ingestionJobKey", -1);
			...
			_mmsEngineDBFacade->changeIngestionJobDependency(
...
)
{
	try
	{
		...
		json referencesRoot;
		{
			...
			youTubeLiveBroadcastTitle =
				JSONUtils::asString(parametersRoot, field, "");

			field = "MadeForKids";
			youTubeLiveBroadcastMadeForKids =
				JSONUtils::asBool(parametersRoot, "MadeForKids", true);

			field = "Description";
			youTubeLiveBroadcastDescription =
				JSONUtils::asString(parametersRoot, field, "");

			field = "PrivacyStatus";
			youTubeLiveBroadcastPrivacyStatus =
				JSONUtils::asString(parametersRoot, field, "unlisted");

			field = "LatencyPreference";
			youTubeLiveBroadcastLatencyPreference =
				JSONUtils::asString(parametersRoot, field, "normal");
			...
			scheduleStartTimeInSeconds =
				JSONUtils::asString(scheduleRoot, field, "");
		...
		string youTubeAccessToken = getYouTubeAccessTokenByConfigurationLabel(
			ingestionJobKey, workspace, youTubeConfigurationLabel
		);
...

				{
					...
					// scheduledStartTime
					{
						int64_t utcScheduleStartTimeInSeconds =
							DateTime::sDateSecondsToUtc(
								scheduleStartTimeInSeconds
							);

						// format: YYYY-MM-DDTHH:MI:SS.000Z
						string scheduleStartTimeInMilliSeconds =
							scheduleStartTimeInSeconds;
					...
					// scheduledEndTime
					{
						...
						// format: YYYY-MM-DDTHH:MI:SS.000Z
						string scheduleEndTimeInMilliSeconds =
							scheduleEndTimeInSeconds;
						scheduleEndTimeInMilliSeconds.insert(
							scheduleEndTimeInSeconds.length() - 1, ".000", 4
						);
		...
		{
			string errorMessage =
				string("YouTube live broadcast management failed") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
		...
		try
		{
			...
			*/
			...
			string streamName =
				JSONUtils::asString(ingestionInfoRoot, field, "");
		...
		{
			string errorMessage =
				string("YouTube live stream management failed") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
				", youTubeURL: " + youTubeURL + ", sResponse: " + sResponse +
				", e.what(): " + e.what();
		...
		try
		{
			...
			* curl -v --request POST \
			...
			\
				...
				--compressed
			*/
			string youTubeDataAPILiveBroadcastBindURI = regex_replace(
				_youTubeDataAPILiveBroadcastBindURI, regex("__BROADCASTID__"),
				broadcastId
			);
			youTubeDataAPILiveBroadcastBindURI = regex_replace(
				youTubeDataAPILiveBroadcastBindURI, regex("__STREAMID__"),
				streamId
			);
				...
				json liveProxyParametersRoot;
				{
					...
					json outputsRoot = json::array();
					{
						...
						try
						{
							bool warningIfMissing = true;
							tuple<
								int64_t, string, string, string, string, string>
								rtmpChannelDetails =
									_mmsEngineDBFacade->getRTMPChannelDetails(
										workspace->_workspaceKey,
										youTubeConfigurationLabel,
										warningIfMissing
									);
							...
							int64_t confKey;
							tie(confKey, ignore, ignore, ignore, ignore,
								ignore) = rtmpChannelDetails;

							_mmsEngineDBFacade->modifyRTMPChannelConf(
								confKey, workspace->_workspaceKey,
								youTubeConfigurationLabel, rtmpURL, "", "", "",
								"", "DEDICATED"
							);
						}
						catch (ConfKeyNotFound &e)
...
)
{
	try
	{
		...
		if (facebookNodeType == "Page")
			facebookToken = getFacebookPageToken(
				ingestionJobKey, workspace, facebookConfigurationLabel,
				facebookNodeId
			);
		...
		*/
		...
		try
		{
			...
			*/
			facebookURL = _facebookGraphAPIProtocol + "://" +
						  ...
						  regex_replace(
							  _facebookGraphAPILiveVideosURI,
							  regex("__NODEID__"), facebookNodeId
						  ) +
						  "?title=" + curlpp::escape(title) +
						  (description != ""
							   ? ("&description=" + curlpp::escape(description))
							   : "") +
		...
		{
			string errorMessage =
				string("Facebook live broadcast management failed") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
				", facebookURL: " + facebookURL +
				", response: " + JSONUtils::toString(responseRoot) +
				...
				json liveProxyParametersRoot;
				{
					...
					json outputsRoot = json::array();
					{
						...
						try
						{
							...
							_mmsEngineDBFacade->modifyRTMPChannelConf(
								confKey, workspace->_workspaceKey,
								facebookConfigurationLabel, rtmpURL, "", "", "",
								"", "DEDICATED"
							);
						}
						catch (ConfKeyNotFound &e)
...
void MMSEngineProcessor::copyContent(
	int64_t ingestionJobKey, string mmsAssetPathName, string localPath,
	string localFileName
)
{
	...
	try
	{
		...
		if (localFileName != "")
		{
			string cleanedFileName;
			{
				...
				transform(
					localFileName.begin(), localFileName.end(),
					cleanedFileName.begin(),
					[](unsigned char c)
					{
						if (c == '/')
							return (int)' ';
				...
				if (cleanedFileName.size() >= suffix.size() &&
					0 == cleanedFileName.compare(
							 cleanedFileName.size() - suffix.size(),
							 suffix.size(), suffix
						 ))
					;
				else
...
)
{
	...
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "handleMultiLocalAssetIngestionEventThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		multiLocalAssetIngestionEvent.getIngestionJobKey()
	);
	...
	try
	{
		...
		{
			string generatedFrames_BaseFileName =
				to_string(multiLocalAssetIngestionEvent.getIngestionJobKey());

			for (fs::directory_entry const &entry :
				 fs::directory_iterator(workspaceIngestionRepository))
			{
				try
				{
					if (!entry.is_regular_file())
					...
					if (entry.path().filename().string().size() >=
							generatedFrames_BaseFileName.size() &&
						0 == entry.path().filename().string().compare(
								 0, generatedFrames_BaseFileName.size(),
								 generatedFrames_BaseFileName
							 ))
						generatedFramesFileNames.push_back(
			 ...
			 it != generatedFramesFileNames.end(); ++it)
		{
			...
			else
			{
				...
				size_t extensionIndex =
					generatedFrameFileName.find_last_of(".");
				if (extensionIndex == string::npos)
				{
					string errorMessage =
						string() +
				...
				string imageMetaDataContent = generateMediaMetadataToIngest(
					multiLocalAssetIngestionEvent.getIngestionJobKey(),
					// mjpeg,
					fileFormat, title, imageOfVideoMediaItemKey,
...

				{
					...
					localAssetIngestionEvent
						->setIngestionRowToBeUpdatedAsSuccess(
							it + 1 == generatedFramesFileNames.end() ? true
																	 : false
						);
...
)
{
	try
	{
		...
		bool stopIfReferenceProcessingError;
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			dependencyInfo = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[0]
			);
		tie(sourceMediaItemKey, sourcePhysicalPathKey, referenceContentType,
			sourceAssetPathName, sourceRelativePath, sourceFileName,
			sourceFileExtension, sourceDurationInMilliSecs,
		...
		fillGenerateFramesParameters(
			...
			periodInSeconds, startTimeInSeconds, maxFramesNumber, videoFilter,
			mjpeg, imageWidth, imageHeight, durationInMilliSeconds
		);
												 ...
												 // encoder
		{
			...
			string directoryNameForFrames = to_string(ingestionJobKey)
				...
				;
			transcoderStagingImagesDirectory =
				_mmsStorage->getStagingAssetPathName(
					neededForTranscoder,
		...
		_mmsEngineDBFacade->addEncoding_GenerateFramesJob(
			...
			sourceTranscoderStagingAssetPathName, // used in case of external
												  // encoder
			sourceAssetPathName, sourcePhysicalPathKey, sourceFileExtension,
			sourceFileName, durationInMilliSeconds, startTimeInSeconds,
			maxFramesNumber, videoFilter, periodInSeconds, mjpeg, imageWidth,
			imageHeight, _mmsWorkflowIngestionURL, _mmsBinaryIngestionURL,
			_mmsIngestionURL
		);
	}
	catch (runtime_error &e)
...
void MMSEngineProcessor::fillGenerateFramesParameters(
	...
	string &videoFilter, bool &mjpeg, int &imageWidth, int &imageHeight,
	int64_t &durationInMilliSeconds
)
{
	try
	{
		...
		periodInSeconds = -1;
		{
			if (ingestionType == MMSEngineDBFacade::IngestionType::Frame)
			{
					 ...
					 ingestionType == MMSEngineDBFacade::IngestionType::
										  MotionJPEGByPeriodicalFrames)
			{
		...
		startTimeInSeconds = 0;
		{
			if (ingestionType == MMSEngineDBFacade::IngestionType::Frame)
			{
				...
				startTimeInSeconds =
					JSONUtils::asDouble(parametersRoot, field, 0);
			}
			else if (ingestionType ==
						 MMSEngineDBFacade::IngestionType::PeriodicalFrames ||
					 ingestionType ==
		...
		if (JSONUtils::isMetadataPresent(
			...
			fixStartTimeIfOvercomeDuration = JSONUtils::asBool(
				parametersRoot, "fixInstantInSecondsIfOvercomeDuration", true
			);
		else if (JSONUtils::isMetadataPresent(
					 parametersRoot, "FixStartTimeIfOvercomeDuration"
				 ))
			fixStartTimeIfOvercomeDuration = JSONUtils::asBool(
				parametersRoot, "FixStartTimeIfOvercomeDuration", true
			);
		...
		{
			if (ingestionType == MMSEngineDBFacade::IngestionType::
									 MotionJPEGByPeriodicalFrames ||
				ingestionType ==
					MMSEngineDBFacade::IngestionType::MotionJPEGByIFrames)
			{
		...
		{
			vector<tuple<
				int64_t, int, int64_t, int, int, string, string, long, string>>
				videoTracks;
			vector<
				tuple<int64_t, int, int64_t, long, string, long, int, string>>
				audioTracks;
			...
			tuple<int64_t, int, int64_t, int, int, string, string, long, string>
				videoTrack = videoTracks[0];

			tie(ignore, ignore, durationInMilliSeconds, videoWidth, videoHeight,
				ignore, ignore, ignore, ignore) = videoTrack;
		...
		if (durationInMilliSeconds < startTimeInSeconds * 1000)
		{
			if (fixStartTimeIfOvercomeDuration)
			{
				...
				SPDLOG_INFO(
					...
					to_string(fixStartTimeIfOvercomeDuration) +
					", previousStartTimeInSeconds: " +
					to_string(previousStartTimeInSeconds) +
...
)
{
	try
	{
		...
		json encodingProfileDetailsRoot;
		{
			...
			{
				encodingProfileKey =
					JSONUtils::asInt64(parametersRoot, keyField, 0);
			}
			else if (JSONUtils::isMetadataPresent(parametersRoot, labelField))
			{
				string encodingProfileLabel =
					JSONUtils::asString(parametersRoot, labelField, "");
			 ...
			 dependencies)
		{
			...
			bool stopIfReferenceProcessingError;
			tuple<
				int64_t, int64_t, MMSEngineDBFacade::ContentType, string,
				string, string, string, int64_t, string, string, bool>
				dependencyInfo = processDependencyInfo(
					workspace, ingestionJobKey, keyAndDependencyType
				);
			tie(sourceMediaItemKey, sourcePhysicalPathKey, referenceContentType,
				sourceAssetPathName, sourceRelativePath, sourceFileName,
				sourceFileExtension, sourceDurationInMilliSecs,
			...
			if (referenceContentType == MMSEngineDBFacade::ContentType::Image)
			{
				...
				imageRoot["sourcePhysicalDeliveryURL"] =
					 ...
					 MMSEngineDBFacade::ContentType::Audio)
			{
				...
				audioRoot["sourcePhysicalDeliveryURL"] =
					sourcePhysicalDeliveryURL;
				...
				if (shortestAudioDurationInSeconds == -1.0 ||
					shortestAudioDurationInSeconds >
						sourceDurationInMilliSecs / 1000)
					shortestAudioDurationInSeconds =
						sourceDurationInMilliSecs / 1000;
			}
			else
		...
		_mmsEngineDBFacade->addEncoding_SlideShowJob(
			workspace, ingestionJobKey, encodingProfileKey,
			encodingProfileDetailsRoot, targetFileFormat, imagesRoot,
			audiosRoot, shortestAudioDurationInSeconds,
			encodedTranscoderStagingAssetPathName,
			encodedNFSStagingAssetPathName.string(), _mmsWorkflowIngestionURL,
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "manageConcatThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
			 ...
			 dependencies)
		{
			...
			if (dependencyType == Validator::DependencyType::MediaItemKey)
			{
				...
				tie(sourcePhysicalPathKey, sourcePhysicalPath, ignore, ignore,
					ignore, ignore, ignore) =
					physicalPathKeyPhysicalPathFileNameSizeInBytesAndDeliveryFileName;
			...
			bool warningIfMissing = false;
			tuple<
				int64_t, MMSEngineDBFacade::ContentType, string, string, string,
				int64_t, string, string, int64_t>
				mediaItemKeyDetails =
			...
			MMSEngineDBFacade::ContentType contentType;
			{
				...
				string ingestionDate;
				int64_t localIngestionJobKey;
				tie(localMediaItemKey, contentType, localTitle, lastUserData,
					ingestionDate, localIngestionJobKey, ignore, ignore,
					ignore) = mediaItemKeyDetails;
			}
			...
			if (firstMedia)
			{
				...
				if (lastUserData != "")
				{
					...
					if (JSONUtils::isMetadataPresent(sourceUserDataRoot, field))
					{
						...
						if (JSONUtils::isMetadataPresent(
								sourceMmsDataRoot, utcStartTimeInMilliSecsField
							))
							utcStartTimeInMilliSecs = JSONUtils::asInt64(
								sourceMmsDataRoot, utcStartTimeInMilliSecsField,
								0
							);
						/*
				...
				forcedAvgFrameRate == "")
			{
				...
				*/
				...
				tie(ignore, ignore, ignore, ignore, ignore, forcedAvgFrameRate,
					ignore, ignore, ignore) = videoTrack;
			}
		}
		...
		if (utcStartTimeInMilliSecs != -1)
		{
			if (lastUserData != "")
			{
				...
				if (JSONUtils::isMetadataPresent(sourceUserDataRoot, field))
				{
					...
					if (JSONUtils::isMetadataPresent(
							sourceMmsDataRoot, utcEndTimeInMilliSecsField
						))
						utcEndTimeInMilliSecs = JSONUtils::asInt64(
							sourceMmsDataRoot, utcEndTimeInMilliSecsField, 0
						);
					/*
					else if (JSONUtils::isMetadataPresent(sourceMmsDataRoot,
		...
		string localSourceFileName = to_string(ingestionJobKey) + "_concat" +
									 "." + fileFormat // + "_source"
			;
		...
		if (sourcePhysicalPaths.size() == 1)
		{
			...
			fs::copy(
				sourcePhysicalPath, concatenatedMediaPathName,
		...
		if (JSONUtils::isMetadataPresent(parametersRoot, field))
		{
			maxDurationInSeconds =
				JSONUtils::asDouble(parametersRoot, field, 0.0);

			field = "ExtraSecondsToCutWhenMaxDurationIsReached";
			if (JSONUtils::isMetadataPresent(parametersRoot, field))
			{
				extraSecondsToCutWhenMaxDurationIsReached =
					JSONUtils::asDouble(parametersRoot, field, 0.0);

				if (extraSecondsToCutWhenMaxDurationIsReached >=
					abs(maxDurationInSeconds))
		...
		SPDLOG_INFO(
			...
			", extraSecondsToCutWhenMaxDurationIsReached: " +
			to_string(extraSecondsToCutWhenMaxDurationIsReached)
		);
		if (maxDurationInSeconds != 0.0)
		{
			...
			if (durationInMilliSeconds > abs(maxDurationInSeconds) * 1000)
			{
				...
				string cutMediaPathName =
					workspaceIngestionRepository + "/" + localCutSourceFileName;
				...
				SPDLOG_INFO(
					...
					concatenatedMediaPathName + ", cutType: " + cutType +
					", startTimeInSeconds: " + to_string(startTimeInSeconds) +
				...
				ffmpeg.cutWithoutEncoding(
					ingestionJobKey, concatenatedMediaPathName,
					concatContentType == MMSEngineDBFacade::ContentType::Video
						? true
						: false,
					cutType,
					...
					to_string(startTimeInSeconds), to_string(endTimeInSeconds),
...

		{
...

			{
				...
				if (forcedAvgFrameRate != "" &&
					concatContentType == MMSEngineDBFacade::ContentType::Video)
					localAssetIngestionEvent->setForcedAvgFrameRate(
						forcedAvgFrameRate
					);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "manageCutMediaThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		string userData;
		{
			...
			tuple<
				MMSEngineDBFacade::ContentType, string, string, string, int64_t,
				int64_t>
			...
			tie(ignore, ignore, userData, ignore, ignore, ignore) =
				mediaItemDetails;
		}
		...
		int framesPerSecond = -1;
		{
			try
			{
					...
					MMSEngineDBFacade::ContentType::Video)
				{
					...
					tie(ignore, ignore, ignore, ignore, ignore,
						forcedAvgFrameRate, ignore, ignore, ignore) =
						videoTrack;
		...
		string endTime = "0.0";
		{
			...
			// di secondo>.
			{
					...
					) == 3)
				{
					...
					SPDLOG_INFO(
						...
						", startTime: {}"
						", newStartTime: {}",
						_processorIdentifier, ingestionJobKey, startTime,
						newStartTime
					);
					...
					) == 3)
				{
					...
					SPDLOG_INFO(
						...
						", endTime: {}"
						", newEndTime: {}",
						_processorIdentifier, ingestionJobKey, endTime,
						newEndTime
					);
					endTime = newEndTime;
				}
			...
			if (JSONUtils::isMetadataPresent(parametersRoot, field))
			{
				...
				if (timeCode == "")
				{
					string errorMessage = fmt::format(
						...
						ingestionJobKey, sourcePhysicalPathKey, metaData,
						timesRelativeToMetaDataField
					);
					SPDLOG_ERROR(errorMessage);
					...
					) == 3)
				{
					...
					SPDLOG_INFO(
						...
						", timeCode: {}"
						", newTimeCode: {}",
						_processorIdentifier, ingestionJobKey, timeCode,
						newTimeCode
					);
					timeCode = newTimeCode;
				}

				long startTimeInCentsOfSeconds =
					FFMpeg::timeToSeconds(ingestionJobKey, startTime).second;
				long endTimeInCentsOfSeconds =
					FFMpeg::timeToSeconds(ingestionJobKey, endTime).second;
				long relativeTimeInCentsOfSeconds =
					FFMpeg::timeToSeconds(ingestionJobKey, timeCode).second;

				long newStartTimeInCentsOfSeconds =
					startTimeInCentsOfSeconds - relativeTimeInCentsOfSeconds;
				string newStartTime = FFMpeg::centsOfSecondsToTime(
					ingestionJobKey, newStartTimeInCentsOfSeconds
				);

				long newEndTimeInCentsOfSeconds =
					endTimeInCentsOfSeconds - relativeTimeInCentsOfSeconds;
				string newEndTime = FFMpeg::centsOfSecondsToTime(
					ingestionJobKey, newEndTimeInCentsOfSeconds
				);
				...
				SPDLOG_INFO(
					...
					", newEndTime: {}",
					_processorIdentifier, ingestionJobKey, timeCode,
					relativeTimeInCentsOfSeconds, startTimeInCentsOfSeconds,
					startTime, newStartTime, endTimeInCentsOfSeconds, endTime,
					newEndTime
				);
			...
			if (startTimeInSeconds < 0.0)
			{
				...
				SPDLOG_INFO(
					...
					", ingestionJobKey: " + to_string(ingestionJobKey) +
					", fixEndTimeIfOvercomeDuration: " +
					to_string(fixEndTimeIfOvercomeDuration) +
					", video sourceMediaItemKey: " +
					to_string(sourceMediaItemKey) +
					", previousStartTimeInSeconds: " +
					to_string(startTimeInSeconds) + ", new startTime: " +
					startTime + ", sourceDurationInMilliSecs (input media): " +
					to_string(sourceDurationInMilliSecs)
				);
			...
			if (framesNumber == -1 && userData != "")
			{
				...
				if (JSONUtils::isMetadataPresent(sourceUserDataRoot, field))
				{
					...
					if (JSONUtils::isMetadataPresent(
						...
						utcStartTimeInMilliSecs = JSONUtils::asInt64(
							sourceMmsDataRoot, utcStartTimeInMilliSecsField, 0
						);
					/*
					else if (JSONUtils::isMetadataPresent(sourceMmsDataRoot,
					utcChunkStartTimeField))
					...
					*/
					...
					if (utcStartTimeInMilliSecs != -1)
					{
						string utcEndTimeInMilliSecsField =
							"utcEndTimeInMilliSecs";
						// string utcChunkEndTimeField = "utcChunkEndTime";
						if (JSONUtils::isMetadataPresent(
								sourceMmsDataRoot, utcEndTimeInMilliSecsField
						...
						*/
						...
						if (utcStartTimeInMilliSecs != -1 &&
							utcEndTimeInMilliSecs != -1)
						{
							newUtcStartTimeInMilliSecs =
								utcStartTimeInMilliSecs;
							newUtcStartTimeInMilliSecs +=
								((int64_t)(FFMpeg::timeToSeconds(
											   ingestionJobKey, startTime
										   )
											   .second *
										   10));
		...
		SPDLOG_INFO(
			...
			", sourceDurationInMilliSecs: " +
			to_string(sourceDurationInMilliSecs) + ", framesNumber: " +
			to_string(framesNumber) + ", startTime: " + startTime +
			", endTime: " + endTime + ", newUtcStartTimeInMilliSecs: " +
			to_string(newUtcStartTimeInMilliSecs) +
			...
			cutType == "KeyFrameSeekingInterval")
		{
			...
			if (outputFileFormat == "")
			{
				string sourceFileExtensionWithoutDot =
					sourceFileExtension.size() > 0
						? sourceFileExtension.substr(1)
						: sourceFileExtension;
				...
				newUtcEndTimeInMilliSecs != -1)
			{
				...
				{
					time_t newUtcStartTimeInSecs =
						newUtcStartTimeInMilliSecs / 1000;
					// i.e.: 2021-02-26T15:41:15Z
					string utcToUtcString =
						DateTime::utcToUtcString(newUtcStartTimeInSecs);
					utcToUtcString.insert(
						utcToUtcString.size() - 1,
						"." + to_string(newUtcStartTimeInMilliSecs % 1000)
					);
				...
				{
					time_t newUtcEndTimeInSecs =
						newUtcEndTimeInMilliSecs / 1000;
					// i.e.: 2021-02-26T15:41:15Z
					string utcToUtcString =
						DateTime::utcToUtcString(newUtcEndTimeInSecs);
					utcToUtcString.insert(
						utcToUtcString.size() - 1,
						"." + to_string(newUtcEndTimeInMilliSecs % 1000)
					);
					destMmsDataRoot[field] = utcToUtcString;
				}
			...
			string localSourceFileName = to_string(ingestionJobKey) + "_cut" +
										 "." + fileFormat // + "_source"
				;
			...
			string mediaMetaDataContent = generateMediaMetadataToIngest(
				ingestionJobKey, fileFormat, title, imageOfVideoMediaItemKey,
				cutOfVideoMediaItemKey, cutOfAudioMediaItemKey,
				FFMpeg::timeToSeconds(ingestionJobKey, startTime).first,
				FFMpeg::timeToSeconds(ingestionJobKey, endTime).first,
				parametersRoot
		...
		else
		{
				...
				if (JSONUtils::isMetadataPresent(parametersRoot, keyField))
				{
					encodingProfileKey =
						JSONUtils::asInt64(parametersRoot, keyField, 0);
							 ...
							 parametersRoot, labelField
						 ))
				{
					string encodingProfileLabel =
						JSONUtils::asString(parametersRoot, labelField, "");

					MMSEngineDBFacade::ContentType videoContentType =
						MMSEngineDBFacade::ContentType::Video;
					encodingProfileKey =
						_mmsEngineDBFacade->getEncodingProfileKeyByLabel(
							workspace->_workspaceKey, videoContentType,
							encodingProfileLabel
						);
				}
				else
			...
			string encodedFileName;
			{
				...
				*/
				...
				encodedFileName = to_string(ingestionJobKey) + "_" +
								  to_string(encodingProfileKey) +
								  getEncodedFileExtensionByEncodingProfile(
									  encodingProfileDetailsRoot
								  ); // "." + fileFormatLowerCase
			...
			_mmsEngineDBFacade->addEncoding_CutFrameAccurate(
				...
				sourceDurationInMilliSecs, sourceFileExtension,
				sourcePhysicalDeliveryURL, sourceTranscoderStagingAssetPathName,
				endTime, encodingProfileKey, encodingProfileDetailsRoot,
				encodedTranscoderStagingAssetPathName,
				encodedNFSStagingAssetPathName.string(),
				_mmsWorkflowIngestionURL, _mmsBinaryIngestionURL,
				_mmsIngestionURL, encodingPriority, newUtcStartTimeInMilliSecs,
				newUtcEndTimeInMilliSecs
			);
		}
	}
	...
	try
	{
		...
		json sourcesToBeEncodedRoot = json::array();
		{
				 ...
				 dependencies)
			{
				...
				{
					tuple<
						int64_t, int64_t, MMSEngineDBFacade::ContentType,
						string, string, string, string, int64_t, string, string,
						bool>
						dependencyInfo = processDependencyInfo(
							workspace, ingestionJobKey, keyAndDependencyType
						);
					tie(sourceMediaItemKey, sourcePhysicalPathKey,
						referenceContentType, mmsSourceAssetPathName,
						sourceRelativePath, sourceFileName, sourceFileExtension,
					...
					string fileFormat;
					{
						fileFormat = JSONUtils::asString(
							encodingProfileDetailsRoot, "fileFormat", ""
						);
...

					{
						...
						encodedTranscoderStagingAssetPathName =
							_mmsStorage->getStagingAssetPathName(
								neededForTranscoder,
								workspace
									->_directoryName, // workspaceDirectoryName
								to_string(ingestionJobKey
								),	 // directoryNamePrefix
								"/", // relativePath,
								// as specified by doc
								...
								// has to be 'content'
								(fileFormat == "hls" || fileFormat == "dash")
									? "content"
									: encodedFileName,
								-1, // _encodingItem->_mediaItemKey, not used
									// because encodedFileName is not ""
...

					{
						...
						{
							vector<tuple<
								int64_t, int, int64_t, int, int, string, string,
								long, string>>
								videoTracks;
							vector<tuple<
								int64_t, int, int64_t, long, string, long, int,
								string>>
								audioTracks;
							...
							for (tuple<
									 int64_t, int, int64_t, int, int, string,
									 string, long, string>
									 videoTrack : videoTracks)
							{
								int trackIndex;
								tie(ignore, trackIndex, ignore, ignore, ignore,
									ignore, ignore, ignore, ignore) =
									videoTrack;
							...
							for (tuple<
									 int64_t, int, int64_t, long, string, long,
									 int, string>
									 audioTrack : audioTracks)
							{
								...
								string language;
								tie(ignore, trackIndex, ignore, ignore, ignore,
									ignore, ignore, language) = audioTrack;
		...
		_mmsEngineDBFacade->addEncodingJob(
			workspace, ingestionJobKey, contentType, encodingPriority,
...
)
{
	try
	{
		...
		tie(sourceMediaItemKey, sourcePhysicalPathKey, referenceContentType,
			sourceAssetPathName, sourceRelativePath, sourceFileName,
			sourceFileExtension, sourceDurationInMilliSeconds,
			sourcePhysicalDeliveryURL, sourceTranscoderStagingAssetPathName,
			stopIfReferenceProcessingError) = dependencyInfo;
		...
		json encodingProfileDetailsRoot = nullptr;
		{
			...
			else if (JSONUtils::isMetadataPresent(parametersRoot, labelField))
			{
				...
				encodingProfileKey =
					_mmsEngineDBFacade->getEncodingProfileKeyByLabel(
						workspace->_workspaceKey, referenceContentType,
						encodingProfileLabel
					);
			}
		...
		_mmsEngineDBFacade->addEncoding_VideoSpeed(
			workspace, ingestionJobKey, sourceMediaItemKey,
			sourcePhysicalPathKey, sourceAssetPathName,
			sourceDurationInMilliSeconds, sourceFileExtension,
			sourcePhysicalDeliveryURL, sourceTranscoderStagingAssetPathName,
			encodingProfileKey, encodingProfileDetailsRoot,
			encodedTranscoderStagingAssetPathName,
			encodedNFSStagingAssetPathName.string(), _mmsWorkflowIngestionURL,
	...
	try
	{
				 ...
				 dependency : dependencies)
		{
			...
			tuple<
				...
				string, string, string, int64_t, string, string, bool>
				dependencyInfo = processDependencyInfo(
					workspace, ingestionJobKey, dependency
				);
			tie(sourceMediaItemKey, sourcePhysicalPathKey, referenceContentType,
				sourceAssetPathName, sourceRelativePath, sourceFileName,
				sourceFileExtension, sourceDurationInMilliSeconds,
			...
			sourceRoot["sourceDurationInMilliSeconds"] =
			...
			sourceRoot["sourceTranscoderStagingAssetPathName"] =
				sourceTranscoderStagingAssetPathName;
			sourceRoot["encodedTranscoderStagingAssetPathName"] =
				encodedTranscoderStagingAssetPathName;
		...
		_mmsEngineDBFacade->addEncoding_AddSilentAudio(
			workspace, ingestionJobKey, sourcesRoot, encodingProfileKey,
			encodingProfileDetailsRoot, _mmsWorkflowIngestionURL,
			_mmsBinaryIngestionURL, _mmsIngestionURL, encodingPriority
		);
	}
	catch (runtime_error &e)
...
)
{
	try
	{
		...
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			dependencyInfo_1 = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[0]
			);
		tie(sourceMediaItemKey_1, sourcePhysicalPathKey_1,
			referenceContentType_1, sourceAssetPathName_1, sourceRelativePath_1,
			sourceFileName_1, sourceFileExtension_1,
			sourceDurationInMilliSeconds_1, sourcePhysicalDeliveryURL_1,
			sourceTranscoderStagingAssetPathName_1,
		...
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			dependencyInfo_2 = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[1]
			);
		tie(sourceMediaItemKey_2, sourcePhysicalPathKey_2,
			referenceContentType_2, sourceAssetPathName_2, sourceRelativePath_2,
			sourceFileName_2, sourceFileExtension_2,
			sourceDurationInMilliSeconds_2, sourcePhysicalDeliveryURL_2,
			sourceTranscoderStagingAssetPathName_2,
		...
		if (secondVideoOverlayedOnFirst)
		{
			...
			mainSourceTranscoderStagingAssetPathName =
				sourceTranscoderStagingAssetPathName_1;
		...
		else
		{
			...
			overlaySourceDurationInMilliSeconds =
				sourceDurationInMilliSeconds_1;
		...
		_mmsEngineDBFacade->addEncoding_PictureInPictureJob(
			workspace, ingestionJobKey, mainSourceMediaItemKey,
			...
			mainSourceTranscoderStagingAssetPathName, overlaySourceMediaItemKey,
			overlaySourcePhysicalPathKey, overlaySourceAssetPathName,
			overlaySourceDurationInMilliSeconds, overlaySourceFileExtension,
			overlaySourcePhysicalDeliveryURL,
			overlaySourceTranscoderStagingAssetPathName, soundOfMain,
			encodingProfileKey, encodingProfileDetailsRoot,
			encodedTranscoderStagingAssetPathName,
			encodedNFSStagingAssetPathName.string(), _mmsWorkflowIngestionURL,
			_mmsBinaryIngestionURL, _mmsIngestionURL, encodingPriority
...
)
{
	try
	{
		...
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			introDependencyInfo = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[0]
			);
		tie(introSourceMediaItemKey, introSourcePhysicalPathKey,
			introReferenceContentType, introSourceAssetPathName,
			introSourceRelativePath, introSourceFileName,
			introSourceFileExtension, introSourceDurationInMilliSeconds,
			introSourcePhysicalDeliveryURL,
			introSourceTranscoderStagingAssetPathName,
			introStopIfReferenceProcessingError) = introDependencyInfo;
		...
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			mainDependencyInfo = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[1]
			);
		tie(mainSourceMediaItemKey, mainSourcePhysicalPathKey,
			mainReferenceContentType, mainSourceAssetPathName,
			mainSourceRelativePath, mainSourceFileName, mainSourceFileExtension,
			mainSourceDurationInMilliSeconds, mainSourcePhysicalDeliveryURL,
			mainSourceTranscoderStagingAssetPathName,
			mainStopIfReferenceProcessingError) = mainDependencyInfo;
		...
		tuple<
			int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string,
			string, string, int64_t, string, string, bool>
			outroDependencyInfo = processDependencyInfo(
				workspace, ingestionJobKey, dependencies[2]
			);
		tie(outroSourceMediaItemKey, outroSourcePhysicalPathKey,
			outroReferenceContentType, outroSourceAssetPathName,
			outroSourceRelativePath, outroSourceFileName,
			outroSourceFileExtension, outroSourceDurationInMilliSeconds,
			outroSourcePhysicalDeliveryURL,
			outroSourceTranscoderStagingAssetPathName,
		...
		string encodedFileName =
			to_string(ingestionJobKey) + "_introOutroOverlay" +
		...
		_mmsEngineDBFacade->addEncoding_IntroOutroOverlayJob(
			...
			encodedTranscoderStagingAssetPathName,
			encodedNFSStagingAssetPathName.string(), _mmsWorkflowIngestionURL,
			_mmsBinaryIngestionURL, _mmsIngestionURL,
...
)
{
	try
	{
		...
		tie(sourceMediaItemKey_1, sourcePhysicalPathKey_1,
			referenceContentType_1, mmsSourceAssetPathName_1, ignore,
			sourceFileName_1, sourceFileExtension_1,
		...
		tie(sourceMediaItemKey_2, sourcePhysicalPathKey_2,
			referenceContentType_2, mmsSourceAssetPathName_2, ignore,
			sourceFileName_2, sourceFileExtension_2,
		...
		else if (referenceContentType_1 ==
		...
		else
		{
			string errorMessage =
				...
				MMSEngineDBFacade::toString(referenceContentType_1) +
				", sourceMediaItemKey_1: " + to_string(sourceMediaItemKey_1) +
				", sourcePhysicalPathKey_1: " +
				to_string(sourcePhysicalPathKey_1) + ", contentType_2: " +
				MMSEngineDBFacade::toString(referenceContentType_2) +
		...
		fs::path encodedNFSStagingAssetPathName;
		{
			...
			fs::path workspaceIngestionRepository =
				_mmsStorage->getWorkspaceIngestionRepository(workspace);
			encodedNFSStagingAssetPathName =
				workspaceIngestionRepository / encodedFileName;
		}

		_mmsEngineDBFacade->addEncoding_OverlayImageOnVideoJob(
			workspace, ingestionJobKey, encodingProfileKey,
			encodingProfileDetailsRoot, sourceVideoMediaItemKey,
			sourceVideoPhysicalPathKey, videoDurationInMilliSeconds,
			mmsSourceVideoAssetPathName, sourceVideoPhysicalDeliveryURL,
			sourceVideoFileExtension, sourceImageMediaItemKey,
			sourceImagePhysicalPathKey, mmsSourceImageAssetPathName,
			sourceImagePhysicalDeliveryURL,
			sourceVideoTranscoderStagingAssetPathName,
			encodedTranscoderStagingAssetPathName,
			encodedNFSStagingAssetPathName.string(), encodingPriority,
			_mmsWorkflowIngestionURL, _mmsBinaryIngestionURL, _mmsIngestionURL
		);
	}
	catch (runtime_error &e)
...
)
{
	try
	{
		...
		_mmsEngineDBFacade->addEncoding_OverlayTextOnVideoJob(
			...
			sourceAssetPathName, sourceDurationInMilliSecs,
			sourcePhysicalDeliveryURL, sourceFileExtension,
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "emailNotificationThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
				 ...
				 dependencies)
			{
				try
				{
						...
						Validator::DependencyType::MediaItemKey)
					{
						...
						string ingestionDate;
						int64_t localIngestionJobKey;
						tie(contentType, title, userData, ingestionDate, ignore,
							localIngestionJobKey) = mediaItemDetails;
						...
						sDependencies += string("MediaItemKey") +
							 ...
							 Validator::DependencyType::PhysicalPathKey)
					{
						bool warningIfMissing = false;
						tuple<
							int64_t, MMSEngineDBFacade::ContentType, string,
						...
						string ingestionDate;
						int64_t localIngestionJobKey;
						tie(mediaItemKey, localContentType, title, userData,
							ingestionDate, localIngestionJobKey, ignore, ignore,
							ignore) = mediaItemDetails;

						sDependencies +=
							string("PhysicalPathKey") +
							", physicalPathKey: " + to_string(key) +
							", title: " + title + ". ";
					}
					else // if (dependencyType ==
						 // Validator::DependencyType::IngestionJobKey)
					{
						bool warningIfMissing = false;
						tuple<
							string, MMSEngineDBFacade::IngestionType,
						...
						tie(label, ingestionType, ingestionStatus,
							metaDataContent, errorMessage) =
							ingestionJobDetails;

						sDependencies +=
							string("<br>IngestionJob") + ", dependencyType: " +
							to_string(static_cast<int>(dependencyType)) +
				...
				{
					string errorMessage =
						string("Exception processing dependencies") +
						", _processorIdentifier: " +
						to_string(_processorIdentifier) +
						", ingestionJobKey: " + to_string(ingestionJobKey);
					SPDLOG_ERROR(string() + errorMessage);
					 ...
					 referenceIndex < referencesRoot.size(); referenceIndex++)
				{
					try
					{
						...
						if (JSONUtils::isMetadataPresent(referenceRoot, field))
						{
							...
							tie(referenceLabel, ingestionType, ignore,
								parameters, referenceErrorMessage) =
								ingestionJobDetails;

							sReferencies +=
								string("<br>IngestionJob") +
								", ingestionType: " +
								MMSEngineDBFacade::toString(ingestionType) +
								", ingestionJobKey: " +
								to_string(referenceIngestionJobKey) +
									...
									CheckStreaming)
							{
								...
								if (inputType == "Channel")
								{
										...
										))
									{
										...
										bool warningIfMissing = false;
										tuple<
											int64_t, string, string, string,
											string, int64_t, string, int,
										...
										string streamSourceType;
										tie(ignore, streamSourceType, ignore,
											checkStreaming_streamingUrl, ignore,
											ignore, ignore, ignore, ignore,
											ignore, ignore, ignore, ignore,
											ignore, ignore, ignore, ignore,
											ignore) = channelDetails;
									}
								}
								else
								{
					...
					{
						string errorMessage =
							string("Exception processing referencies") +
							", _processorIdentifier: " +
							to_string(_processorIdentifier) +
							", ingestionJobKey: " + to_string(ingestionJobKey);
						SPDLOG_ERROR(string() + errorMessage);
		...
		tuple<string, string, string> email =
			_mmsEngineDBFacade->getEMailByConfigurationLabel(
				workspace->_workspaceKey, configurationLabel
			);
...

		{
			...
			while (subject.find(strToBeReplaced) != string::npos)
				subject.replace(
					subject.find(strToBeReplaced), strToBeReplaced.length(),
					strToReplace
				);
			while (message.find(strToBeReplaced) != string::npos)
				message.replace(
					message.find(strToBeReplaced), strToBeReplaced.length(),
					strToReplace
				);
		}
		{
		...
		MMSCURL::sendEmail(
			...
			tosCommaSeparated, _emailCcsCommaSeparated, subject, emailBody,
			_emailPassword
		);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "checkStreamingThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		if (inputType == "Stream")
		{
			...
			bool warningIfMissing = false;
			tuple<
				int64_t, string, string, string, string, int64_t, string, int,
				string, int, int, string, int, int, int, int, int, int64_t>
				ipChannelDetails = _mmsEngineDBFacade->getStreamDetails(
					workspace->_workspaceKey, configurationLabel,
					warningIfMissing
				);
			string streamSourceType;
			tie(ignore, streamSourceType, ignore, streamingUrl, ignore, ignore,
...

		{
			...
			SPDLOG_INFO(
				...
				videoTracks.size(), audioTracks.size()
			);
		}
...
)
{
	try
	{
		...
		MMSEngineDBFacade::CrossReferenceType crossReferenceType =
			MMSEngineDBFacade::toCrossReferenceType(
				JSONUtils::asString(parametersRoot, field, "")
			);
		if (crossReferenceType ==
			MMSEngineDBFacade::CrossReferenceType::VideoOfImage)
			crossReferenceType =
				MMSEngineDBFacade::CrossReferenceType::ImageOfVideo;
		else if (crossReferenceType ==
				 MMSEngineDBFacade::CrossReferenceType::AudioOfImage)
			crossReferenceType =
				MMSEngineDBFacade::CrossReferenceType::ImageOfAudio;
		else if (crossReferenceType ==
				 MMSEngineDBFacade::CrossReferenceType::VideoOfPoster)
			crossReferenceType =
		...
		int64_t firstMediaItemKey;
		{
			...
			tie(key, firstContentType, dependencyType,
				stopIfReferenceProcessingError) = keyAndDependencyType;
			...
			if (dependencyType == Validator::DependencyType::MediaItemKey)
			{
				...
				_mmsEngineDBFacade->getMediaItemKeyDetails(
					workspace->_workspaceKey, firstMediaItemKey, false, false
				);
			}
			else
			{
		...
		int64_t secondMediaItemKey;
		{
			...
			tie(key, secondContentType, dependencyType,
				stopIfReferenceProcessingError) = keyAndDependencyType;
			...
			if (dependencyType == Validator::DependencyType::MediaItemKey)
			{
				...
				// (MediaItemKeyNotFound)
				_mmsEngineDBFacade->getMediaItemKeyDetails(
					workspace->_workspaceKey, secondMediaItemKey, false, false
				);
			}
			else
			{
				...
				MMSEngineDBFacade::CrossReferenceType::PosterOfVideo)
		{
				...
				secondContentType == MMSEngineDBFacade::ContentType::Image)
			{
				...
				_mmsEngineDBFacade->addCrossReference(
					ingestionJobKey, secondMediaItemKey, crossReferenceType,
					firstMediaItemKey, crossReferenceParametersRoot
				);
			}
			else if (firstContentType ==
						 MMSEngineDBFacade::ContentType::Image &&
			...
			else
			{
				string errorMessage =
					...
					MMSEngineDBFacade::toString(crossReferenceType) +
					", firstContentType: " +
					MMSEngineDBFacade::toString(firstContentType) +
					", secondContentType: " +
...
)
{
	...
	if (JSONUtils::isMetadataPresent(parametersRoot, field))
	{
		string fileFormatSpecifiedByUser =
			JSONUtils::asString(parametersRoot, field, "");
		if (fileFormatSpecifiedByUser != fileFormat)
		{
			string errorMessage =
				string("Wrong fileFormat") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
...
void MMSEngineProcessor::handleCheckEncodingEvent()
{
	try
	{
		...
		_mmsEngineDBFacade->getEncodingJobs(
...
)
{
	...
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "handleContentRetentionEventThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		-1 // ingestionJobKey
	);
	...
	{
		vector<tuple<shared_ptr<Workspace>, int64_t, int64_t>>
		...
		while (moreRemoveToBeDone)
		{
			try
			{
				...
				_mmsEngineDBFacade->getExpiredMediaItemKeysCheckingDependencies(
					_processorMMS, mediaItemKeyOrPhysicalPathKeyToBeRemoved,
					maxMediaItemKeysNumber
				);
			...
			for (tuple<shared_ptr<Workspace>, int64_t, int64_t>
				 ...
				 mediaItemKeyOrPhysicalPathKeyToBeRemoved)
			{
				...
				catch (runtime_error &e)
				{
					SPDLOG_ERROR(
						...
						to_string(physicalPathKey) + ", exception: " + e.what()
					);

					try
					{
						string processorMMSForRetention = "";
						_mmsEngineDBFacade->updateMediaItem(
...
void MMSEngineProcessor::handleDBDataRetentionEventThread()
{
	...
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "handleDBDataRetentionEventThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		-1 // ingestionJobKey,
	);
...
void MMSEngineProcessor::handleGEOInfoEventThread()
{
	...
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "updateGEOInfo", _processorIdentifier,
		_processorsThreadsNumber.use_count(),
		-1 // ingestionJobKey,
	);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "handleCheckRefreshPartitionFreeSizeEventThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		-1 // ingestionJobKey,
	);
...
)
{
	...
	*/
	...
	SPDLOG_INFO(
		...
		", mediaSourceURL: " + mediaSourceURL + ", mediaFileFormat: " +
		mediaFileFormat + ", md5FileCheckSum: " + md5FileCheckSum +
		", fileSizeInBytes: " + to_string(fileSizeInBytes) +
		", externalReadOnlyStorage: " + to_string(externalReadOnlyStorage)
	);

	return make_tuple(
		nextIngestionStatus, mediaSourceURL, mediaFileFormat, md5FileCheckSum,
		fileSizeInBytes, externalReadOnlyStorage
	);
}

void MMSEngineProcessor::validateMediaSourceFile(
	int64_t ingestionJobKey, string mediaSourcePathName, string mediaFileFormat,
...
)
{
	...
	if (mediaFileFormat != "m3u8-tar.gz" && md5FileCheckSum != "")
	{
		...
		do
		{
			...
			for (size_t bytes_done = 0; bytes_done < size;)
			{
				...
				auto dataStart =
					static_cast<void *>(static_cast<char *>(data) + bytes_done);
		...
		for (auto c : md)
			hashBuffer << std::hex << std::setw(2) << std::setfill('0')
		...
		bool isCaseInsensitiveEqual =
			md5FileCheckSum.length() != md5RealDigest.length()
				? false
				: equal(
					  md5FileCheckSum.begin(), md5FileCheckSum.end(),
					  md5RealDigest.begin(),
	...
	if (mediaFileFormat != "m3u8-tar.gz" && fileSizeInBytes != -1)
	{
		unsigned long downloadedFileSizeInBytes =
			fs::file_size(mediaSourcePathName);
...
size_t curlDownloadCallback(char *ptr, size_t size, size_t nmemb, void *f)
{
	...
	MMSEngineProcessor::CurlDownloadData *curlDownloadData =
		(MMSEngineProcessor::CurlDownloadData *)f;
				 ...
				 curlDownloadData->maxChunkFileSize)
	{
		...
		SPDLOG_INFO(
			...
			curlDownloadData->currentTotalSize,
			curlDownloadData->maxChunkFileSize,
			(curlDownloadData->mediaSourceFileStream).tellp()
		);
	}
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "downloadMediaSourceFileThread",
		_processorIdentifier, _processorsThreadsNumber.use_count(),
		ingestionJobKey
	);
	...
	// in case of youtube url, the real URL to be used has to be calcolated
	{
		string youTubePrefix1("https://www.youtube.com/");
		string youTubePrefix2("https://youtu.be/");
		if ((sourceReferenceURL.size() >= youTubePrefix1.size() &&
			 0 == sourceReferenceURL.compare(
					  0, youTubePrefix1.size(), youTubePrefix1
				  )) ||
	...
	string workspaceIngestionRepository =
		_mmsStorage->getWorkspaceIngestionRepository(workspace);
	string destBinaryPathName = workspaceIngestionRepository + "/" +
								to_string(ingestionJobKey) + "_source";
	// 0: no m3u8
	// 1: m3u8 by .tar.gz
	...
	else
	{
		for (int attemptIndex = 0;
			 attemptIndex < _maxDownloadAttemptNumber && !downloadingCompleted;
			 attemptIndex++)
		{
			...
			try
			{
				...
				if (attemptIndex == 0)
				{
					...
					curlDownloadData.maxChunkFileSize =
						_downloadChunkSizeInMegaBytes * 1000000;
					...
					curlpp::OptionTrait<void *, CURLOPT_WRITEDATA>
						curlDownloadDataData(&curlDownloadData);
					...
					// %40
					request.setOpt(
						new curlpp::options::Url(localSourceReferenceURL)
					);
					string httpsPrefix("https");
					if (localSourceReferenceURL.size() >= httpsPrefix.size() &&
						0 == localSourceReferenceURL.compare(
					...
					curlpp::types::ProgressFunctionFunctor functor = bind(
						&MMSEngineProcessor::progressDownloadCallback, this,
						ingestionJobKey, lastProgressUpdate,
						lastPercentageUpdated, downloadingStoppedByUser,
				...
				else
				{
					...
					long long fileSize;
					{
						ofstream mediaSourceFileStream(
							destBinaryPathName, ofstream::binary | ofstream::app
						);
					...
					curlDownloadData.currentChunkNumber =
						fileSize % curlDownloadData.maxChunkFileSize;
					...
					curlpp::types::ProgressFunctionFunctor functor = bind(
						&MMSEngineProcessor::progressDownloadCallback, this,
						ingestionJobKey, lastTimeProgressUpdate,
						lastPercentageUpdated, downloadingStoppedByUser,
						placeholders::_1, placeholders::_2, placeholders::_3,
						placeholders::_4
					);
					...
					if (fileSize > 2 * 1000 * 1000 * 1000)
						request.setOpt(
							new curlpp::options::ResumeFromLarge(fileSize)
						);
					else
				...
				if (localM3u8TarGzOrM3u8Streaming == 1)
				{
					try
					{
						...
						_mmsStorage->manageTarFileInCaseOfIngestionOfSegments(
							ingestionJobKey, destBinaryPathName,
							workspaceIngestionRepository, sourcePathName
						);
					}
					catch (runtime_error &e)
					{
			...
			catch (curlpp::LogicError &e)
			{
				SPDLOG_ERROR(
					...
					", exception: {}",
					_processorIdentifier, ingestionJobKey,
					localSourceReferenceURL, downloadingStoppedByUser, e.what()
				);
				...
				else
				{
					...
					else
					{
						SPDLOG_INFO(
							...
							", localSourceReferenceURL: " +
							localSourceReferenceURL +
							", _secondsWaitingAmongDownloadingAttempt: " +
							to_string(_secondsWaitingAmongDownloadingAttempt)
						);
...
void MMSEngineProcessor::ftpUploadMediaSource(
	...
	int64_t ingestionJobKey, shared_ptr<Workspace> workspace,
	int64_t mediaItemKey, int64_t physicalPathKey, string ftpServer,
...
)
{
	...
	try
	{
		...
		curlpp::types::ProgressFunctionFunctor functor = bind(
			&MMSEngineProcessor::progressUploadCallback, this, ingestionJobKey,
			lastProgressUpdate, lastPercentageUpdated, uploadingStoppedByUser,
			placeholders::_1, placeholders::_2, placeholders::_3,
			placeholders::_4
		);
		MMSCURL::ftpFile(
			_logger, ingestionJobKey, mmsAssetPathName, fileName, sizeInBytes,
			ftpServer, ftpPort, ftpUserName, ftpPassword, ftpRemoteDirectory,
...
)
{
	...
	try
	{
		...
		*/
		...
		// start
		{
			string facebookURI = string("/") + _facebookGraphAPIVersion + "/" +
								 facebookNodeId + "/videos";

			facebookURL = _facebookGraphAPIProtocol + "://" +
						  _facebookGraphAPIVideoHostName + ":" +
						  to_string(_facebookGraphAPIPort) + facebookURI;
			...
			uploadSessionId =
				JSONUtils::asString(facebookResponseRoot, field, "");
...
size_t
curlUploadVideoOnYouTubeCallback(char *ptr, size_t size, size_t nmemb, void *f)
{
	MMSEngineProcessor::CurlUploadYouTubeData *curlUploadData =
		(MMSEngineProcessor::CurlUploadYouTubeData *)f;
	...
	*/
	...
	if (currentFilePosition + (size * nmemb) <= curlUploadData->fileSizeInBytes)
		...
		curlUploadData->mediaSourceFileStream.read(
			ptr, curlUploadData->fileSizeInBytes - currentFilePosition
		);
	...
	try
	{
		...
		*/
		string videoContentType = "video/*";
		{
			...
			if (sHeaderResponse.find("Location: ") == string::npos &&
			...
			int locationEndIndex =
				sHeaderResponse.find("\r", locationStartIndex);
			if (locationEndIndex == string::npos)
				locationEndIndex =
					sHeaderResponse.find("\n", locationStartIndex);
			if (locationEndIndex == string::npos)
				youTubeUploadURL = sHeaderResponse.substr(locationStartIndex);
			else
				youTubeUploadURL = sHeaderResponse.substr(
					locationStartIndex, locationEndIndex - locationStartIndex
				);
		}
			...
			*/

			{
				...
				headerList.push_back(
					string("Authorization: Bearer ") + youTubeAccessToken
				);
				if (curlUploadData.lastByteSent == -1)
					headerList.push_back(
						string("Content-Length: ") + to_string(sizeInBytes)
					);
				else
					headerList.push_back(
				...
				if (curlUploadData.lastByteSent == -1)
					headerList.push_back(
						string("Content-Type: ") + videoContentType
					);
				else
					headerList.push_back(
					...
					*/
					{
						...
						request.setOpt(
							new curlpp::options::HttpHeader(headerList)
						);
						...
						if (responseCode != 308 ||
							sResponse.find("Range: bytes=") == string::npos)
						{
							// error
							string errorMessage(
						...
						*/
						...
						int rangeEndIndex =
							sResponse.find("\r", rangeStartIndex);
						...
						if (rangeEndIndex == string::npos)
							...
							rangeHeader = sResponse.substr(
								rangeStartIndex, rangeEndIndex - rangeStartIndex
							);
						...
						curlUploadData.lastByteSent =
							stoll(rangeHeader.substr(rangeStartOffsetIndex + 1)
							) +
							1;
						curlUploadData.mediaSourceFileStream.seekg(
							curlUploadData.lastByteSent, ios::beg
						);
					}
				}
				else
...
)
{
	...
	try
	{
		...
		youTubeURL = _youTubeDataAPIProtocol + "://" + _youTubeDataAPIHostName +
					 ":" + to_string(_youTubeDataAPIPort) +
					 _youTubeDataAPIRefreshTokenURI;

		string body = string("client_id=") + _youTubeDataAPIClientId +
...
string MMSEngineProcessor::getFacebookPageToken(
	int64_t ingestionJobKey, shared_ptr<Workspace> workspace,
	string facebookConfigurationLabel, string facebookPageId
)
{
	string facebookURL;
	json responseRoot;
	...
	try
	{
		string userAccessToken =
			_mmsEngineDBFacade->getFacebookUserAccessTokenByConfigurationLabel(
				workspace->_workspaceKey, facebookConfigurationLabel
			);
		...
		facebookURL = _facebookGraphAPIProtocol + "://" +
					  _facebookGraphAPIHostName + ":" +
					  to_string(_facebookGraphAPIPort) + "/" +
					  _facebookGraphAPIVersion + "/" + facebookPageId +
					  "?fields=access_token" +
	...
	{
		string errorMessage =
			string("facebook access token failed") +
			", ingestionJobKey: " + to_string(ingestionJobKey) +
			", facebookURL: " + facebookURL +
			", response: " + JSONUtils::toString(responseRoot) +
			", e.what(): " + e.what();
...
)
{
	...
	try
	{
		...
		{
			string userHeader =
				JSONUtils::asString(userHeadersRoot[userHeaderIndex]);
		...
		if (httpMethod == "PUT")
		{
			if (formData)
			{
				...
				MMSCURL::httpPutFormData(
					_logger, ingestionJobKey, userURL, formData,
					callbackTimeoutInSeconds, maxRetries
				);
			}
			else
			{
				...
				MMSCURL::httpPutString(
					_logger, ingestionJobKey, userURL, callbackTimeoutInSeconds,
					userName, password, httpBody, contentType, otherHeaders,
					maxRetries
				);
			}
		}
		...
		else // if (httpMethod == "GET")
		{
			...
			MMSCURL::httpGet(
				_logger, ingestionJobKey, userURL, callbackTimeoutInSeconds,
				userName, password, otherHeaders, maxRetries
			);
		}
	}
	catch (runtime_error e)
...
void MMSEngineProcessor::moveMediaSourceFileThread(
	shared_ptr<long> processorsThreadsNumber, string sourceReferenceURL,
	int m3u8TarGzOrM3u8Streaming, int64_t ingestionJobKey,
	shared_ptr<Workspace> workspace
)
{

	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "moveMediaSourceFileThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		string mvPrefix("mv://");
		if (!(sourceReferenceURL.size() >= movePrefix.size() &&
			  0 == sourceReferenceURL.compare(0, movePrefix.size(), movePrefix)
			) &&
			!(sourceReferenceURL.size() >= mvPrefix.size() &&
			  0 == sourceReferenceURL.compare(0, mvPrefix.size(), mvPrefix)))
		{
			string errorMessage =
				string("sourceReferenceURL is not a move reference") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
				", sourceReferenceURL: " + sourceReferenceURL;
		...
		int64_t elapsedInSeconds = MMSStorage::move(
			ingestionJobKey, sourcePathName, destBinaryPathName, _logger
		);
...
{
	ThreadsStatistic::ThreadStatistic threadStatistic(
		_mmsThreadsStatistic, "copyMediaSourceFileThread", _processorIdentifier,
		_processorsThreadsNumber.use_count(), ingestionJobKey
	);
	...
	try
	{
		...
		string copyPrefix("copy://");
		string cpPrefix("cp://");
		if (!(sourceReferenceURL.size() >= copyPrefix.size() &&
			  0 == sourceReferenceURL.compare(0, copyPrefix.size(), copyPrefix)
			) &&
			!(sourceReferenceURL.size() >= cpPrefix.size() &&
			  0 == sourceReferenceURL.compare(0, cpPrefix.size(), cpPrefix)))
		{
			string errorMessage =
				string("sourceReferenceURL is not a copy reference") +
				", _processorIdentifier: " + to_string(_processorIdentifier) +
				", ingestionJobKey: " + to_string(ingestionJobKey) +
				", sourceReferenceURL: " + sourceReferenceURL;
		...
		chrono::system_clock::time_point startCoping =
			chrono::system_clock::now();
		fs::copy(
			sourcePathName, destBinaryPathName, fs::copy_options::recursive
		);
		...
		SPDLOG_INFO(
			...
			to_string(
				chrono::duration_cast<chrono::seconds>(endCoping - startCoping)
					.count()
			) +
			"@"
		);
...
)
{
	...
	if (dltotal != 0 && (dltotal == dlnow ||
						 now - lastTimeProgressUpdate >=
							 chrono::seconds(_progressUpdatePeriodInSeconds)))
	{
		...
		SPDLOG_INFO(
			...
			lastPercentageUpdated, downloadingStoppedByUser, dltotal, dlnow,
			ultotal, ulnow
		);
		...
		if (lastPercentageUpdated != downloadingPercentage)
		{
			...
			downloadingStoppedByUser =
				_mmsEngineDBFacade
					->updateIngestionJobSourceDownloadingInProgress(
...
int MMSEngineProcessor::progressUploadCallback(
	int64_t ingestionJobKey,
	chrono::system_clock::time_point &lastTimeProgressUpdate,
	double &lastPercentageUpdated, bool &uploadingStoppedByUser, double dltotal,
...
tuple<
	int64_t, int64_t, MMSEngineDBFacade::ContentType, string, string, string,
	string, int64_t, string, string, bool>
...
)
{
	...
	bool stopIfReferenceProcessingError;
	{
		...
		tie(key, contentType, dependencyType, stopIfReferenceProcessingError) =
			keyAndDependencyType;
		...
		if (dependencyType == Validator::DependencyType::MediaItemKey)
		{
			...
			bool warningIfMissing = true;
			tuple<int64_t, int, string, string, int64_t, bool, int64_t>
				physicalPathDetails = _mmsEngineDBFacade->getSourcePhysicalPath(
		...
		else
		{
			...
			tie(mediaItemKey, ignore, ignore, ignore, ignore, ignore, fileName,
				relativePath, durationInMilliSecs) = mediaItemDetails;
		}
	}
	...
	string assetPathName;
	{
		...
		tie(assetPathName, ignore, ignore, ignore, ignore, ignore) =
			physicalPathDetails;
	}
	...
	return make_tuple(
		mediaItemKey, physicalPathKey, contentType, assetPathName, relativePath,
		fileName, fileExtension, durationInMilliSecs, physicalDeliveryURL,
...
)
{
	...
	fileFormatLowerCase.resize(fileFormat.size());
	transform(
		fileFormat.begin(), fileFormat.end(), fileFormatLowerCase.begin(),
		[](unsigned char c) { return tolower(c); }
	);
