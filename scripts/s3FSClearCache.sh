#!/bin/sh
#
# See https://github.com/s3fs-fuse/s3fs-fuse/blob/master/test/sample_delcache.sh
...
#
ALL_STATS_ATIMELIST=`find $STATS_CDIR -type f -exec echo -n {} \; -exec echo -n "-__-" \; -exec stat -c %X {} \; | awk -F"-__-" '{print $2":"$1}' | sort`
...
TMP_STATS=""
TMP_CFILE=""
echo "$ALL_STATS_ATIMELIST"
IFS=$(echo -en "\n\b")
for part in $ALL_STATS_ATIMELIST; do
  TMP_ATIME=`echo $part | sed 's/\:/-__-/' | awk -F"-__-" '{print $1}'`
  TMP_STATS=`echo $part | sed 's/\:/-__-/' | awk -F"-__-" '{print $2}'`
  ...
  if [ `stat -c %X $TMP_STATS` -eq $TMP_ATIME ]; then
    ...
    if [ $? -ne 0 ]; then
      ...
      if [ $SILENT -ne 1 ]; then
        echo "removed files: $TMP_CFILE	$TMP_STATS"
      fi
    fi
  fi
