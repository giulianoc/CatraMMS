...
then
	...
	#3. waiting the core
	#ulimit -c unlimited
	$CatraMMS_PATH/CatraMMS/bin/mmsEngineService $OPTIONS $CatraMMS_PATH/CatraMMS/conf/mms.rsi.mms2.cfg
elif [ "$command" == "resetdata" ]
then
	$CatraMMS_PATH/CatraMMS/bin/mmsEngineService --resetdata $CatraMMS_PATH/CatraMMS/conf/mms.rsi.mms2.cfg
elif [ "$command" == "status" ]
then
