...
then
	...
	sudo LD_LIBRARY_PATH=$CatraMMS_PATH/ffmpeg/lib:$CatraMMS_PATH/ffmpeg/lib64 $CatraMMS_PATH/nginx/sbin/nginx -p $CatraMMS_PATH/nginx
elif [ "$command" == "status" ]
then
	ps -ef | grep nginx | grep -v grep | grep -v status
elif [ "$command" == "stop" ]
then
	sudo LD_LIBRARY_PATH=$CatraMMS_PATH/ffmpeg/lib:$CatraMMS_PATH/ffmpeg/lib64 $CatraMMS_PATH/nginx/sbin/nginx -s quit
