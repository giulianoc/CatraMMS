...
while [ $currentInSeconds -lt $endInSeconds ]; do
		...
		sqlCommand="CREATE TEMPORARY TABLE t SELECT A.requestStatisticKey FROM MMS_RequestStatistic AS A WHERE A.workspaceKey = $workspaceKey AND A.requestTimestamp between '$startFormatted' AND '$endFormatted' AND EXISTS (SELECT B.requestStatisticKey FROM MMS_RequestStatistic AS B WHERE B.workspaceKey = $workspaceKey and A.userId = B.userId AND B.requestTimestamp between '$startFormattedForNestedSelect' AND '$endFormatted' AND B.requestTimestamp <= DATE_ADD(A.requestTimestamp, INTERVAL $datesTooCloseInSeconds SECOND) AND B.requestTimestamp > A.requestTimestamp); delete from MMS_RequestStatistic where requestStatisticKey in (select requestStatisticKey from t); drop temporary table t;"
...
endCommand=$(date +%s)
echo "$workspaceKey / $datesTooCloseInSeconds / $startDate / $endDate -> $((endCommand-startCommand)) seconds, removed $((rowsBeforeCommand-rowsAfterCommand)) ($rowsBeforeCommand-$rowsAfterCommand) rows" >> $debugFile
