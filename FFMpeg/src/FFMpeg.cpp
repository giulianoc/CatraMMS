...
bool FFMpeg::isOutputFileExisting()
{
    try
    {
        return FileIO::isFileExisting(_outputFfmpegPathFileName.c_str());
    }
    catch(runtime_error e)
    {
        _logger->info(__FILEREF__ + "ffmpeg: FileIO::isFileExisting failed"
            + ", _currentIngestionJobKey: " + to_string(_currentIngestionJobKey)
            + ", _currentEncodingJobKey: " + to_string(_currentEncodingJobKey)
            + ", _currentMMSSourceAssetPathName: " + _currentMMSSourceAssetPathName
            + ", _currentStagingEncodedAssetPathName: " + _currentStagingEncodedAssetPathName
    ...
    {
        _logger->error(__FILEREF__ + "ffmpeg: FileIO::isFileExisting failed"
            + ", _currentIngestionJobKey: " + to_string(_currentIngestionJobKey)
            + ", _currentEncodingJobKey: " + to_string(_currentEncodingJobKey)
            + ", _currentMMSSourceAssetPathName: " + _currentMMSSourceAssetPathName
            + ", _currentStagingEncodedAssetPathName: " + _currentStagingEncodedAssetPathName
...
bool FFMpeg::outputFileAlive(int maxSecondsToWaitToHaveSuccess)
{
	bool outputFileAlive = true;

    try
    {
        if (!FileIO::isFileExisting(_outputFfmpegPathFileName.c_str()))
			outputFileAlive = false; 
		else
		{
			// file exist

			time_t lastModificationTime = 0;
			bool lastModificationTimeCheckSuccessful = false;
			int secondsToWaitBetweenChecks = 1;
			chrono::system_clock::time_point startCheck
				= chrono::system_clock::now();

			while(!lastModificationTimeCheckSuccessful &&
					chrono::duration_cast<chrono::seconds>(
						chrono::system_clock::now() - startCheck).count()
					< maxSecondsToWaitToHaveSuccess)
			{
				time_t utcCurrentFileLastModificationTime;

				FileIO::getFileTime (_outputFfmpegPathFileName.c_str(),
					&utcCurrentFileLastModificationTime);
				if (lastModificationTime == 0)
				{
					lastModificationTime = utcCurrentFileLastModificationTime;

					this_thread::sleep_for(chrono::seconds(
						secondsToWaitBetweenChecks));
				}
				else if (utcCurrentFileLastModificationTime != lastModificationTime)
					lastModificationTimeCheckSuccessful = true;
				else
					this_thread::sleep_for(chrono::seconds(
						secondsToWaitBetweenChecks));
			}

			if (!lastModificationTimeCheckSuccessful)
				outputFileAlive = false;
		}

		return outputFileAlive;
    }
    catch(runtime_error e)
    {
        _logger->info(__FILEREF__ + "ffmpeg: checkOutputFileAlive failed"
            + ", _currentIngestionJobKey: " + to_string(_currentIngestionJobKey)
            + ", _currentEncodingJobKey: " + to_string(_currentEncodingJobKey)
            + ", _currentMMSSourceAssetPathName: " + _currentMMSSourceAssetPathName
            + ", _currentStagingEncodedAssetPathName: " + _currentStagingEncodedAssetPathName
    ...
    {
        _logger->error(__FILEREF__ + "ffmpeg: checkOutputFileAlive failed"
            + ", _currentIngestionJobKey: " + to_string(_currentIngestionJobKey)
            + ", _currentEncodingJobKey: " + to_string(_currentEncodingJobKey)
            + ", _currentMMSSourceAssetPathName: " + _currentMMSSourceAssetPathName
            + ", _currentStagingEncodedAssetPathName: " + _currentStagingEncodedAssetPathName
