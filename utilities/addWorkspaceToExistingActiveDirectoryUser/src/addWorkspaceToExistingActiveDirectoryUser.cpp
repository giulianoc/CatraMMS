...
int main(int iArgc, char *pArgv[])
{
	...
	logger->info(
		__FILEREF__ + "addWorkspaceToExistingActiveDirectoryUser" + ", userKey: " + to_string(userKey) + ", userEmailAddress: " + userEmailAddress +
...

	{
		...
		mmsEngineDBFacade->createAPIKeyForActiveDirectoryUser(
			userKey, userEmailAddress, createRemoveWorkspace, ingestWorkflow, createProfiles, deliveryAuthorization, shareWorkspace, editMedia,
			editConfiguration, killEncoding, cancelIngestionJob, editEncodersPool, applicationRecorder, workspaceKey, 30
		);
	}
