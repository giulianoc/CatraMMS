...
int main(int iArgc, char *pArgv[])
{
	...
	{
		logger->info(
			__FILEREF__ + "Registering User and Workspace..." + ", name: " + name + ", email: " + email + ", password: " + password +
			", workspaceName: " + workspaceName + ", country: " + country + ", encodingPriority: " + MMSEngineDBFacade::toString(encodingPriority) +
			", encodingPeriod: " + MMSEngineDBFacade::toString(encodingPeriod) + ", maxIngestionsNumber: " + to_string(maxIngestionsNumber) +
			", maxStorageInMB: " + to_string(maxStorageInMB)
		);
		workspaceKeyUserKeyAndConfirmationCode = mmsEngineDBFacade->registerUserAndAddWorkspace(
		...
		logger->info(
			...
			", userKey: " + to_string(userKey) + ", confirmationCode: " + get<2>(workspaceKeyUserKeyAndConfirmationCode)
		);
	}
	catch (exception &e)
	{
	...
	try
	{
		tuple<string, string, string> apiKeyNameAndEmailAddress =
			mmsEngineDBFacade->confirmRegistration(get<2>(workspaceKeyUserKeyAndConfirmationCode), 30);
		...
		logger->info(
			__FILEREF__ + "User confirmed" + ", workspaceName: " + workspaceName + ", email: " + email + ", userKey: " +
			to_string(get<1>(workspaceKeyUserKeyAndConfirmationCode)) + ", confirmationCode: " + get<2>(workspaceKeyUserKeyAndConfirmationCode)
		);
...

		{
			...
			emailBody.push_back(
				string("<p>here follows the user key <b>") + to_string(get<1>(workspaceKeyUserKeyAndConfirmationCode)) + "</b>, email <b>" + email +
...
Json::Value loadConfigurationFile(const char *configurationPathName)
{
	...
	{
		cerr << string("wrong json configuration format") + ", configurationPathName: " + configurationPathName << endl;
	}
