	...
	unordered_map<string, string> queryParameters)
{
    ...
    try
    {
		...
		auto cdn77ChannelConfigurationLabelIt = queryParameters.find("cdn77ChannelConfigurationLabel");
		if (cdn77ChannelConfigurationLabelIt != queryParameters.end() && cdn77ChannelConfigurationLabelIt->second != "")
		{
			cdn77ChannelConfigurationLabel = cdn77ChannelConfigurationLabelIt->second;
			...
			string plus = "\\+";
			string plusDecoded = " ";
			string firstDecoding = regex_replace(cdn77ChannelConfigurationLabel, regex(plus), plusDecoded);

			cdn77ChannelConfigurationLabel = curlpp::unescape(firstDecoding);
		}

		long expirationInMinutes = 10;
        auto expirationInMinutesIt = queryParameters.find("expirationInMinutes");
        if (expirationInMinutesIt != queryParameters.end())
			expirationInMinutes = stol(expirationInMinutesIt->second);

		long streamConfKey = -1;
        auto streamConfKeyIt = queryParameters.find("streamConfKey");
        if (streamConfKeyIt != queryParameters.end())
			streamConfKey = stol(streamConfKeyIt->second);
		...
		if (titleIt != queryParameters.end())
		{
		...
		{
			string errorMessage = string("label parameter cannot be missing")
			;
			_logger->error(__FILEREF__ + errorMessage);
		...
		tuple<string, string, string> cdn77ChannelDetails = _mmsEngineDBFacade->getCDN77ChannelDetails(
			requestWorkspace->_workspaceKey, cdn77ChannelConfigurationLabel);
		string resourceURL;
		string filePath;
		string secureToken;
		tie(resourceURL, filePath, secureToken) = cdn77ChannelDetails;

		string deliveryURL = MMSDeliveryAuthorization::getSignedCDN77URL(resourceURL, filePath,
			secureToken, expirationInMinutes, _logger);

		if (!filteredByStatistic && userId != "" && streamConfKey != -1 && title != "")
		{
			try
			{
		...
		string responseBody;
		{
			...
			string field = "deliveryURL";
			responseBodyRoot[field] = deliveryURL;
