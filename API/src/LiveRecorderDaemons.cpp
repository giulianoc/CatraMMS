    ...
    : FFMPEGEncoderBase(configuration, logger)
{
    try
    {
        ...
        _logger->info(
            ...
            to_string(_liveRecorderChunksIngestionCheckInSeconds)
        );
        ...
        _logger->info(
            ...
            to_string(_liveRecorderVirtualVODIngestionInSeconds)
        );
    }
...
void LiveRecorderDaemons::startChunksIngestionThread()
{
    ...
    while (!_liveRecorderChunksIngestionThreadShutdown)
    {
        try
        {
            chrono::system_clock::time_point startAllChannelsIngestionChunks =
                chrono::system_clock::now();

            lock_guard<mutex> locker(*_liveRecordingMutex);

            for (shared_ptr<FFMPEGEncoderBase::LiveRecording> liveRecording :
                 *_liveRecordingsCapability)
            {
                if (liveRecording->_childPid != 0) // running
                {
                    _logger->info(
                        __FILEREF__ + "processSegmenterOutput ..." +
                    ...
                    chrono::system_clock::time_point
                        startSingleChannelIngestionChunks =
                            chrono::system_clock::now();
                    ...
                    try
                    {
                            ...
                            Json::nullValue)
                        {
                            ...
                            string outputFileFormat;
                            {
                                ...
                                outputFileFormat = JSONUtils::asString(
                                    liveRecording->_ingestedParametersRoot,
                                    field, "ts"
                                );
                            }
                            ...
                            if (liveRecording->_segmenterType ==
                                "streamSegmenter")
                            {
                                lastRecordedAssetInfo = processStreamSegmenterOutput(
                                    ...
                                    liveRecording
                                        ->_chunksTranscoderStagingContentsPath,
                                    liveRecording
                                        ->_chunksNFSStagingContentsPath,
                                    liveRecording->_segmentListFileName,
                                    liveRecording->_recordedFileNamePrefix,
                                    liveRecording->_lastRecordedAssetFileName,
                                    liveRecording
                                        ->_lastRecordedAssetDurationInSeconds,
                                    liveRecording
                                        ->_lastRecordedSegmentUtcStartTimeInMillisecs
                                );
                            }
                    ...
                    {
                        string errorMessage =
                            string("processSegmenterOutput failed") +
                            ", liveRecording->_ingestionJobKey: " +
                            to_string(liveRecording->_ingestionJobKey) +
                            ", liveRecording->_encodingJobKey: " +
                            to_string(liveRecording->_encodingJobKey) +
                            ", e.what(): " + e.what();
                    ...
                    _logger->info(
                        __FILEREF__ + "Single Channel Ingestion Chunks" +
                        ", ingestionJobKey: " +
                        to_string(liveRecording->_ingestionJobKey) +
                        ", encodingJobKey: " +
                        to_string(liveRecording->_encodingJobKey) +
            ...
            _logger->info(
                __FILEREF__ + "All Channels Ingestion Chunks" +
                ", @MMS statistics@ - elapsed time: @" +
                to_string(chrono::duration_cast<chrono::seconds>(
                              chrono::system_clock::now() -
                              startAllChannelsIngestionChunks
                )
                              .count()) +
                "@"
            );
        ...
        this_thread::sleep_for(
            chrono::seconds(_liveRecorderChunksIngestionCheckInSeconds)
        );
    }
}
...
void LiveRecorderDaemons::startVirtualVODIngestionThread()
{
    ...
    while (!_liveRecorderVirtualVODIngestionThreadShutdown)
    {
        ...
        try
        {
            ...
            vector<shared_ptr<FFMPEGEncoderBase::LiveRecording>>
                copiedRunningLiveRecordingCapability;
            ...
            vector<shared_ptr<FFMPEGEncoderBase::LiveRecording>>
                sourceLiveRecordingCapability;

            chrono::system_clock::time_point startClone =
                chrono::system_clock::now();
            ...
            int liveRecordingVirtualVODCounter = 0;
            {
                ...
                for (shared_ptr<FFMPEGEncoderBase::LiveRecording>
                         liveRecording : *_liveRecordingsCapability)
                {
                    if (liveRecording->_childPid != 0 &&
                ...
                _logger->info(
                    ...
                    ", liveRecordingVirtualVODCounter: " +
                    to_string(liveRecordingVirtualVODCounter) +
                    ", liveRecordingNotVirtualVODCounter: " +
                    to_string(liveRecordingNotVirtualVODCounter)
                );
            }
            _logger->info(
                __FILEREF__ + "virtualVOD clone" +
                ", copiedRunningLiveRecordingCapability.size: " +
                to_string(copiedRunningLiveRecordingCapability.size()) +
                ", @MMS statistics@ - elapsed (millisecs): " +
                to_string(chrono::duration_cast<chrono::milliseconds>(
                              chrono::system_clock::now() - startClone
                )
                              .count())
            );

            chrono::system_clock::time_point startAllChannelsVirtualVOD =
                chrono::system_clock::now();
            ...
            for (int liveRecordingIndex = 0;
                 ...
                 liveRecordingIndex++)
            {
                shared_ptr<FFMPEGEncoderBase::LiveRecording>
                    copiedLiveRecording = copiedRunningLiveRecordingCapability
                        [liveRecordingIndex];
                shared_ptr<FFMPEGEncoderBase::LiveRecording>
                    sourceLiveRecording =
                ...
                _logger->info(
                    __FILEREF__ + "virtualVOD" + ", ingestionJobKey: " +
                ...
                if (sourceLiveRecording->_childPid == 0 ||
                    copiedLiveRecording->_recordingStart !=
                        sourceLiveRecording->_recordingStart)
                {
                    _logger->info(
                        __FILEREF__ + "virtualVOD. LiveRecorder changed" +
                        ", ingestionJobKey: " +
                        to_string(copiedLiveRecording->_ingestionJobKey) +
                        ", encodingJobKey: " +
                        to_string(copiedLiveRecording->_encodingJobKey) +
                        ...
                        to_string(sourceLiveRecording->_childPid) +
                        ", "
                        "copiedLiveRecording->_recordingStart.time_since_epoch("
                        ")."
                        "count(): " +
                        to_string(copiedLiveRecording->_recordingStart
                                      .time_since_epoch()
                                      .count()) +
                        ", "
                        ...
                        ")."
                        "count(): " +
                        to_string(sourceLiveRecording->_recordingStart
                                      .time_since_epoch()
                                      .count())
                    );
                ...
                {
                    chrono::system_clock::time_point
                        startSingleChannelVirtualVOD =
                            chrono::system_clock::now();

                    virtualVODsNumber++;

                    _logger->info(
                        __FILEREF__ + "buildAndIngestVirtualVOD ..." +
                        ", ingestionJobKey: " +
                        to_string(copiedLiveRecording->_ingestionJobKey) +
                        ", encodingJobKey: " +
                        to_string(copiedLiveRecording->_encodingJobKey) +
                        ", externalEncoder: " +
                        to_string(copiedLiveRecording->_externalEncoder) +
                        ", childPid: " +
                        to_string(copiedLiveRecording->_childPid) +
                        ", virtualVOD: " +
                        to_string(copiedLiveRecording->_virtualVOD) +
                        ", virtualVODsNumber: " + to_string(virtualVODsNumber) +
                        ", monitorVirtualVODManifestDirectoryPath: " +
                        copiedLiveRecording
                            ->_monitorVirtualVODManifestDirectoryPath +
                        ", monitorVirtualVODManifestFileName: " +
                        copiedLiveRecording
                            ->_monitorVirtualVODManifestFileName +
                    ...
                    try
                    {
                        int64_t recordingCode = JSONUtils::asInt64(
                            copiedLiveRecording->_ingestedParametersRoot,
                            "recordingCode", 0
                        );
                        string ingestionJobLabel = JSONUtils::asString(
                            copiedLiveRecording->_encodingParametersRoot,
                            "ingestionJobLabel", ""
                        );
                        string liveRecorderVirtualVODUniqueName =
                            ingestionJobLabel + "(" + to_string(recordingCode) +
                            "_" +
                            to_string(copiedLiveRecording->_ingestionJobKey) +
                            ")";
                        ...
                        string apiKey;
                        {
                                ...
                                ))
                            {
                                    ...
                                    ))
                                {
                                    ...
                                    apiKey =
                                        Encrypt::opensslDecrypt(apiKeyEncrypted
                                        );
                                }
                            }
                        }
                        ...
                        string mmsBinaryIngestionURL;
                        {
                            ...
                            mmsWorkflowIngestionURL = JSONUtils::asString(
                                copiedLiveRecording->_encodingParametersRoot,
                                field, ""
                            );
                            ...
                            mmsBinaryIngestionURL = JSONUtils::asString(
                                copiedLiveRecording->_encodingParametersRoot,
                                field, ""
                            );
                        }
                        ...
                        segmentsNumber = buildAndIngestVirtualVOD(
                            ...
                            recordingCode, ingestionJobLabel,
                            liveRecorderVirtualVODUniqueName,
                            _liveRecorderVirtualVODRetention,
                            copiedLiveRecording
                                ->_liveRecorderVirtualVODImageMediaItemKey,
                            userKey, apiKey, mmsWorkflowIngestionURL,
                    ...
                    {
                        string errorMessage =
                            string("buildAndIngestVirtualVOD failed") +
                            ", copiedLiveRecording->_ingestionJobKey: " +
                            to_string(copiedLiveRecording->_ingestionJobKey) +
                            ", copiedLiveRecording->_encodingJobKey: " +
                            to_string(copiedLiveRecording->_encodingJobKey) +
                    ...
                    _logger->info(
                        __FILEREF__ + "Single Channel Virtual VOD" +
                        ", ingestionJobKey: " +
                        to_string(copiedLiveRecording->_ingestionJobKey) +
                        ", encodingJobKey: " +
                        to_string(copiedLiveRecording->_encodingJobKey) +
            ...
            _logger->info(
                __FILEREF__ + "All Channels Virtual VOD" +
                ", @MMS statistics@ - elapsed time: @" +
                to_string(chrono::duration_cast<chrono::seconds>(
                              chrono::system_clock::now() -
                              startAllChannelsVirtualVOD
                )
                              .count()) +
                "@"
            );
        ...
        {
            string errorMessage =
                string("liveRecorderVirtualVODIngestion failed") +
                ", e.what(): " + e.what();
        ...
        if (virtualVODsNumber < 5)
            this_thread::sleep_for(
                chrono::seconds(_liveRecorderVirtualVODIngestionInSeconds * 2)
            );
        else
            this_thread::sleep_for(
                chrono::seconds(_liveRecorderVirtualVODIngestionInSeconds)
            );
    }
}
...
)
{
    ...
    double newLastRecordedAssetDurationInSeconds =
        lastRecordedAssetDurationInSeconds;
    int64_t newLastRecordedSegmentUtcStartTimeInMillisecs =
        lastRecordedSegmentUtcStartTimeInMillisecs;
    try
    {
        _logger->info(
            __FILEREF__ + "processStreamSegmenterOutput" +
            ...
            to_string(segmentDurationInSeconds) + ", outputFileFormat: " +
            outputFileFormat + ", chunksTranscoderStagingContentsPath: " +
            ...
            ", lastRecordedAssetDurationInSeconds: " +
            to_string(lastRecordedAssetDurationInSeconds)
        );
        ...
        {
            string errorMessage =
                __FILEREF__ + "No segment list file found yet" +
                ", chunksTranscoderStagingContentsPath: " +
                chunksTranscoderStagingContentsPath +
                ", segmentListFileName: " + segmentListFileName +
                ", lastRecordedAssetFileName: " + lastRecordedAssetFileName;
            ...
            return make_tuple(
                lastRecordedAssetFileName, lastRecordedAssetDurationInSeconds,
        ...
        while (getline(segmentList, currentRecordedAssetFileName))
        {
            if (!reachedNextFileToProcess)
            {
                ...
                else if (currentRecordedAssetFileName ==
            ...
            time_t utcCurrentRecordedFileCreationTime =
                getMediaLiveRecorderStartTime(
                    ingestionJobKey, encodingJobKey,
                    currentRecordedAssetFileName, segmentDurationInSeconds,
                    isFirstChunk
                );
            ...
            */
            ...
            newLastRecordedSegmentUtcStartTimeInMillisecs =
                utcCurrentRecordedFileCreationTime * 1000;
            ...
            */
            ...
            Json::Value userDataRoot;
            {
                ...
                liveRecordingChunkRoot["ingestionJobKey"] =
            ...
            string addContentTitle;
            {
                ...
                */
...

                {
                    ...
                    localtime_r(
                        &utcCurrentRecordedFileCreationTime, &tmDateTime
                    );
                    ...
                    */
                    ...
                    sprintf(
                        strCurrentRecordedFileTime, "%02d:%02d:%02d",
...

                {
                    ...
                    // from utc to local time
                    localtime_r(
                        &utcCurrentRecordedFileLastModificationTime, &tmDateTime
                    );
            ...
            if (isFirstChunk)
            {
                _logger->info(
                    ...
                    ", currentRecordedAssetPathName: " +
                    chunksTranscoderStagingContentsPath +
                    currentRecordedAssetFileName + ", title: " + addContentTitle
                );
            ...
            else
            {
                ...
                {
                    _logger->info(
                        __FILEREF__ + "ingest Recorded media" +
                        ", ingestionJobKey: " + to_string(ingestionJobKey) +
                        ", encodingJobKey: " + to_string(encodingJobKey) +
                        ", chunksTranscoderStagingContentsPath: " +
                        chunksTranscoderStagingContentsPath +
                    ...
                    if (externalEncoder)
                        ingestRecordedMediaInCaseOfExternalTranscoder(
                            ingestionJobKey,
                            chunksTranscoderStagingContentsPath,
                            currentRecordedAssetFileName, addContentTitle,
                            uniqueName,
                            /* highAvailability, */ userDataRoot,
                            outputFileFormat, ingestedParametersRoot,
                        ...
                        ingestRecordedMediaInCaseOfInternalTranscoder(
                            ...
                            /* highAvailability, */ userDataRoot,
                            outputFileFormat, ingestedParametersRoot,
                            encodingParametersRoot, false
                        );
                }
                catch (runtime_error &e)
                {
                    _logger->error(
                        __FILEREF__ + "ingestRecordedMedia failed" +
                        ", encodingJobKey: " + to_string(encodingJobKey) +
                        ", ingestionJobKey: " + to_string(ingestionJobKey) +
    ...
    {
        _logger->error(
            __FILEREF__ + "processStreamSegmenterOutput failed" +
            ", encodingJobKey: " + to_string(encodingJobKey) +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", chunksTranscoderStagingContentsPath: " +
            chunksTranscoderStagingContentsPath +
    ...
    return make_tuple(
        newLastRecordedAssetFileName, newLastRecordedAssetDurationInSeconds,
        newLastRecordedSegmentUtcStartTimeInMillisecs
    );
}
...
)
{
    ...
    {
        _logger->info(
            __FILEREF__ + "processHLSSegmenterOutput" + ", ingestionJobKey: " +
            to_string(ingestionJobKey) + ", encodingJobKey: " +
            to_string(encodingJobKey)
            // + ", highAvailability: " + to_string(highAvailability)
            // + ", main: " + to_string(main)
            + ", segmentDurationInSeconds: " +
        ...
        string toBeIngestedSegmentFileName;
        {
            ...
            {
                _logger->info(
                    __FILEREF__ + "Reading manifest line" +
                    ", ingestionJobKey: " + to_string(ingestionJobKey) +
                    ", encodingJobKey: " + to_string(encodingJobKey) +
                    ", manifestLine: " + manifestLine +
                    ", toBeIngested: " + to_string(toBeIngested) +
                    ", toBeIngestedSegmentDuration: " +
                    to_string(toBeIngestedSegmentDuration) +
                    ...
                    toBeIngestedSegmentFileName + ", currentSegmentDuration: " +
                    to_string(currentSegmentDuration) +
                    ", currentSegmentUtcStartTimeInMillisecs: " +
                    to_string(currentSegmentUtcStartTimeInMillisecs) +
                ...
                if (manifestLine.size() >= durationPrefix.size() &&
                    0 == manifestLine.compare(
                             0, durationPrefix.size(), durationPrefix
                         ))
                {
                    ...
                    {
                        string errorMessage =
                            string("wrong manifest line format") +
                            ", ingestionJobKey: " + to_string(ingestionJobKey) +
                            ", encodingJobKey: " + to_string(encodingJobKey) +
                            ", manifestLine: " + manifestLine;
                        _logger->error(__FILEREF__ + errorMessage);
                    ...
                    if (toBeIngested)
                        toBeIngestedSegmentDuration = stod(manifestLine.substr(
                            durationPrefix.size(),
                            endOfSegmentDuration - durationPrefix.size()
                        ));
                    else
                        currentSegmentDuration = stod(manifestLine.substr(
                            durationPrefix.size(),
                            endOfSegmentDuration - durationPrefix.size()
                        ));
                }
                else if (manifestLine.size() >= dateTimePrefix.size() &&
                         0 == manifestLine.compare(
                                  0, dateTimePrefix.size(), dateTimePrefix
                              ))
                {
                    if (toBeIngested)
                        toBeIngestedSegmentUtcStartTimeInMillisecs =
                            DateTime::sDateMilliSecondsToUtc(
                     ...
                     lastRecordedAssetFileName == ""))
                {
                    ...
                    // newLastRecordedSegmentUtcStartTimeInMillisecs
                    {
                        int64_t toBeIngestedSegmentUtcEndTimeInMillisecs =
                            toBeIngestedSegmentUtcStartTimeInMillisecs +
                            (toBeIngestedSegmentDuration * 1000);
                        ...
                        _logger->info(
                            ...
                            to_string(toBeIngestedSegmentUtcEndTimeInMillisecs
                            ) +
                            ", toBeIngestedSegmentFileName: " +
                            toBeIngestedSegmentFileName
                        );
                        ...
                        else if (toBeIngestedSegmentUtcStartTimeInMillisecs <=
                                 newLastRecordedSegmentUtcStartTimeInMillisecs)
                        {
                            // toBeIngestedSegmentUtcStartTimeInMillisecs:
                        ...
                        // initialize metadata and ingest the asset
                        {
                            ...
                            Json::Value userDataRoot;
                            {
                                ...
                                {
                                    int64_t recordingCode = JSONUtils::asInt64(
                                        ingestedParametersRoot, "recordingCode",
                                        0
                                    );
                                    // recordingCode is used by DB generated
                                    // column
                                    liveRecordingChunkRoot["recordingCode"] =
                                        recordingCode;
                                }
                                liveRecordingChunkRoot["ingestionJobLabel"] =
                                    ingestionJobLabel;
                                liveRecordingChunkRoot["ingestionJobKey"] =
                                    (int64_t)(ingestionJobKey);
                                liveRecordingChunkRoot["utcChunkStartTime"] =
                                    (int64_t)(toBeIngestedSegmentUtcStartTimeInMillisecs /
                                              1000);
                                ...
                                liveRecordingChunkRoot["uniqueName"] =
                                    uniqueName;

                                mmsDataRoot["liveRecordingChunk"] =
                                    liveRecordingChunkRoot;
                            ...
                            string addContentTitle;
                            {
...

                                {
                                    ...
                                    time_t toBeIngestedSegmentUtcStartTimeInSeconds =
                                        toBeIngestedSegmentUtcStartTimeInMillisecs /
                                        1000;
                                    int toBeIngestedSegmentMilliSecs =
                                        toBeIngestedSegmentUtcStartTimeInMillisecs %
                                        1000;

                                    // from utc to local time
                                    localtime_r(
                                        &toBeIngestedSegmentUtcStartTimeInSeconds,
                                        &tmDateTime
                                    );
                                    ...
                                    */
                                    ...
                                    addContentTitle +=
                                        strCurrentRecordedFileTime; // local
                                                                    // time
                                }
...

                                {
                                    ...
                                    // from utc to local time
                                    localtime_r(
                                        &toBeIngestedSegmentUtcEndTimeInSeconds,
                                        &tmDateTime
                                    );
    ...
    {
        _logger->error(
            __FILEREF__ + "processHLSSegmenterOutput failed" +
            ", encodingJobKey: " + to_string(encodingJobKey) +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", chunksTranscoderStagingContentsPath: " +
            chunksTranscoderStagingContentsPath +
...
)
{
    try
    {
        ...
        {
            _logger->info(
                __FILEREF__ + "Chunk copying" +
                ", ingestionJobKey: " + to_string(ingestionJobKey) +
                ", currentRecordedAssetFileName: " +
                currentRecordedAssetFileName +
                ", source: " + chunksTranscoderStagingContentsPath +
                currentRecordedAssetFileName +
                ", dest: " + chunksNFSStagingContentsPath
            );

            chrono::system_clock::time_point startCopying =
                chrono::system_clock::now();
            fs::copy(
                chunksTranscoderStagingContentsPath +
                    currentRecordedAssetFileName,
            ...
            chrono::system_clock::time_point endCopying =
                chrono::system_clock::now();

            _logger->info(
                __FILEREF__ + "Chunk copied" +
                ", ingestionJobKey: " + to_string(ingestionJobKey) +
                ", source: " + chunksTranscoderStagingContentsPath +
                currentRecordedAssetFileName +
                ", dest: " + chunksNFSStagingContentsPath +
        ...
        else
        {
            ...
            chrono::system_clock::time_point startMoving =
                chrono::system_clock::now();
            MMSStorage::move(
                ingestionJobKey,
                chunksTranscoderStagingContentsPath +
            ...
            chrono::system_clock::time_point endMoving =
                chrono::system_clock::now();

            _logger->info(
                __FILEREF__ + "Chunk moved" +
                ", ingestionJobKey: " + to_string(ingestionJobKey) +
                ", source: " + chunksTranscoderStagingContentsPath +
                currentRecordedAssetFileName +
                ", dest: " + chunksNFSStagingContentsPath +
    ...
    catch (runtime_error e)
    {
        ...
        _logger->error(
            __FILEREF__ + "Coping/Moving of the chunk failed" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", exception: " + errorMessage
        );
        if (errorMessage.find(string("errno: 28")) != string::npos)
            _logger->error(
                __FILEREF__ + "No space left on storage" +
                ", ingestionJobKey: " + to_string(ingestionJobKey) +
                ", exception: " + errorMessage
            );

        _logger->info(
            __FILEREF__ + "remove" + ", generated chunk: " +
    ...
    {
        _logger->error(
            __FILEREF__ + "Ingested URL failed (exception)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", exception: " + e.what()
        );
    ...
    try
    {
        workflowMetadata = buildChunkIngestionWorkflow(
            ...
            currentRecordedAssetFileName, chunksNFSStagingContentsPath,
            addContentTitle, uniqueName, userDataRoot, fileFormat,
        ...
        string sResponse = MMSCURL::httpPostString(
                               _logger, ingestionJobKey,
                               mmsWorkflowIngestionURL, _mmsAPITimeoutInSeconds,
                               to_string(userKey), apiKey, workflowMetadata,
    ...
    {
        _logger->error(
            __FILEREF__ + "Ingested URL failed (runtime_error)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
...
)
{
    ...
    try
    {
        ...
        addContentIngestionJobKey =
            getAddContentIngestionJobKey(ingestionJobKey, sResponse);
    }
    catch (runtime_error e)
    {
        _logger->error(
            __FILEREF__ + "Ingestion workflow failed (runtime_error)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", mmsWorkflowIngestionURL: " + mmsWorkflowIngestionURL +
            ", workflowMetadata: " + workflowMetadata +
            ", exception: " + e.what()
    ...
    {
        _logger->error(
            __FILEREF__ + "Ingestion workflow failed (exception)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", mmsWorkflowIngestionURL: " + mmsWorkflowIngestionURL +
            ", workflowMetadata: " + workflowMetadata +
            ", exception: " + e.what()
    ...
    try
    {
        int64_t chunkFileSize = fs::file_size(
            chunksTranscoderStagingContentsPath + currentRecordedAssetFileName
        );
        ...
        mmsBinaryURL =
            mmsBinaryIngestionURL + "/" + to_string(addContentIngestionJobKey);

        string sResponse = MMSCURL::httpPostFile(
            _logger, ingestionJobKey, mmsBinaryURL, _mmsBinaryTimeoutInSeconds,
            to_string(userKey), apiKey,
            chunksTranscoderStagingContentsPath + currentRecordedAssetFileName,
            chunkFileSize,
            3 // maxRetryNumber
        );
    ...
    {
        _logger->error(
            __FILEREF__ + "Ingestion binary failed" + ", ingestionJobKey: " +
            to_string(ingestionJobKey) + ", mmsBinaryURL: " + mmsBinaryURL +
            ", workflowMetadata: " + workflowMetadata +
            ", exception: " + e.what()
        );
...
)
{
    ...
    try
    {
        ...
        */
        ...
        int64_t utcChunkStartTime =
            JSONUtils::asInt64(liveRecordingChunkRoot, "utcChunkStartTime", -1);
...

        {
            ...
            char currentUtcChunkStartTime_HHMISS[64];
            {
                ...
                sprintf(
                    currentUtcChunkStartTime_HHMISS, "%02d:%02d:%02d",
                    tmDateTime.tm_hour, tmDateTime.tm_min, tmDateTime.tm_sec
                );
            }
            {
        ...
        _logger->info(
            __FILEREF__ + "Recording Workflow metadata generated" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) + ", " +
            addContentTitle + ", " + currentRecordedAssetFileName +
            ", prev: " + to_string(utcPreviousChunkStartTime) + ", from: " +
            to_string(utcChunkStartTime) + ", to: " + to_string(utcChunkEndTime)
        );
    ...
    {
        _logger->error(
            __FILEREF__ + "buildRecordedMediaWorkflow failed (runtime_error)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", workflowMetadata: " + workflowMetadata +
            ", exception: " + e.what()
        );
    ...
    {
        _logger->error(
            __FILEREF__ + "buildRecordedMediaWorkflow failed (exception)" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", workflowMetadata: " + workflowMetadata +
            ", exception: " + e.what()
        );
...
bool LiveRecorderDaemons::isLastLiveRecorderFile(
    ...
    time_t utcCurrentRecordedFileCreationTime,
    string chunksTranscoderStagingContentsPath, string recordedFileNamePrefix,
    int segmentDurationInSeconds, bool isFirstChunk
)
{
    ...
    try
    {
        ...
        for (fs::directory_entry const &entry :
             fs::directory_iterator(chunksTranscoderStagingContentsPath))
        {
            try
            {
                _logger->info(
                    __FILEREF__ + "readDirectory" +
                ...
                if (entry.path().filename().string().size() >=
                        recordedFileNamePrefix.size() &&
                    entry.path().filename().string().compare(
                        0, recordedFileNamePrefix.size(), recordedFileNamePrefix
                    ) == 0)
                {
    ...
    {
        _logger->error(
            __FILEREF__ + "isLastLiveRecorderFile failed" +
            ", e.what(): " + e.what()
        );
    }
    catch (exception &e)
...
)
{
    ...
    _logger->info(
        __FILEREF__ + "Received getMediaLiveRecorderStartTime" +
        ", ingestionJobKey: " + to_string(ingestionJobKey) +
        ", encodingJobKey: " + to_string(encodingJobKey) +
    ...
    {
        string errorMessage =
            __FILEREF__ + "wrong media live recorder format" +
            ", mediaLiveRecorderFileName: " + mediaLiveRecorderFileName;
        _logger->error(errorMessage);
    ...
    time_t utcMediaLiveRecorderStartTime =
        stol(mediaLiveRecorderFileName.substr(
            beginUTCIndex + 1, endIndex - beginUTCIndex + 1
        ));

    {
        ...
        int seconds =
            stoi(mediaLiveRecorderFileName.substr(beginUTCIndex - 2, 2));
        if (!isFirstChunk && seconds % segmentDurationInSeconds != 0)
        {
            ...
            {
                _logger->warn(
                    __FILEREF__ + "Wrong seconds (start time), force it to 0" +
                    ", ingestionJobKey: " + to_string(ingestionJobKey) +
                    ", encodingJobKey: " + to_string(encodingJobKey) +
            ...
            else if (seconds > halfSegmentDurationInSeconds &&
                     seconds < segmentDurationInSeconds)
            {
                _logger->warn(
                    __FILEREF__ + "Wrong seconds (start time), increase it" +
                    ", ingestionJobKey: " + to_string(ingestionJobKey) +
                    ", encodingJobKey: " + to_string(encodingJobKey) +
                    ", mediaLiveRecorderFileName: " +
                    mediaLiveRecorderFileName +
                ...
                utcMediaLiveRecorderStartTime +=
                    (segmentDurationInSeconds - seconds);
            }
        }
    }
...
)
{
    ...
    time_t utcCurrentRecordedFileLastModificationTime;
    {
        chrono::system_clock::time_point fileLastModification =
            chrono::time_point_cast<chrono::system_clock::duration>(
                fs::last_write_time(mediaLiveRecorderFileName) -
                fs::file_time_type::clock::now() + chrono::system_clock::now()
            );
        utcCurrentRecordedFileLastModificationTime =
            chrono::system_clock::to_time_t(fileLastModification);
    }
    ...
    {
        _logger->warn(
            __FILEREF__ + "Wrong seconds (end time), force it to 0" +
            ", ingestionJobKey: " + to_string(ingestionJobKey) +
            ", encodingJobKey: " + to_string(encodingJobKey) +
            ", mediaLiveRecorderFileName: " + mediaLiveRecorderFileName +
...
long LiveRecorderDaemons::buildAndIngestVirtualVOD(
    int64_t liveRecorderIngestionJobKey, int64_t liveRecorderEncodingJobKey,
    ...
    int64_t recordingCode, string liveRecorderIngestionJobLabel,
    string liveRecorderVirtualVODUniqueName,
    string liveRecorderVirtualVODRetention,
    int64_t liveRecorderVirtualVODImageMediaItemKey,
    int64_t liveRecorderUserKey, string liveRecorderApiKey,
    string mmsWorkflowIngestionURL, string mmsBinaryIngestionURL
)
{

    _logger->info(
        __FILEREF__ + "Received buildAndIngestVirtualVOD" +
        ...
        sourceSegmentsDirectoryPathName + ", sourceManifestFileName: " +
        sourceManifestFileName + ", stagingLiveRecorderVirtualVODPathName: " +
        ...
        to_string(liveRecorderVirtualVODImageMediaItemKey) +
    ...
    {
        {
            ...
            // ;
            {
                size_t endOfPathIndex =
                    stagingLiveRecorderVirtualVODPathName.find_last_of("/");
                if (endOfPathIndex == string::npos)
                {
                    string errorMessage =
                        string("No stagingLiveRecorderVirtualVODPathName found"
                        ) +
                        ", liveRecorderIngestionJobKey: " +
                        to_string(liveRecorderIngestionJobKey) +
                        ", liveRecorderEncodingJobKey: " +
                ...
                virtualVODM3u8DirectoryName =
                    stagingLiveRecorderVirtualVODPathName.substr(
                        endOfPathIndex + 1
                    );
            }
        ...
        string sourceManifestPathFileName =
            sourceSegmentsDirectoryPathName + "/" + sourceManifestFileName;
        if (!fs::exists(sourceManifestPathFileName.c_str()))
        {
            string errorMessage =
                string("manifest file not existing") +
                ", liveRecorderIngestionJobKey: " +
                to_string(liveRecorderIngestionJobKey) +
                ", liveRecorderEncodingJobKey: " +
                to_string(liveRecorderEncodingJobKey) +
        ...
        */
        ...
        string destManifestPathFileName =
            stagingLiveRecorderVirtualVODPathName + "/" +
        ...
        // create the destination directory and copy the manifest file
        {
            ...
            {
                _logger->info(
                    __FILEREF__ + "Creating directory" +
                    ", liveRecorderIngestionJobKey: " +
                    to_string(liveRecorderIngestionJobKey) +
                    ", liveRecorderEncodingJobKey: " +
                    to_string(liveRecorderEncodingJobKey) +
            ...
            _logger->info(
                __FILEREF__ + "Coping" + ", liveRecorderIngestionJobKey: " +
                to_string(liveRecorderIngestionJobKey) +
                ", liveRecorderEncodingJobKey: " +
                to_string(liveRecorderEncodingJobKey) +
                ", sourceManifestPathFileName: " + sourceManifestPathFileName +
        ...
        {
            _logger->info(
                __FILEREF__ + "Reading copied manifest file" +
                ", liveRecorderIngestionJobKey: " +
                to_string(liveRecorderIngestionJobKey) +
                ", liveRecorderEncodingJobKey: " +
                to_string(liveRecorderEncodingJobKey) +
            ...
            {
                string errorMessage =
                    string("Not authorized: manifest file not opened") +
                    ", liveRecorderIngestionJobKey: " +
                    to_string(liveRecorderIngestionJobKey) +
                    ", liveRecorderEncodingJobKey: " +
                    to_string(liveRecorderEncodingJobKey) +
            ...
            string manifestLine;
            {
                while (getline(ifManifestFile, manifestLine))
                {
                    ...
                    if (manifestLine.size() >= extInfPrefix.size() &&
                        0 == manifestLine.compare(
                                 0, extInfPrefix.size(), extInfPrefix
                             ))
                    {
                        break;
                    }
            ...
            do
            {
                         ...
                         ))
                {
                    ...
                    lastSegmentDuration = stod(manifestLine.substr(
                        extInfPrefix.size(),
                        endOfSegmentDuration - extInfPrefix.size()
                         ...
                         0 == manifestLine.compare(
                                  0, programDatePrefix.size(), programDatePrefix
                              ))
                    lastSegmentUtcStartTimeInMillisecs =
                        DateTime::sDateMilliSecondsToUtc(
                            manifestLine.substr(programDatePrefix.size())
                        );
                else if (manifestLine != "" && manifestLine[0] != '#')
                {
                    string sourceTSPathFileName =
                        sourceSegmentsDirectoryPathName + "/" + manifestLine;
                    string copiedTSPathFileName =
                        stagingLiveRecorderVirtualVODPathName + "/" +
                        manifestLine;
                    ...
                    {
                        string errorMessage =
                            string("copyFile failed, previous segments of the "
                                   "manifest "
                                   "will be omitted") +
                            ", sourceTSPathFileName: " + sourceTSPathFileName +
                            ", copiedTSPathFileName: " + copiedTSPathFileName +
                        ...
                        ofManifestFile.open(
                            destManifestPathFileName, ofstream::trunc
                        );
                        ofManifestFile << firstPartOfManifest;
                ...
                if (firstSegmentDuration == -1.0 &&
                    firstSegmentUtcStartTimeInMillisecs == -1 &&
                    lastSegmentDuration != -1.0 &&
                    lastSegmentUtcStartTimeInMillisecs != -1)
                {
                    firstSegmentDuration = lastSegmentDuration;
                    firstSegmentUtcStartTimeInMillisecs =
                        lastSegmentUtcStartTimeInMillisecs;
                }
        ...
        utcEndTimeInMilliSecs =
            lastSegmentUtcStartTimeInMillisecs + (lastSegmentDuration * 1000);
...

        {
            ...
            try
            {
                tarGzStagingLiveRecorderVirtualVODPathName =
                    stagingLiveRecorderVirtualVODPathName + ".tar.gz";
                ...
                {
                    string errorMessage =
                        string("No stagingLiveRecorderVirtualVODDirectory found"
                        ) +
                        ", liveRecorderIngestionJobKey: " +
                        to_string(liveRecorderIngestionJobKey) +
                        ", liveRecorderEncodingJobKey: " +
                ...
                _logger->info(
                    __FILEREF__ + "Start tar command " +
                    ", executeCommand: " + executeCommand +
                    ", secondsToSleep: " + to_string(secondsToSleep)
                );
                this_thread::sleep_for(chrono::seconds(secondsToSleep));

                chrono::system_clock::time_point startTar =
                    chrono::system_clock::now();
                int executeCommandStatus =
                    ProcessUtility::execute(executeCommand);
                chrono::system_clock::time_point endTar =
                    chrono::system_clock::now();
                _logger->info(
                    __FILEREF__ + "End tar command " +
                    ", executeCommand: " + executeCommand +
                ...
                {
                    string errorMessage =
                        string("ProcessUtility::execute failed") +
                        ", liveRecorderIngestionJobKey: " +
                        to_string(liveRecorderIngestionJobKey) +
                        ", liveRecorderEncodingJobKey: " +
                        to_string(liveRecorderEncodingJobKey) +
                        ", executeCommandStatus: " +
                        to_string(executeCommandStatus) +
                        ", executeCommand: " + executeCommand;
                    _logger->error(__FILEREF__ + errorMessage);
    ...
    try
    {
        workflowMetadata = buildVirtualVODIngestionWorkflow(
            liveRecorderIngestionJobKey, liveRecorderEncodingJobKey,
            externalEncoder,

            utcStartTimeInMilliSecs, utcEndTimeInMilliSecs, recordingCode,
            liveRecorderIngestionJobLabel,
            tarGzStagingLiveRecorderVirtualVODPathName,
            liveRecorderVirtualVODUniqueName, liveRecorderVirtualVODRetention,
            liveRecorderVirtualVODImageMediaItemKey
        );
    }
    catch (runtime_error e)
    {
        string errorMessage =
            string("build workflowMetadata live recorder VOD failed") +
            ", liveRecorderIngestionJobKey: " +
            to_string(liveRecorderIngestionJobKey) +
            ", liveRecorderEncodingJobKey: " +
            to_string(liveRecorderEncodingJobKey) + ", e.what: " + e.what();
        _logger->error(__FILEREF__ + errorMessage);
    ...
    try
    {
        ...
        string sResponse =
            MMSCURL::httpPostString(
                _logger, liveRecorderIngestionJobKey, mmsWorkflowIngestionURL,
                _mmsAPITimeoutInSeconds, to_string(liveRecorderUserKey),
    ...
    {
        string errorMessage =
            string("ingest live recorder VOD failed") +
            ", liveRecorderIngestionJobKey: " +
            to_string(liveRecorderIngestionJobKey) +
            ", liveRecorderEncodingJobKey: " +
            to_string(liveRecorderEncodingJobKey) +
            ", mmsWorkflowIngestionURL: " + mmsWorkflowIngestionURL +
            ", workflowMetadata: " + workflowMetadata + ", e.what: " + e.what();
        _logger->error(__FILEREF__ + errorMessage);
    ...
    if (externalEncoder)
    {
        ...
        try
        {
            ...
            {
                string errorMessage =
                    string("Ingested URL failed, addContentIngestionJobKey is "
                           "not valid") +
                    ", liveRecorderIngestionJobKey: " +
                    to_string(liveRecorderIngestionJobKey);
                _logger->error(__FILEREF__ + errorMessage);
            ...
            int64_t chunkFileSize =
                fs::file_size(tarGzStagingLiveRecorderVirtualVODPathName);

            mmsBinaryURL = mmsBinaryIngestionURL + "/" +
                           to_string(addContentIngestionJobKey);

            string sResponse = MMSCURL::httpPostFileSplittingInChunks(
                _logger, liveRecorderIngestionJobKey, mmsBinaryURL,
                _mmsBinaryTimeoutInSeconds, to_string(liveRecorderUserKey),
                liveRecorderApiKey, tarGzStagingLiveRecorderVirtualVODPathName,
                chunkFileSize,
                3 // maxRetryNumber
            );
        ...
        {
            _logger->error(
                __FILEREF__ + "Ingestion binary failed" +
                ", liveRecorderIngestionJobKey: " +
                to_string(liveRecorderIngestionJobKey) + ", mmsBinaryURL: " +
                mmsBinaryURL + ", workflowMetadata: " + workflowMetadata +
                ", exception: " + e.what()
            );
...
string LiveRecorderDaemons::buildVirtualVODIngestionWorkflow(
    ...
    int64_t utcStartTimeInMilliSecs, int64_t utcEndTimeInMilliSecs,
    int64_t recordingCode, string liveRecorderIngestionJobLabel,
    string tarGzStagingLiveRecorderVirtualVODPathName,
    string liveRecorderVirtualVODUniqueName,
    string liveRecorderVirtualVODRetention,
    ...
    try
    {
...

        {
            ...
            string utcToUtcString =
                DateTime::utcToUtcString(utcEndTimeInSeconds);
            utcToUtcString.insert(
                utcToUtcString.size() - 1,
                "." + to_string(utcEndTimeInMilliSecs % 1000)
            );
        ...
        if (liveRecorderVirtualVODImageMediaItemKey != -1)
        {
            ...
            {
                string errorMessage =
                    string("getMediaItemKeyDetailsByUniqueName failed") +
                    ", liveRecorderIngestionJobKey: " +
                    to_string(liveRecorderIngestionJobKey) +
                    ", liveRecorderEncodingJobKey: " +
                    to_string(liveRecorderEncodingJobKey) +
        ...
        _logger->info(
            ...
            to_string(liveRecorderIngestionJobKey) +
            ", liveRecorderEncodingJobKey: " +
            to_string(liveRecorderEncodingJobKey) + ", " + addContentLabel +
            ", "
        );
