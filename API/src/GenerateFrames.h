...
class GenerateFrames : public FFMPEGEncoderTask
{
  ...
  private:
	int64_t generateFrames_ingestFrame(
		int64_t ingestionJobKey, bool externalEncoder, string imagesDirectory, string generatedFrameFileName, string addContentTitle,
		json userDataRoot, string outputFileFormat, json ingestedParametersRoot, json encodingParametersRoot
	);
};
