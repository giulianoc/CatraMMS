        ...
        /* string& responseBody, */ Json::Value& responseBodyTasksRoot)
{
	...
	*/
	...
	//		is receiving the same input of the 'Face Recognition' task.
	...
	//		of the parent task.
	//		So, in case of OnError, the task (in our case the emailNotification task)
	//		will receive as input:
	//		1. the ReferenceIngestionJobKey of the granparent, in order to received
	//			the same input by his parent (scenario of the 2022-03-15 comment
	...
	string requestBody)
{
    ...
    try
    {
		...
		// switchBehaviour: applyNewPlaylistNow or applyNewPlaylistAtTheEndOfCurrentMedia
							...
							broadcastDefaultPlaylistItemRoot, field))
						{
								...
								referencePhysicalPathKeyIndex++)
							{
								...
								int64_t broadcastDefaultPhysicalPathKey = JSONUtils::asInt64(
									referencePhysicalPathKeyRoot, "physicalPathKey", -1);
		...
		try
		{
			...
			if (broadcastEncodingJobKey != -1)
			{
				...
				{
					pair<string, bool> encoderDetails = _mmsEngineDBFacade->getEncoderURL(
						broadcastEncoderKey);
