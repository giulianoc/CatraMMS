...
void API::ingestion(
    ...
    FCGX_Request &request, int64_t userKey, string apiKey,
...
)
{
    ...
    _logger->info(
        __FILEREF__ + "Received " + api + ", requestBody: " + requestBody
    );
    ...
    try
    {
        ...
        shared_ptr<PostgresConnection> conn =
            _mmsEngineDBFacade->beginIngestionJobs();
            ...
            */
...
#ifdef __POSTGRES__
            int64_t ingestionRootKey = _mmsEngineDBFacade->addIngestionRoot(
                conn, trans, workspace->_workspaceKey, userKey, rootType,
                rootLabel, requestBody.c_str()
            );
#else
            int64_t ingestionRootKey = _mmsEngineDBFacade->addIngestionRoot(
                conn, workspace->_workspaceKey, userKey, rootType, rootLabel,
                requestBody.c_str()
            );
#endif
            ...
            if (taskType == "GroupOfTasks")
            {
                ...
                ingestionGroupOfTasks(
                    conn, trans, userKey, apiKey, workspace, ingestionRootKey,
                    taskRoot, dependOnIngestionJobKeysForStarting,
                    localDependOnSuccess, dependOnIngestionJobKeysForStarting,
                    mapLabelAndIngestionJobKey,
                    /* responseBody, */ responseBodyTasksRoot
                );
#else
                ingestionGroupOfTasks(
                    conn, userKey, apiKey, workspace, ingestionRootKey,
                    taskRoot, dependOnIngestionJobKeysForStarting,
                    localDependOnSuccess, dependOnIngestionJobKeysForStarting,
                    mapLabelAndIngestionJobKey,
                    /* responseBody, */ responseBodyTasksRoot
            ...
            bool commit = true;
#ifdef __POSTGRES__
            _mmsEngineDBFacade->endIngestionJobs(
                conn, trans, commit, ingestionRootKey, processedMetadataContent
            );
#else
            _mmsEngineDBFacade->endIngestionJobs(
                conn, commit, ingestionRootKey, processedMetadataContent
            );
#endif
        ...
        catch (AlreadyLocked &e)
        {
            ...
            _logger->error(
                __FILEREF__ + "Ingestion locked" + ", e.what(): " + e.what()
            );
        ...
        catch (runtime_error &e)
        {
            ...
            _logger->error(
                __FILEREF__ + "request body parsing failed" +
                ", e.what(): " + e.what()
            );
        ...
        sendSuccess(
            sThreadId, requestIdentifier, responseBodyCompressed, request, "",
            api, 201, responseBody
        );

        chrono::system_clock::time_point endPoint = chrono::system_clock::now();
        _logger->info(
            __FILEREF__ + "Ingestion" +
            ", @MMS statistics@ - elapsed (secs): @" +
            to_string(
                chrono::duration_cast<chrono::seconds>(endPoint - startPoint)
                    .count()
            ) +
            "@"
        );
    ...
    {
        _logger->error(
            __FILEREF__ + "API failed" + ", API: " + api +
            ", requestBody: " + requestBody + ", e.what(): " + e.what()
        );
...
)
{
    ...
    try
    {
        ...
        {
            string sVariablesValuesToBeUsedRoot =
                JSONUtils::toString(variablesValuesToBeUsedRoot);
        ...
        if (JSONUtils::isMetadataPresent(requestBodyRoot, field))
        {
            ...
            // if (variablesRoot.size() > 0)
            {
                     ...
                     it != variablesRoot.end(); ++it)
                {
                    ...
                    _logger->info(
                        __FILEREF__ + "variable processing" + ", sKey: " + sKey
                    );
                    ...
                    string sValue;
                    {
                        ...
                        field = "type";
                        string variableType =
                            JSONUtils::asString(variableDetails, field, "");

                        field = "isNull";
                        bool variableIsNull =
                            JSONUtils::asBool(variableDetails, field, false);
                        ...
                        if (variablesValuesToBeUsedRoot == Json::nullValue)
                        {
                            ...
                            if (variableType == "string")
                            {
                                ...
                                {
                                    sValue = JSONUtils::asString(
                                        variableDetails, field, ""
                                    );
                            ...
                            else if (variableType == "integer")
                            {
                                if (variableIsNull)
                                    ...
                                    sValue = to_string(JSONUtils::asInt64(
                                        variableDetails, field, 0
                                    ));
                            }
                            else if (variableType == "decimal")
                            {
                                if (variableIsNull)
                                    ...
                                    sValue = to_string(JSONUtils::asDouble(
                                        variableDetails, field, 0.0
                                    ));
                            }
                            else if (variableType == "boolean")
                            {
                                ...
                                {
                                    bool bValue = JSONUtils::asBool(
                                        variableDetails, field, false
                                    );
                            ...
                            _logger->info(
                                __FILEREF__ + "variable information" +
                                ", sKey: " + sKey + ", variableType: " +
                                variableType + ", variableIsNull: " +
                                to_string(variableIsNull) +
                                ", sValue: " + sValue
                            );
                        }
                        else
                        {
                            ...
                            {
                                sValue = JSONUtils::asString(
                                    variablesValuesToBeUsedRoot, sKey, ""
                                );
                                ...
                                //contains " we have 	to replace it with \"
                                sValue =
                                    regex_replace(sValue, regex("\""), "\\\"");
                            }
                            else if (variableType == "integer")
                                sValue = to_string(JSONUtils::asInt64(
                                    variablesValuesToBeUsedRoot, sKey, 0
                                ));
                            else if (variableType == "decimal")
                                sValue = to_string(JSONUtils::asDouble(
                                    variablesValuesToBeUsedRoot, sKey, 0.0
                                ));
                            else if (variableType == "boolean")
                            {
                                bool bValue = JSONUtils::asBool(
                                    variablesValuesToBeUsedRoot, sKey, false
                                );
                                sValue = bValue ? "true" : "false";
                            }
                            else if (variableType == "datetime")
                    ...
                    _logger->info(
                        __FILEREF__ + "requestBody, replace" +
                    ...
                    while (true)
                    {
                        ...
                        index =
                            localRequestBody.find(variableToBeReplaced, index);
                        if (index == string::npos)
                            break;

                        // Make the replacement.
                        localRequestBody.replace(
                            index, variableToBeReplaced.length(), sValue
                        );
...
)
{
    ...
    // just for logging
    {
        ...
        {
            if (referenceIndex == 0)
                sDependOnIngestionJobKeysOverallInput = to_string(
            ...
            if (dependenciesToBeAddedToReferencesAt != "")
            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (dependenciesToBeAddedToReferencesAtIndex >
                            referencesRoot.size())
                            dependenciesToBeAddedToReferencesAtIndex =
                                referencesRoot.size();
                    }
                    ...
                    {
                        string errorMessage =
                            string("dependenciesToBeAddedToReferencesAt is not "
                                   "well format") +
                            ", dependenciesToBeAddedToReferencesAt: " +
                            dependenciesToBeAddedToReferencesAt;
                        _logger->error(__FILEREF__ + errorMessage);
             ...
             ++referenceIndex)
        {
            ...
            if (JSONUtils::isMetadataPresent(referenceRoot, field))
            {
                string referenceLabel =
                    JSONUtils::asString(referenceRoot, field, "");

                if (referenceLabel == "")
                {
                    string errorMessage =
                        __FILEREF__ + "The 'label' value cannot be empty" +
                ...
                vector<int64_t> ingestionJobKeys =
                    mapLabelAndIngestionJobKey[referenceLabel];

                if (ingestionJobKeys.size() == 0)
                {
                    string errorMessage =
                        __FILEREF__ + "The 'label' value is not found" +
                        ", processing label: " + taskOrGroupOfTasksLabel +
                        ", referenceLabel: " + referenceLabel;
                    _logger->error(errorMessage);
                ...
                else if (ingestionJobKeys.size() > 1)
                {
                    string errorMessage =
                        ...
                        to_string(ingestionJobKeys.size());
                    _logger->error(errorMessage);
                ...
                vector<int64_t>::iterator itrIngestionJobKey = find(
                    dependOnIngestionJobKeysForStarting.begin(),
                    dependOnIngestionJobKeysForStarting.end(),
                    ingestionJobKeys.back()
                );
                if (itrIngestionJobKey ==
                    dependOnIngestionJobKeysForStarting.end())
                    dependOnIngestionJobKeysForStarting.push_back(
                        ingestionJobKeys.back()
                    );
            }
        }
    ...
    _logger->info(
        __FILEREF__ + "add to referencesRoot all the inherited references?" +
        ", ingestionRootKey: " + to_string(ingestionRootKey) +
        ", taskOrGroupOfTasksLabel: " + taskOrGroupOfTasksLabel +
        ", IngestionType: " + ingestionType +
        ...
        to_string(dependOnIngestionJobKeysOverallInput.size())
    );

    // add to referencesRoot all the inherited references
    if ((!referencesSectionPresent ||
         dependenciesToBeAddedToReferencesAtIndex != -1) &&
        ...
        {
            {
                ...
                int dependOnIngestionJobKeysSize =
                    dependOnIngestionJobKeysOverallInput.size();
                ...
                _logger->info(
                    ...
                    to_string(previousReferencesRootSize) +
                    ", dependOnIngestionJobKeysSize: " +
                    to_string(dependOnIngestionJobKeysSize) +
                    ", dependenciesToBeAddedToReferencesAtIndex: " +
                    to_string(dependenciesToBeAddedToReferencesAtIndex)
                );
                ...
                {
                    _logger->info(
                        __FILEREF__ + "making 'space' in referencesRoot" +
                        ", ingestionRootKey: " + to_string(ingestionRootKey) +
                        ", from " + to_string(index) + " to " +
                        to_string(index + dependOnIngestionJobKeysSize)
                    );

                    referencesRoot[index + dependOnIngestionJobKeysSize] =
                        referencesRoot[index];
                }
                ...
                {
                    _logger->info(
                        __FILEREF__ + "fill in dependOnIngestionJobKey" +
                        ", ingestionRootKey: " + to_string(ingestionRootKey) +
                        ", from " +
                        to_string(
                            index - dependenciesToBeAddedToReferencesAtIndex
    ...
    if (taskOrGroupOfTasksLabel == "Check Streaming OnError")
    {
        ...
        _logger->info(
            __FILEREF__ + "testttttttt" + ", taskMetadata: " + taskMetadata +
            ", referencesSectionPresent: " +
            to_string(referencesSectionPresent) +
            ", dependenciesToBeAddedToReferencesAtIndex: " +
            to_string(dependenciesToBeAddedToReferencesAtIndex) +
            ", dependOnIngestionJobKeysOverallInput.size: " +
            to_string(dependOnIngestionJobKeysOverallInput.size())
...
#endif
{
    ...
    _logger->info(
        __FILEREF__ + "Processing SingleTask..." +
        ", ingestionRootKey: " + to_string(ingestionRootKey) +
        ", type: " + type + ", taskLabel: " + taskLabel
    );
    ...
    if (type == "Encode")
    {
        ...
        if (parametersSectionPresent &&
            (
                ...
                (JSONUtils::isMetadataPresent(
                     parametersRoot, encodingProfilesSetKeyField
                 ) ||
                 JSONUtils::isMetadataPresent(
                     parametersRoot, encodingProfilesSetLabelField
                 )) ||
                // case 2
                (JSONUtils::isMetadataPresent(
                     parametersRoot, referencesField
                 ) &&
            ...
            ))
        {
                ...
                ))
            {
                ...
                {
                    int64_t encodingProfilesSetKey = JSONUtils::asInt64(
                        parametersRoot, encodingProfilesSetKeyField, 0
                    );

                    encodingProfilesSetReference =
                        to_string(encodingProfilesSetKey);
                ...
                {
                    string encodingProfilesSetLabel = JSONUtils::asString(
                        parametersRoot, encodingProfilesSetLabelField, ""
                    );
            ...
            {
                string errorMessage =
                    __FILEREF__ + "It's not possible to be here" +
                    ", type: " + type + ", taskLabel: " + taskLabel;
                _logger->error(errorMessage);
            ...
            return ingestionGroupOfTasks(
                conn, trans, userKey, apiKey, workspace, ingestionRootKey,
                newTasksGroupRoot, dependOnIngestionJobKeysForStarting,
                dependOnSuccess, dependOnIngestionJobKeysOverallInput,
                mapLabelAndIngestionJobKey,
                /* responseBody, */ responseBodyTasksRoot
            );
#else
        ...
        {
            _logger->info(
                __FILEREF__ + "No special management for Encode" +
                ", ingestionRootKey: " + to_string(ingestionRootKey) +
                ", taskLabel: " + taskLabel + ", workspace->_workspaceKey: " +
    ...
    else if (type == "Face-Recognition")
    {
        ...
        {
            _logger->info(
                __FILEREF__ + "No special management for Face-Recognition" +
                ", ingestionRootKey: " + to_string(ingestionRootKey) +
                ", taskLabel: " + taskLabel + ", workspace->_workspaceKey: " +
                to_string(workspace->_workspaceKey)
            );
...
Json::Value internalMMSRoot;
    ...
    else if (type == "Workflow-As-Library")
    {
        ...
        string workflowLibraryContent;
        {
            ...
            if (!JSONUtils::isMetadataPresent(
                    parametersRoot, workflowAsLibraryTypeField
                ) ||
                !JSONUtils::isMetadataPresent(
                    parametersRoot, workflowAsLibraryLabelField
                ))
            {
                string errorMessage =
                    __FILEREF__ +
            ...
            string workflowAsLibraryType = JSONUtils::asString(
                parametersRoot, workflowAsLibraryTypeField, ""
            );
            string workflowAsLibraryLabel = JSONUtils::asString(
                parametersRoot, workflowAsLibraryLabelField, ""
            );
            ...
            workflowLibraryContent =
                _mmsEngineDBFacade->getWorkflowAsLibraryContent(
        ...
        return ingestionGroupOfTasks(
            conn, trans, userKey, apiKey, workspace, ingestionRootKey,
            newGroupOfTasksRoot, dependOnIngestionJobKeysForStarting,
            dependOnSuccess, dependOnIngestionJobKeysOverallInput,
            mapLabelAndIngestionJobKey,
            /* responseBody, */ responseBodyTasksRoot
        );
...
JSONUtils::toString(parametersRoot)
    ...
    */
    ...
    manageReferencesInput(
        ingestionRootKey, taskLabel, type, taskRoot, parametersSectionPresent,
        parametersRoot, dependOnIngestionJobKeysForStarting,
                 ...
                 ++waitForIndex)
            {
                ...
                if (JSONUtils::isMetadataPresent(waitForLabelRoot, field))
                {
                    string waitForGlobalIngestionLabel =
                        JSONUtils::asString(waitForLabelRoot, field, "");
                    ...
                    _logger->info(
                        __FILEREF__ + "getIngestionJobsKeyByGlobalLabel" +
                        ", ingestionRootKey: " + to_string(ingestionRootKey) +
                        ", taskLabel: " + taskLabel +
                        ...
                        to_string(waitForGlobalIngestionJobKeys.size())
                    );
                }
            }
        }
    ...
    string processingStartingFrom;
    {
        ...
        if (processingStartingFrom == "")
        {
            ...
            sprintf(
                sProcessingStartingFrom, "%04d-%02d-%02dT%02d:%02d:%02dZ",
                tmUTCDateTime.tm_year + 1900, tmUTCDateTime.tm_mon + 1,
                tmUTCDateTime.tm_mday, tmUTCDateTime.tm_hour,
                tmUTCDateTime.tm_min, tmUTCDateTime.tm_sec
            );
    ...
    _logger->info(
        __FILEREF__ + "add IngestionJob" + ", ingestionRootKey: " +
        to_string(ingestionRootKey) + ", taskLabel: " + taskLabel +
        ", taskMetadata: " + taskMetadata + ", IngestionType: " + type +
        ", processingStartingFrom: " + processingStartingFrom +
        ", dependOnIngestionJobKeysForStarting.size(): " +
        to_string(dependOnIngestionJobKeysForStarting.size()) +
...
#ifdef __POSTGRES__
    int64_t localDependOnIngestionJobKeyExecution =
        _mmsEngineDBFacade->addIngestionJob(
            conn, trans, workspace->_workspaceKey, ingestionRootKey, taskLabel,
            taskMetadata, MMSEngineDBFacade::toIngestionType(type),
            processingStartingFrom, dependOnIngestionJobKeysForStarting,
            dependOnSuccess, waitForGlobalIngestionJobKeys
        );
#else
    int64_t localDependOnIngestionJobKeyExecution =
        _mmsEngineDBFacade->addIngestionJob(
            conn, workspace->_workspaceKey, ingestionRootKey, taskLabel,
            taskMetadata, MMSEngineDBFacade::toIngestionType(type),
            processingStartingFrom, dependOnIngestionJobKeysForStarting,
            dependOnSuccess, waitForGlobalIngestionJobKeys
        );
    ...
    _logger->info(
        __FILEREF__ + "Save Label..." + ", ingestionRootKey: " +
        to_string(ingestionRootKey) + ", taskLabel: " + taskLabel +
        ", localDependOnIngestionJobKeyExecution: " +
        to_string(localDependOnIngestionJobKeyExecution)
    );
    if (taskLabel != "")
        (mapLabelAndIngestionJobKey[taskLabel])
            .push_back(localDependOnIngestionJobKeyExecution);
    ...
    localDependOnIngestionJobKeysForStarting.push_back(
        localDependOnIngestionJobKeyExecution
    );
    localDependOnIngestionJobKeysOverallInput.push_back(
        localDependOnIngestionJobKeyExecution
    );
    ...
    dependOnIngestionJobKeysOverallInputOnError.insert(
        dependOnIngestionJobKeysOverallInputOnError.end(),
...
#endif
{
    ...
    _logger->info(
        __FILEREF__ + "Processing GroupOfTasks..." + ", ingestionRootKey: " +
        to_string(ingestionRootKey) + ", groupOfTaskLabel: " + groupOfTaskLabel
    );
...
GroupOfTasks item"; _logger->error(errorMessage);
    ...
    */
    ...
    for (int taskIndex = 0; taskIndex < tasksRoot.size(); ++taskIndex)
    {
...
#ifdef __POSTGRES__
                localIngestionTaskDependOnIngestionJobKeyExecution =
                    ingestionGroupOfTasks(
                        conn, trans, userKey, apiKey, workspace,
                        ingestionRootKey, taskRoot,
                        dependOnIngestionJobKeysForStarting,
                        localDependOnSuccess,
                        dependOnIngestionJobKeysOverallInput,
                        mapLabelAndIngestionJobKey,
                        /* responseBody, */ responseBodyTasksRoot
                    );
                ...
                if (taskType == "GroupOfTasks")
                {
                    ...
                    localIngestionTaskDependOnIngestionJobKeyExecution =
                        ingestionGroupOfTasks(
                            conn, trans, userKey, apiKey, workspace,
                            ingestionRootKey, taskRoot,
                            lastDependOnIngestionJobKeysForStarting,
                            localDependOnSuccess,
                            dependOnIngestionJobKeysOverallInput,
                            mapLabelAndIngestionJobKey,
                            /* responseBody, */ responseBodyTasksRoot
                    ...
                    localIngestionTaskDependOnIngestionJobKeyExecution =
                        ingestionGroupOfTasks(
                            conn, userKey, apiKey, workspace, ingestionRootKey,
                            taskRoot, lastDependOnIngestionJobKeysForStarting,
                            localDependOnSuccess,
                            dependOnIngestionJobKeysOverallInput,
                            mapLabelAndIngestionJobKey,
                            /* responseBody, */ responseBodyTasksRoot
        ...
        for (int64_t localDependOnIngestionJobKey :
             localIngestionTaskDependOnIngestionJobKeyExecution)
        {
            // newDependOnIngestionJobKeysForStarting.push_back(localDependOnIngestionJobKey);
            newDependOnIngestionJobKeysOverallInputBecauseOfTasks.push_back(
                localDependOnIngestionJobKey
            );
        }
    }
    ...
    bool referencesOutputPresent = false;
    {
        ...
        if (referencesOutputPresent)
        {
                 ...
                 referenceIndex < referencesOutputRoot.size(); ++referenceIndex)
            {
                ...
                {
                    string referenceLabel =
                        JSONUtils::asString(referenceOutputRoot, field, "");
                    ...
                    if (ingestionJobKeys.size() == 0)
                    {
                        string errorMessage =
                            ...
                            ", referenceLabel: " + referenceLabel +
                            ", groupOfTasksRoot: " +
                            JSONUtils::toString(groupOfTasksRoot);
                        _logger->error(errorMessage);
                    ...
                    newDependOnIngestionJobKeysOverallInputBecauseOfReferencesOutput
                        .push_back(ingestionJobKeys.back());
        ...
        else if (newDependOnIngestionJobKeysOverallInputBecauseOfTasks.size() >
                 0)
        {
            _logger->info(
                __FILEREF__ +
                ...
                to_string(referencesOutputPresent) +
                ", "
            ...
            _logger->info(
                ...
                to_string(referencesOutputRoot.size())
            );
    ...
    _logger->info(
        __FILEREF__ + "add IngestionJob (Group of Tasks)" +
        ", ingestionRootKey: " + to_string(ingestionRootKey) +
        ", groupOfTaskLabel: " + groupOfTaskLabel +
        ", taskMetadata: " + taskMetadata + ", IngestionType: " + type +
        ", processingStartingFrom: " + processingStartingFrom +
    ...
    int64_t localDependOnIngestionJobKeyExecution =
        _mmsEngineDBFacade->addIngestionJob(
            conn, trans, workspace->_workspaceKey, ingestionRootKey,
            groupOfTaskLabel, taskMetadata,
    ...
    int64_t localDependOnIngestionJobKeyExecution =
        _mmsEngineDBFacade->addIngestionJob(
            conn, workspace->_workspaceKey, ingestionRootKey, groupOfTaskLabel,
            taskMetadata, MMSEngineDBFacade::toIngestionType(type),
            processingStartingFrom,
            referencesOutputPresent
                ? newDependOnIngestionJobKeysOverallInputBecauseOfReferencesOutput
    ...
    // IngestionJobKey is set
    {
        int64_t parentGroupOfTasksIngestionJobKey =
            localDependOnIngestionJobKeyExecution;
        for (int64_t childIngestionJobKey :
             newDependOnIngestionJobKeysOverallInputBecauseOfTasks)
        {
#ifdef __POSTGRES__
...
#else
            _mmsEngineDBFacade->updateIngestionJobParentGroupOfTasks(
                conn, childIngestionJobKey, parentGroupOfTasksIngestionJobKey
            );
#endif
        }
    }
    ...
    if (groupOfTaskLabel != "")
        (mapLabelAndIngestionJobKey[groupOfTaskLabel])
            .push_back(localDependOnIngestionJobKeyExecution);
    ...
    ingestionEvents(
        conn, trans, userKey, apiKey, workspace, ingestionRootKey,
        groupOfTasksRoot, localDependOnIngestionJobKeysForStarting,
        localDependOnIngestionJobKeysForStarting,
...
#else
    ingestionEvents(
        conn, userKey, apiKey, workspace, ingestionRootKey, groupOfTasksRoot,
        localDependOnIngestionJobKeysForStarting,
        localDependOnIngestionJobKeysForStarting,
    ...
    if (JSONUtils::isMetadataPresent(taskOrGroupOfTasksRoot, field))
    {
        ...
        if (taskType == "GroupOfTasks")
        {
            ...
            localIngestionJobKeys = ingestionGroupOfTasks(
                conn, trans, userKey, apiKey, workspace, ingestionRootKey,
                taskRoot, dependOnIngestionJobKeysForStarting,
                localDependOnSuccess, dependOnIngestionJobKeysOverallInput,
                mapLabelAndIngestionJobKey,
                /* responseBody, */ responseBodyTasksRoot
            );
#else
        ...
        // method
        {
            ...
            {
                for (int64_t localReferenceOutputIngestionJobKey :
                     referencesOutputIngestionJobKeys)
                {
#ifdef __POSTGRES__
                    _mmsEngineDBFacade->addIngestionJobDependency(
                        conn, trans, localIngestionJobKey, dependOnSuccess,
                        localReferenceOutputIngestionJobKey, orderNumber,
                        referenceOutputDependency
                    );
#else
                    _mmsEngineDBFacade->addIngestionJobDependency(
                        conn, localIngestionJobKey, dependOnSuccess,
                        localReferenceOutputIngestionJobKey, orderNumber,
                        referenceOutputDependency
                    );
#endif
...
void API::uploadedBinary(
    string sThreadId, int64_t requestIdentifier, bool responseBodyCompressed,
    FCGX_Request &request, string requestMethod,
...
)
{
    ...
    try
    {
        ...
        {
            string errorMessage =
                string("no rights to execute this method") +
        ...
        {
            string errorMessage =
                string("'ingestionJobKey' URI parameter is missing");
            _logger->error(__FILEREF__ + errorMessage);
        ...
        if (contentRangeIt != requestDetails.end())
        {
            ...
            {
                parseContentRange(
                    contentRange, contentRangeStart, contentRangeEnd,
                    contentRangeSize
                );
            ...
            {
                string errorMessage =
                    string("Content-Range is not well done. Expected format: "
        ...
        {
            string errorMessage =
                string("mmsEngineDBFacade->getIngestionJobDetails failed") +
                ", workspace->_workspaceKey: " +
                to_string(workspace->_workspaceKey) +
        ...
        to_string(ingestionJobKey)
            ...
            */
            ...
            _logger->info(
                ...
                to_string(ingestionJobKey) + ", sourceBinaryTransferred: " +
                to_string(sourceBinaryTransferred)
            );
        ...
        else
        {
            ...
            if (fs::exists(destBinaryPathName))
            {
                ...
                {
                    // content is reset
                    ofstream osDestStream(
                        destBinaryPathName.c_str(),
                ...
                unsigned long destBinaryPathNameSizeInBytes =
                    fs::file_size(destBinaryPathName);
                unsigned long sourceBinaryPathFileSizeInBytes =
                    fs::file_size(sourceBinaryPathFile);

                _logger->info(
                    __FILEREF__ + "Content-Range before concat" +
                    ", ingestionJobKey: " + to_string(ingestionJobKey) +
                    ...
                    to_string(destBinaryPathNameSizeInBytes) +
                    ", sourceBinaryPathFile: " + sourceBinaryPathFile +
                    ", sourceBinaryPathFileSizeInBytes: " +
                    to_string(sourceBinaryPathFileSizeInBytes)
                );
                ...
                while (contentRangeStart != destBinaryPathNameSizeInBytes &&
                       chrono::system_clock::now() < end)
                {
                ...
                {
                    string errorMessage =
                        string("Content-Range. This is NOT the next expected "
                ...
                try
                {
                    ...
                    chrono::system_clock::time_point start =
                        chrono::system_clock::now();
                    {
                        ofstream ofDestination(
                            destBinaryPathName,
                            std::ios_base::binary | std::ios_base::app
                        );
                        ifstream ifSource(
                            sourceBinaryPathFile, std::ios_base::binary
                        );
                    ...
                    _logger->info(
                        __FILEREF__ + "Content-Range after concat" +
                        ", ingestionJobKey: " + to_string(ingestionJobKey) +
                        ", contentRangeStart: " + to_string(contentRangeStart) +
                        ", contentRangeEnd: " + to_string(contentRangeEnd) +
                        ", contentRangeSize: " + to_string(contentRangeSize) +
                        ...
                        ", destBinaryPathName: " + destBinaryPathName +
                        ", destBinaryPathNameSizeInBytes: " +
                        to_string(fs::file_size(destBinaryPathName)) +
                        ", sourceBinaryPathFile: " + sourceBinaryPathFile +
                        ", sourceBinaryPathFileSizeInBytes: " +
                        to_string(sourceBinaryPathFileSizeInBytes) +
                        ", concat elapsed (secs): " +
                ...
                {
                    string errorMessage =
                        string("Content-Range. Error to concat file") +
                        ", ingestionJobKey: " + to_string(ingestionJobKey) +
                        ", destBinaryPathName: " + destBinaryPathName +
                        ", sourceBinaryPathFile: " + sourceBinaryPathFile;
                    _logger->error(__FILEREF__ + errorMessage);
            ...
            else
            {
                ...
                {
                    string errorMessage =
                        string("Content-Range. This is the first chunk of the "
                ...
                try
                {
                    ...
                    MMSStorage::move(
                        ingestionJobKey, sourceBinaryPathFile,
                        destBinaryPathName, _logger
                    );
                }
                catch (exception &e)
                {
                    string errorMessage =
                        string("Content-Range. Error to move file") +
                        ", ingestionJobKey: " + to_string(ingestionJobKey) +
                        ", sourceBinaryPathFile: " + sourceBinaryPathFile +
                        ", destBinaryPathName: " + destBinaryPathName;
                    _logger->error(__FILEREF__ + errorMessage);
            ...
            if (contentRangeEnd + 1 == contentRangeSize)
            {
                ...
                */
                ...
                bool sourceBinaryTransferred = true;
                _logger->info(
                    __FILEREF__ + "Content-Range. Update IngestionJob" +
                    ", ingestionJobKey: " + to_string(ingestionJobKey) +
...
void API::fileUploadProgressCheck()
{
    ...
    while (!_fileUploadProgressThreadShutdown)
    {
        ...
        for (auto itr = _fileUploadProgressData
                            ->_filesUploadProgressToBeMonitored.begin();
             itr !=
             _fileUploadProgressData->_filesUploadProgressToBeMonitored.end();)
        {
            ...
            if (itr->_callFailures >= _maxProgressCallFailures)
            {
                _logger->error(
                    ...
                    to_string(_maxProgressCallFailures)
                );
                itr = _fileUploadProgressData->_filesUploadProgressToBeMonitored
                          .erase(itr); // returns iterator to the next element
            ...
            try
            {
                ...
                string progressIdHeader =
                    string("X-Progress-ID: ") + itr->_progressId;
                string hostHeader =
                    string("Host: ") + itr->_binaryVirtualHostName;

                _logger->info(
                    __FILEREF__ + "Call for upload progress" +
                    ", ingestionJobKey: " + to_string(itr->_ingestionJobKey) +
                    ", progressId: " + itr->_progressId +
                    ", binaryVirtualHostName: " + itr->_binaryVirtualHostName +
                    ", binaryListenHost: " + itr->_binaryListenHost +
                    ", callFailures: " + to_string(itr->_callFailures) +
                    ", progressURL: " + progressURL + ", progressIdHeader: " +
                    progressIdHeader + ", hostHeader: " + hostHeader
                );
    ...
    + ", sResponse: " + sResponse
                ...
                */
                ...
                try
                {
                    ...
                    string state = JSONUtils::asString(
                        uploadProgressResponse, "state", ""
                    );
                    if (state == "done")
                    {
                        ...
                        if (itr->_contentRangePresent)
                            absoluteProgress = ((double)absoluteReceived /
                                                (double)absoluteSize) *
                                               100;
                        ...
                        if (itr->_contentRangePresent)
                            absoluteUploadingPercentage =
                                ((double)((int)(absoluteProgress * 10))) / 10;

                        if (itr->_contentRangePresent)
                        {
                            _logger->info(
                                __FILEREF__ + "Upload just finished" +
                                ", ingestionJobKey: " +
                                to_string(itr->_ingestionJobKey) +
                                ", progressId: " + itr->_progressId +
                                ", binaryVirtualHostName: " +
                                itr->_binaryVirtualHostName +
                                ", binaryListenHost: " +
                                itr->_binaryListenHost +
                                ", relativeProgress: " +
                                to_string(relativeProgress) +
                                ", relativeUploadingPercentage: " +
                                to_string(relativeUploadingPercentage) +
                                ", absoluteProgress: " +
                                to_string(absoluteProgress) +
                                ", absoluteUploadingPercentage: " +
                                to_string(absoluteUploadingPercentage) +
                                ", lastPercentageUpdated: " +
                                to_string(itr->_lastPercentageUpdated)
                            );
                        }
                        else
                        {
                        ...
                        if (itr->_contentRangePresent)
                        {
                            ...
                            _mmsEngineDBFacade
                                ->updateIngestionJobSourceUploadingInProgress(
                        ...
                        itr = _fileUploadProgressData
                                  ->_filesUploadProgressToBeMonitored.erase(itr
                    ...
                    {
                        int64_t relativeReceived = JSONUtils::asInt64(
                            uploadProgressResponse, "received", 0
                        );
                        int64_t absoluteReceived = -1;
                        if (itr->_contentRangePresent)
                            absoluteReceived =
                                relativeReceived + itr->_contentRangeStart;
                        int64_t relativeSize = JSONUtils::asInt64(
                            uploadProgressResponse, "size", 0
                        );
                        int64_t absoluteSize = -1;
                        if (itr->_contentRangePresent)
                            absoluteSize = itr->_contentRangeSize;
                        ...
                        double relativeProgress =
                            ((double)relativeReceived / (double)relativeSize) *
                            100;
                        double absoluteProgress;
                        if (itr->_contentRangePresent)
                            absoluteProgress = ((double)absoluteReceived /
                        ...
                        // this is to have one decimal in the percentage
                        double relativeUploadingPercentage =
                            ((double)((int)(relativeProgress * 10))) / 10;
                        double absoluteUploadingPercentage;
                        if (itr->_contentRangePresent)
                            absoluteUploadingPercentage =
                                ((double)((int)(absoluteProgress * 10))) / 10;
                        ...
                        if (itr->_contentRangePresent)
                        {
                            _logger->info(
                                ...
                                to_string(itr->_lastPercentageUpdated) +
                                ", relativeReceived: " +
                                to_string(relativeReceived) +
                                ", absoluteReceived: " +
                                to_string(absoluteReceived) +
                    ...
                    {
                        string errorMessage =
                            string("file upload progress. State is wrong") +
                            ", state: " + state + ", ingestionJobKey: " +
                            to_string(itr->_ingestionJobKey) +
                            ", progressId: " + itr->_progressId +
                            ", binaryVirtualHostName: " +
                            itr->_binaryVirtualHostName +
                ...
                {
                    string errorMessage =
                        string("response Body json is not well format")
                        // + ", sResponse: " + sResponse
                        ;
                    _logger->error(__FILEREF__ + errorMessage);
            ...
            catch (curlpp::LogicError &e)
            {
                _logger->error(
                    __FILEREF__ +
                    "Call for upload progress failed (LogicError)" +
                    ", ingestionJobKey: " + to_string(itr->_ingestionJobKey) +
                    ", progressId: " + itr->_progressId +
                    ", binaryVirtualHostName: " + itr->_binaryVirtualHostName +
                    ", binaryListenHost: " + itr->_binaryListenHost +
            ...
            catch (curlpp::RuntimeError &e)
            {
                _logger->error(
                    __FILEREF__ +
                    "Call for upload progress failed (RuntimeError)" +
                    ", ingestionJobKey: " + to_string(itr->_ingestionJobKey) +
                    ", progressId: " + itr->_progressId +
                    ", binaryVirtualHostName: " + itr->_binaryVirtualHostName +
                    ", binaryListenHost: " + itr->_binaryListenHost +
            ...
            catch (runtime_error e)
            {
                _logger->error(
                    __FILEREF__ +
                    "Call for upload progress failed (runtime_error)" +
                    ", ingestionJobKey: " + to_string(itr->_ingestionJobKey) +
                    ", progressId: " + itr->_progressId +
                    ", binaryVirtualHostName: " + itr->_binaryVirtualHostName +
                    ", binaryListenHost: " + itr->_binaryListenHost +
            ...
            catch (exception e)
            {
                _logger->error(
                    __FILEREF__ +
                    "Call for upload progress failed (exception)" +
                    ", ingestionJobKey: " + to_string(itr->_ingestionJobKey) +
                    ", progressId: " + itr->_progressId +
                    ", binaryVirtualHostName: " + itr->_binaryVirtualHostName +
                    ", binaryListenHost: " + itr->_binaryListenHost +
...
void API::ingestionRootsStatus(
    string sThreadId, int64_t requestIdentifier, bool responseBodyCompressed,
    FCGX_Request &request, shared_ptr<Workspace> workspace,
    unordered_map<string, string> queryParameters, string requestBody
)
{
    ...
    try
    {
        ...
        if (rowsIt != queryParameters.end() && rowsIt->second != "")
        {
            ...
            if (rows > _maxPageSize)
            {
                ...
                string errorMessage =
                    __FILEREF__ + "rows parameter too big" +
                    ", rows: " + to_string(rows) +
        ...
        if (labelIt != queryParameters.end() && labelIt->second != "")
        {
            ...
            string plus = "\\+";
            string plusDecoded = " ";
            string firstDecoding =
                regex_replace(label, regex(plus), plusDecoded);
        ...
        auto ingestionJobOutputsIt =
            queryParameters.find("ingestionJobOutputs");
        if (ingestionJobOutputsIt != queryParameters.end() &&
            ingestionJobOutputsIt->second != "")
        {
        ...
        if (dependencyInfoIt != queryParameters.end() &&
            dependencyInfoIt->second != "")
        {
...

        {
            Json::Value ingestionStatusRoot =
                _mmsEngineDBFacade->getIngestionRootsStatus(
                    ...
                    startIngestionDate, endIngestionDate, label, status, asc,
...
)
{
    ...
    try
    {
        ...
        {
            string errorMessage =
                string("The 'ingestionRootKey' parameter is not found");
            _logger->error(__FILEREF__ + errorMessage);
        ...
        if (processedMetadataIt != queryParameters.end() &&
            processedMetadataIt->second != "")
            processedMetadata =
                (processedMetadataIt->second == "true" ? true : false);

        {
            ...
            sendSuccess(
                sThreadId, requestIdentifier, responseBodyCompressed, request,
                "", api, 200, ingestionRootMetaDataContent
            );
        }
    }
    catch (runtime_error &e)
...
)
{
    ...
    try
    {
        ...
        if (ingestionTypeIt != queryParameters.end() &&
            ingestionTypeIt->second != "")
        {
        ...
        if (configurationLabelIt != queryParameters.end() &&
            configurationLabelIt->second != "")
        {
            ...
            string plusDecoded = " ";
            string firstDecoding =
                regex_replace(configurationLabel, regex(plus), plusDecoded);
        ...
        if (outputChannelLabelIt != queryParameters.end() &&
            outputChannelLabelIt->second != "")
        {
            ...
            string plusDecoded = " ";
            string firstDecoding =
                regex_replace(outputChannelLabel, regex(plus), plusDecoded);
        ...
        if (recordingCodeIt != queryParameters.end() &&
            recordingCodeIt->second != "")
        {
        ...
        auto broadcastIngestionJobKeyNotNullIt =
            queryParameters.find("broadcastIngestionJobKeyNotNull");
        ...
        auto jsonParametersConditionIt =
            queryParameters.find("jsonParametersCondition");
        if (jsonParametersConditionIt != queryParameters.end() &&
            jsonParametersConditionIt->second != "")
        {
            ...
            string plus = "\\+";
            string plusDecoded = " ";
            string firstDecoding = regex_replace(
                jsonParametersCondition, regex(plus), plusDecoded
            );
...

        {
            Json::Value ingestionStatusRoot =
                _mmsEngineDBFacade->getIngestionJobsStatus(
                    ...
                    endIngestionDate, startScheduleDate, ingestionType,
                    configurationLabel, outputChannelLabel, recordingCode,
                    broadcastIngestionJobKeyNotNull, jsonParametersCondition,
                    asc, status, dependencyInfo, ingestionJobOutputs, fromMaster
                );
...
)
{
    ...
    try
    {
        ...
        {
            string errorMessage =
                string("The 'ingestionJobKey' parameter is not found");
            _logger->error(__FILEREF__ + errorMessage);
        ...
        if (forceCancelIt != queryParameters.end() &&
            forceCancelIt->second != "")
        {
        ...
        tie(ignore, ignore, ingestionStatus, ignore, ignore) =
            ingestionJobDetails;
...
void API::updateIngestionJob(
    string sThreadId, int64_t requestIdentifier, bool responseBodyCompressed,
    FCGX_Request &request, shared_ptr<Workspace> workspace, int64_t userKey,
    unordered_map<string, string> queryParameters, string requestBody,
    bool admin
)
{
    ...
    try
    {
        ...
        {
            _logger->info(
                __FILEREF__ + "getIngestionJobDetails" +
                ", workspace->_workspaceKey: " +
                to_string(workspace->_workspaceKey) +
                ", ingestionJobKey: " + to_string(ingestionJobKey)
            );
            ...
            tie(label, ingestionType, ingestionStatus, metaDataContent,
                ignore) = ingestionJobDetails;

            if (ingestionStatus !=
                MMSEngineDBFacade::IngestionStatus::Start_TaskQueued)
            {
                string errorMessage =
                    string("It is not possible to update an IngestionJob that "
            ...
            {
                string errorMessage =
                    string("IngestionType field is missing") +
                    ", ingestionJobKey: " + to_string(ingestionJobKey);
                _logger->error(__FILEREF__ + errorMessage);

                throw runtime_error(errorMessage);
            }
            string sIngestionType =
                JSONUtils::asString(metadataRoot, "IngestionType", "");
                ...
                ))
            {
                ...
                {
                    string errorMessage =
                        string("It was requested an Update of Live-Recorder "
...

                {
...

                    {
                        ...
                        if (JSONUtils::isMetadataPresent(metadataRoot, field))
                        {
                            ...
                            newIngestionJobLabel = JSONUtils::asString(
                                metadataRoot, "IngestionJobLabel", ""
                            );
                        }
                        ...
                        if (JSONUtils::isMetadataPresent(metadataRoot, field))
                        {
                            ...
                            newChannelLabel = JSONUtils::asString(
                                metadataRoot, "ChannelLabel", ""
                            );
                        }
                        ...
                        if (JSONUtils::isMetadataPresent(metadataRoot, field))
                        {
                            ...
                            newRecordingPeriodStart = JSONUtils::asString(
                                metadataRoot, "scheduleStart", ""
                            );
                        }
                        ...
                        if (JSONUtils::isMetadataPresent(metadataRoot, field))
                        {
                            ...
                            newRecordingPeriodEnd = JSONUtils::asString(
                                metadataRoot, "scheduleEnd", ""
                            );
                        }
                        ...
                        if (JSONUtils::isMetadataPresent(metadataRoot, field))
                        {
                            ...
                            newRecordingVirtualVOD = JSONUtils::asBool(
                                metadataRoot, "RecordingVirtualVOD", false
                            );
                        }
                    }
                    ...
                    _logger->info(
                        __FILEREF__ + "IngestionJob updated" +
                        ", workspaceKey: " +
                        to_string(workspace->_workspaceKey) +
                        ", ingestionJobKey: " + to_string(ingestionJobKey)
                    );
        ...
        {
            _logger->error(
                __FILEREF__ + api + " failed" + ", e.what(): " + e.what()
            );
...
)
{
    ...
    try
    {
        ...
        // switchBehaviour: applyNewPlaylistNow or
        // applyNewPlaylistAtTheEndOfCurrentMedia
        ...
        _logger->info(
            __FILEREF__ + api + ", broadcasterIngestionJobKey: " +
            to_string(broadcasterIngestionJobKey) +
        ...
        try
        {
            ...
            tie(ignore, ingestionType, ingestionStatus, metaDataContent,
                ignore) = ingestionJobDetails;

            if (ingestionType != MMSEngineDBFacade::IngestionType::LiveProxy)
            {
                string errorMessage =
                    string("Ingestion type is not a Live/VODProxy") +
                    ", broadcasterIngestionJobKey: " +
                    to_string(broadcasterIngestionJobKey) +
                    ", ingestionType: " +
                    MMSEngineDBFacade::toString(ingestionType);
            ...
            if (sIngestionStatus.size() >= prefixIngestionStatus.size() &&
                0 == sIngestionStatus.compare(
                         0, prefixIngestionStatus.size(), prefixIngestionStatus
                     ))
            {
                string errorMessage =
                    string("Ingestion job is already finished") +
                    ", broadcasterIngestionJobKey: " +
                    to_string(broadcasterIngestionJobKey) +
            ...
            if (!JSONUtils::isMetadataPresent(metadataContentRoot, field))
            {
                string errorMessage =
                    ...
                    ", broadcasterIngestionJobKey: " +
                    to_string(broadcasterIngestionJobKey) + ", Field: " + field;
                _logger->error(errorMessage);
            ...
            broadcastIngestionJobKey =
                JSONUtils::asInt64(broadcasterRoot, field, 0);
            if (broadcastIngestionJobKey == 0)
            {
                string errorMessage = __FILEREF__ +
                                      "No broadcastIngestionJobKey found" +
                                      ", broadcasterIngestionJobKey: " +
                                      to_string(broadcasterIngestionJobKey) +
                                      ", broadcastIngestionJobKey: " +
                                      to_string(broadcastIngestionJobKey);
                _logger->error(errorMessage);
            ...
            field = "timePeriod";
            bool timePeriod =
                JSONUtils::asBool(metadataContentRoot, field, false);
            if (!timePeriod)
            {
                string errorMessage =
                    __FILEREF__ +
                    "The LiveProxy IngestionJob has to have timePeriod" +
                    ", broadcasterIngestionJobKey: " +
                    to_string(broadcasterIngestionJobKey) +
            ...
            if (JSONUtils::isMetadataPresent(broadcasterRoot, field))
            {
                ...
                {
                    broadcastDefaultMediaType = JSONUtils::asString(
                        broadcastDefaultPlaylistItemRoot, field, ""
                    );
                            ...
                            ))
                        {
                                 ...
                                 referencePhysicalPathKeyIndex++)
                            {
                                ...
                                string sourcePhysicalPathName;
                                {
                                    ...
                                    tie(sourcePhysicalPathName, ignore, ignore,
                                    ...
                                    tie(ignore, vodContentType, ignore, ignore,
                    ...
                    else if (broadcastDefaultMediaType == "Countdown")
                    {
                        ...
                        field = "text";
                        string broadcastDefaultText = JSONUtils::asString(
                            broadcastDefaultPlaylistItemRoot, field, ""
                        );
                            ...
                            ))
                        {
                            string errorMessage =
                                __FILEREF__ +
                                "Countdown has to have drawTextDetails" +
                                ", broadcasterIngestionJobKey: " +
                                to_string(broadcasterIngestionJobKey) +
                        ...
                        broadcastDefaultCountdownInputRoot =
                            _mmsEngineDBFacade->getCountdownInputRoot(
                                ...
                                sourcePhysicalDeliveryURL,
                                broadcastDefaultPhysicalPathKey,
                                videoDurationInMilliSeconds, drawTextDetailsRoot
                            );
                    }
                    else if (broadcastDefaultMediaType == "Direct URL")
                    {
                        field = "url";
                        string broadcastDefaultURL = JSONUtils::asString(
                            broadcastDefaultPlaylistItemRoot, field, ""
                        );
                        ...
                        broadcastDefaultDirectURLInputRoot =
                            _mmsEngineDBFacade->getDirectURLInputRoot(
                                broadcastDefaultURL, drawTextDetailsRoot
                            );
                    }
                    else
                    {
        ...
        try
        {
                            ...
                            ))
                        {
                                 ...
                                 sourceIndex++)
                            {
                                ...
                                int64_t physicalPathKey =
                                    JSONUtils::asInt64(sourceRoot, field, -1);
            ...
            vector<Json::Value> vNewReceivedPlaylist;
            {
                ...
                sort(
                    ...
                    [](Json::Value aRoot, Json::Value bRoot)
                    {
                        ...
                        int64_t bUtcProxyPeriodStart =
                            JSONUtils::asInt64(bRoot, "utcScheduleStart", -1);
                ...
                _logger->info(
                    __FILEREF__ + "Sort playlist items" +
                    ", vNewReceivedPlaylist.size: " +
                    to_string(vNewReceivedPlaylist.size())
                );
            }
            ...
            // precedenti a 'now'
            {
                     ...
                     newReceivedPlaylistIndex++)
                {
                    ...
                    int64_t utcProxyPeriodStart = JSONUtils::asInt64(
                        newReceivedPlaylistItemRoot, "utcScheduleStart", -1
                    );
                    int64_t utcProxyPeriodEnd = JSONUtils::asInt64(
                        newReceivedPlaylistItemRoot, "utcScheduleEnd", -1
                    );
                ...
                {
                    _logger->info(
                        __FILEREF__ + "Erase playlist items in the past: " +
                        to_string(
                            currentPlaylistIndex - leavePastEntriesNumber
                        ) +
                        " items" + ", currentPlaylistIndex: " +
                        to_string(currentPlaylistIndex) +
                        ", leavePastEntriesNumber: " +
                        to_string(leavePastEntriesNumber) +
                        ", vNewReceivedPlaylist.size: " +
                        to_string(vNewReceivedPlaylist.size())
                    );

                    vNewReceivedPlaylist.erase(
                        vNewReceivedPlaylist.begin(),
                        vNewReceivedPlaylist.begin() +
                            (currentPlaylistIndex - leavePastEntriesNumber)
                    );
                }
            ...
            // add the default media in case of hole filling newPlaylistRoot
            {
                     ...
                     newReceivedPlaylistIndex++)
                {
                    ...
                    int64_t utcProxyPeriodStart = JSONUtils::asInt64(
                        newReceivedPlaylistItemRoot, field, -1
                    );
                    ...
                    _logger->info(
                        __FILEREF__ + "Processing newReceivedPlaylistRoot" +
                         ...
                         // utcProxyPeriodStart)
                    {
                        ...
                        newdPlaylistItemToBeAddedRoot[field] =
                            utcCurrentBroadcasterStart;
        ...
        try
        {
                ...
                ingestionType != MMSEngineDBFacade::IngestionType::Countdown)
            {
                string errorMessage =
                    ...
                    ", broadcastIngestionJobKey: " +
                    to_string(broadcastIngestionJobKey) + ", ingestionType: " +
                    MMSEngineDBFacade::toString(ingestionType);
                _logger->error(__FILEREF__ + errorMessage);
            ...
            try
            {
                ...
                tie(broadcastEncodingJobKey, broadcastEncoderKey,
                    broadcastParameters) = encodingJobDetails;
            }
            catch (runtime_error &e)
            {
            ...
            if (broadcastEncodingJobKey != -1)
            {
                ...
                string newBroadcastParameters =
                    JSONUtils::toString(broadcastParametersRoot);
                ...
                if (broadcastEncoderKey > 0)
                {
                    pair<string, bool> encoderDetails =
                        _mmsEngineDBFacade->getEncoderURL(broadcastEncoderKey);
            ...
            else
            {
                ...
                string newMetadataContentRoot =
                    JSONUtils::toString(metadataContentRoot);
        ...
        catch (curlpp::LogicError &e)
        {
            _logger->error(
                __FILEREF__ +
                "changeLiveProxyPlaylist URL failed (LogicError)" +
                ", ffmpegEncoderURL: " + ffmpegEncoderURL + ", exception: " +
                e.what() + ", response.str(): " + response.str()
            );
        ...
        {
            string errorMessage =
                string("changeLiveProxyPlaylist URL failed (RuntimeError)") +
        ...
        catch (runtime_error e)
        {
            _logger->error(
                __FILEREF__ +
                "changeLiveProxyPlaylist URL failed (runtime_error)" +
                ", ffmpegEncoderURL: " + ffmpegEncoderURL + ", exception: " +
                e.what() + ", response.str(): " + response.str()
            );
        ...
        {
            _logger->error(
                __FILEREF__ + "changeLiveProxyPlaylist URL failed (exception)" +
                ", ffmpegEncoderURL: " + ffmpegEncoderURL + ", exception: " +
                e.what() + ", response.str(): " + response.str()
            );
